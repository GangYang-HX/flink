...
public class BackendRestorerProcedure<
	...
	public T createAndRestore(@Nonnull List<? extends Collection<S>> restoreOptions) throws Exception {
		...
		LOG.error("Could not restore " + logDescription + " from any of the " + restoreOptions.size() +
			" provided restore options.", collectedException);
		Exception finalCollectedException = collectedException;
		ExceptionUtils.findThrowable(collectedException, RestoreCheckpointException.class).ifPresent(e -> {
			throw new RestoreCheckpointException(e.getRestoreFailureReason(), finalCollectedException);
		} );

		throw new RestoreCheckpointException(RestoreFailureReason.STATE_RESTORED_EXCEPTION, collectedException);
	}
