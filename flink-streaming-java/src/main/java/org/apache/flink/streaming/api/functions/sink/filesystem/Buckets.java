...
import org.apache.flink.core.fs.MetricsWrapperFactory;
...
import org.apache.flink.streaming.api.functions.sink.filesystem.metrics.HadoopFileMetricsWrapper;
...
public class Buckets<IN, BucketID> {
	...
	public Buckets(
		...
		this(basePath, bucketAssigner, bucketFactory, bucketWriter, rollingPolicy, subtaskIndex, outputFileConfig);
		if (bucketWriter instanceof BulkBucketWriter) {
			RecoverableWriter recoverableWriter = ((BulkBucketWriter<IN, BucketID>) bucketWriter).recoverableWriter;
			if (recoverableWriter instanceof HadoopRecoverableWriter) {
				HadoopRecoverableWriter hadoopRecoverableWriter = (HadoopRecoverableWriter) recoverableWriter;
				MetricsWrapper metricsWrapper = MetricsWrapperFactory.create(HadoopFileMetricsWrapper.class).register(metricGroup);
				hadoopRecoverableWriter.setMetricWrapper(metricsWrapper);
			}
		} else if (bucketWriter instanceof RowWiseBucketWriter) {
			RecoverableWriter recoverableWriter = ((RowWiseBucketWriter<IN, BucketID>) bucketWriter).recoverableWriter;
			if (recoverableWriter instanceof HadoopRecoverableWriter) {
				HadoopRecoverableWriter hadoopRecoverableWriter = (HadoopRecoverableWriter) recoverableWriter;
				MetricsWrapper metricsWrapper = MetricsWrapperFactory.create(HadoopFileMetricsWrapper.class).register(metricGroup);
				hadoopRecoverableWriter.setMetricWrapper(metricsWrapper);
