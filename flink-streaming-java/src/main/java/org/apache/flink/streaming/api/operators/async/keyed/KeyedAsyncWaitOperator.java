...
/**
 * The {@link KeyedAsyncWaitOperator} allows to asynchronously process incoming stream records. For that
 ...
 * opened tail to head. The reason for this is that an opened {@link KeyedAsyncWaitOperator} starts
...
public class KeyedAsyncWaitOperator<IN, OUT>
        ...
        implements OneInputStreamOperator<IN, OUT>, BoundedOneInput, Triggerable {
    ...
    private MapState<Long, List<RetryRecord>> elementData;
    ...
    public void open() throws Exception {
        ...
        if (recoveredStreamElements != null) {
            ...
            InternalTimerService<VoidNamespace> internalTimerService =
                    getInternalTimerService("user-async-timers", VoidNamespaceSerializer.INSTANCE, this);

            timerService = new SimpleTimerService(internalTimerService);

            MapStateDescriptor<Long, List<RetryRecord>> elementDataDesc = new MapStateDescriptor("keyAsyncWaitOperator map",
                    BasicTypeInfo.LONG_TYPE_INFO, new ListTypeInfo<>(RetryRecord.class));
            elementData = getRuntimeContext().getMapState(elementDataDesc);
        }
    ...
    public void processElement(StreamRecord<IN> element) throws Exception {
        ...
        //wrapElement
        RetryRecord newElement = new RetryRecord(getCurrentKey(), element, 0);
        final ResultHandler resultHandler = new ResultHandler(newElement, entry);
    ...
    public void retryElement(RetryRecord retryRecord) throws Exception {

        // add element first to the queue
        final ResultFuture<OUT> entry = addToWorkQueue(retryRecord.realRecord);

        final ResultHandler resultHandler = new ResultHandler(retryRecord, entry);
        ...
        if (timeout > 0L) {
            ...
            final ScheduledFuture<?> timeoutTimer = getProcessingTimeService().registerTimer(
                    timeoutTimestamp,
                    timestamp -> userFunction.timeout(retryRecord.realRecord.getValue(), resultHandler));

            resultHandler.setTimeoutTimer(timeoutTimer);
        }

        userFunction.asyncInvoke(retryRecord.realRecord.getValue(), resultHandler);
    }
    ...
    public void onEventTime(InternalTimer timer) throws Exception {
        throw new UnsupportedOperationException("only support process time");
    }

    @Override
    public void onProcessingTime(InternalTimer timer) throws Exception {
        List<RetryRecord> elements = elementData.get(timer.getTimestamp());
        for (RetryRecord record : elements) {
            mailboxExecutor.execute(
                    () -> retryElement(record),
                    "retry process element");
        }
    }
    ...
    private class ResultHandler implements ResultFuture<OUT> {
         ...
         */
        private final RetryRecord inputRecord;
        /**
        ...
        ResultHandler(RetryRecord inputRecord, ResultFuture<OUT> resultFuture) {
        ...
        @Override
        public void completeOrRetry(Collection<OUT> results) {
            if (inputRecord.retryNums >= maxRetry) {
                complete(results);
                return;
            }
            inputRecord.retryNums++;
            KeyedAsyncWaitOperator.this.setCurrentKey(inputRecord.key);
            long retryTime = System.currentTimeMillis() + backoff;
            timerService.registerProcessingTimeTimer(retryTime);
            try {
                List<RetryRecord> elements = elementData.get(retryTime);
                if (elements == null) {
                    elements = new ArrayList<>();
                }
                elements.add(inputRecord);
                elementData.put(retryTime, elements);
    ...
    private class RetryRecord {
        private Object key;
        private StreamRecord<IN> realRecord;
        private int retryNums;

        public RetryRecord(Object key, StreamRecord<IN> realRecord, int retryNums) {
            this.key = key;
