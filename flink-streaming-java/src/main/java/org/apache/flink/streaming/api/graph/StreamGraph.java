...
import static org.apache.flink.api.common.NodeShardInfo.UNKNOWN_SHARD_NUM;
...
public class StreamGraph implements Pipeline {
	...
	private HashMap<Integer, NodeShardInfo> leafNodeShardState;

	private final StreamPartitionerFactory streamPartitionerFactory;
	...
	public StreamGraph(
		...
		this.leafNodeShardState = this.executionConfig.getLeafNodeShardState();
		this.streamPartitionerFactory = new StreamPartitionerFactory();
	...
	public <IN, OUT> void addOperator(
		...
		addOperator(vertexID, slotSharingGroup, coLocationGroup, operatorFactory, inTypeInfo,
				outTypeInfo, operatorName, invokableClass, operatorType);
	}
	...
	private <IN, OUT> void addOperator(
		...
		addNode(vertexID, slotSharingGroup, coLocationGroup, invokableClass, operatorFactory, operatorName, operatorType);
	...
	public <IN1, IN2, OUT> void addCoOperator(
		...
		addNode(vertexID, slotSharingGroup, coLocationGroup, vertexClass, taskOperatorFactory, operatorName, operatorType);
	...
	public <OUT> void addMultipleInputOperator(
		...
		addNode(vertexID, slotSharingGroup, coLocationGroup, vertexClass, operatorFactory, operatorName, operatorType);
	...
	private void generateSourceStateWithDownStream(
		StreamNode upstreamNode,
		StreamNode downstreamNode) {

		NodeShardInfo upstreamShardState = leafNodeShardState.get(upstreamNode.getId());
		if (upstreamNode.getOperatorName().startsWith("Source") &&
			downstreamNode.getOperatorName().startsWith("Calc")) {
			/*
			* if we have a watermark like
			* source->calc->watermark, we should make sure the source forward the records to calc,
			*  so we use needToKeepPrevShard
			*
			* else we should rebalance the log right after the source
			*/
			if (!executionConfig.isUseWatermark()) {
				leafNodeShardState.put(
				...
				leafNodeShardState.put(
					...
					new NodeShardInfo(
						true,
						upstreamShardState != null ? upstreamShardState.shardNumber : UNKNOWN_SHARD_NUM));
			}
		} else if (downstreamNode.getOperatorName().startsWith("WatermarkAssigner")) {
			/*
			* generally speaking, all the watermark need to keep the sharding info,
			* in order to make sure the watermark in order
			*/
			leafNodeShardState.put(
				downstreamNode.getId() ,
				new NodeShardInfo(
			...
			if (upstreamShardState != null) {
				leafNodeShardState.put(
					downstreamNode.getId(),
					new NodeShardInfo(false, upstreamShardState.shardNumber));
			}
		}
	}
	...
	private void addEdgeInternal(Integer upStreamVertexID,
		...
		if (virtualSideOutputNodes.containsKey(upStreamVertexID)) {
			...
			generateSourceStateWithDownStream(upstreamNode, downstreamNode);

			// Override the parallelism of both upstreamNode and downstreamNode if necessary,
			// since the partitioner depends on both. Otherwise, if the parallelism of downstreamNode
			// changes in the next loop, the partitioner may be inaccurate this time.
			...
			partitioner = streamPartitionerFactory.create(partitioner, upstreamNode, downstreamNode);
				...
				if (upstreamNode.getParallelism() != downstreamNode.getParallelism()) {
					throw new UnsupportedOperationException(
						"Forward partitioning does not allow change of parallelism." +
							" Upstream operation: " + upstreamNode +
	...
	private void overrideParallelismIfNecessary(StreamNode streamNode) {
		int streamVertexID = streamNode.getId();
		NodeShardInfo nodeShardInfo = leafNodeShardState.get(streamVertexID);
		if (nodeShardInfo == null) {
			return;
		}

		boolean isSourceRelatedNode = nodeShardInfo.needToKeepPrevShard || sources.contains(streamVertexID);
		boolean shouldOverride = isSourceRelatedNode && nodeShardInfo.shardNumber != UNKNOWN_SHARD_NUM &&
								 streamNode.getParallelism() > nodeShardInfo.shardNumber;
		if (shouldOverride) {
			...
			streamNode.setParallelism(nodeShardInfo.shardNumber);
			LOG.info("Override source related operator {} parallelism to {}",
				streamNode.getOperatorName(), nodeShardInfo.shardNumber);
		}
	}
	...
	class StreamPartitionerFactory {
		...
		public StreamPartitioner<?> create(
			...
			boolean loadBasedChannelSelectorEnabled = executionConfig.isLoadBasedChannelSelectorEnabled();

			if (needForceRebalanceForSource(upstreamNode, downstreamNode)) {
				partitioner = new RebalancePartitioner<>(loadBasedChannelSelectorEnabled);
			} else if (needRescaleForSource(upstreamNode, downstreamNode)) {
				partitioner = new RescalePartitioner<>(loadBasedChannelSelectorEnabled);
			} else if (needForwardForSource(upstreamNode, downstreamNode)) {
				partitioner = new ForwardPartitioner<>();
			} else if (upstreamNode.getParallelism() == downstreamNode.getParallelism()) {
				// If no partitioner was specified and the parallelism of upstream and downstream
				// operator matches use forward partitioner, use rebalance otherwise.
				partitioner = needRebalanceForSource(upstreamVertexID, downstreamVertexID) ?
					new RebalancePartitioner<>(loadBasedChannelSelectorEnabled) :
		...
		private boolean needForwardForSource(StreamNode upstreamNode, StreamNode downstreamNode) {
			if (upstreamNode.getParallelism() != downstreamNode.getParallelism()) {
				return false;
			}
			boolean forceForward = executionConfig.isForceForward();
			if (forceForward) {
				LOG.info("Up stream operator {} has parallelism {}, " +
		 ...
		 */
		private boolean needForceRebalanceForSource(
			StreamNode upstreamNode,
			StreamNode downstreamNode) {
			...
			NodeShardInfo upStreamShardState = leafNodeShardState.get(upstreamNode.getId());
			NodeShardInfo downStreamShardState = leafNodeShardState.get(downstreamNode.getId());

			// The upStreamShardState may be null when the up stream node is not kafka source related node.
			if (upStreamShardState == null) {
				if (downStreamShardState != null && downStreamShardState.needToKeepPrevShard) {
					LOG.info("ForceRebalance doesn't work on this source related operator because of " +
							"downStreamShardState.needToKeepPrevShard = {}, up stream operator: {}, down stream " +
							"operator: {}", downStreamShardState.needToKeepPrevShard,
						truncateOperatorNameIfNecessary(upstreamNode.getOperatorName()),
						truncateOperatorNameIfNecessary(downstreamNode.getOperatorName()));
					return false;
				} else {
			...
			boolean isSourceRelatedNode =
				upStreamShardState.needToKeepPrevShard || sources.contains(upstreamNode.getId());

			if (!isSourceRelatedNode) {
				LOG.info("ForceRebalance doesn't work on not source related operator, up stream operator: {}, down " +
						"stream operator: {}", truncateOperatorNameIfNecessary(upstreamNode.getOperatorName()),
					truncateOperatorNameIfNecessary(downstreamNode.getOperatorName()));
				return false;
			}
			return downStreamShardState == null || !downStreamShardState.needToKeepPrevShard;
		}

		private boolean needRescaleForSource(StreamNode upstreamNode, StreamNode downstreamNode) {
			int upstreamVertexID = upstreamNode.getId();
			NodeShardInfo upNodeShardInfo = leafNodeShardState.get(upstreamVertexID);
			if (upNodeShardInfo == null) {
				return false;
			}

			boolean isSourceRelatedNode = upNodeShardInfo.needToKeepPrevShard || sources.contains(upstreamVertexID);
			...
			boolean needRescale = downstreamNode.getParallelism() % upstreamNode.getParallelism() == 0 ||
		...
		private boolean needRebalanceForSource(Integer upStreamVertexID, Integer downStreamVertexID) {
			NodeShardInfo upStreamShardState = leafNodeShardState.get(upStreamVertexID);
			NodeShardInfo downStreamShardState = leafNodeShardState.get(downStreamVertexID);
			if (upStreamShardState == null || downStreamShardState == null) {
				return false;
			}
			//only need to rebalance when adjacent vertexes differs in source state
			// and upStream's shard if KNOWN differs from downStream's parallelism
			if (upStreamShardState.needToKeepPrevShard && !downStreamShardState.needToKeepPrevShard) {
				StreamNode downstreamNode = getStreamNode(downStreamVertexID);
				boolean setRebalanceForShard =
					(upStreamShardState.shardNumber % downstreamNode.getParallelism()) != 0
						&& upStreamShardState.shardNumber != UNKNOWN_SHARD_NUM;
				if (setRebalanceForShard) {
					StreamNode upstreamNode = getStreamNode(upStreamVertexID);
					LOG.info("source related operator {} has shard number {}" +
							", down stream operator {} has parallel {}, need rebalance",
						truncateOperatorNameIfNecessary(upstreamNode.getOperatorName()),
						upStreamShardState.shardNumber,
						truncateOperatorNameIfNecessary(downstreamNode.getOperatorName()),
						downstreamNode.getParallelism());
				}
				return setRebalanceForShard;
			} else {
				return false;
			}
		}
