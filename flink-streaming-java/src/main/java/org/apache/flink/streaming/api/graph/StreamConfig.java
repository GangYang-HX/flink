...
public class StreamConfig implements Serializable {
    ...
    public TimeCharacteristic getTimeCharacteristic(int ordinal) {
        if (ordinal >= 0) {
            return TimeCharacteristic.values()[ordinal];
        } else {
            throw new CorruptConfigurationException("time characteristic is not set");
    ...
    public void setOperatorDescription(String description) {
        this.config.setString(OPERATOR_DESCRIPTION, description);
    }

    public String getOperatorDescriptionOrName() {
        return this.config.getString(OPERATOR_DESCRIPTION, getOperatorName());
    }

    // Get the original operator description from description which was been converted to
    // [id]:description format by {@see ExecNodeBase#createFormattedTransformationDescription()}
    // in SQL mode. Additionally, In stream api mode, It will be mistake when setting the
    // description
    // to contain string ']:'.
    public String getOriginalOperatorDescription() {
        String operatorDescription = getOperatorDescriptionOrName();
        int index = operatorDescription.indexOf("]:");
        if (index != -1) {
            return operatorDescription.substring(index + 1);
        }
