...
public abstract class OutputStreamBasedPartFileWriter<IN, BucketID> extends AbstractPartFileWriter<IN, BucketID> {
	...
	public InProgressFileRecoverable persist() throws IOException {
		RecoverableWriter.ResumeRecoverable resumeRecoverable = currentPartStream.persist();
		if (resumeRecoverable instanceof HadoopFsRecoverable) {
			((HadoopFsRecoverable) resumeRecoverable).setSize(getElementSize());
		}
		return new OutputStreamBasedInProgressFileRecoverable(resumeRecoverable);
	}

	@Override
	public PendingFileRecoverable closeForCommit() throws IOException {
		RecoverableWriter.CommitRecoverable commitRecoverable = currentPartStream.closeForCommit().getRecoverable();
		if (commitRecoverable instanceof HadoopFsRecoverable) {
			((HadoopFsRecoverable) commitRecoverable).setSize(getElementSize());
		}
		return new OutputStreamBasedPendingFileRecoverable(commitRecoverable);
	}
	...
	abstract static class OutputStreamBasedBucketWriter<IN, BucketID> implements BucketWriter<IN, BucketID> {
		...
		protected final RecoverableWriter recoverableWriter;
