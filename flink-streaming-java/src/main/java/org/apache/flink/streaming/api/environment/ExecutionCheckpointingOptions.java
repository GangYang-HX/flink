...
public class ExecutionCheckpointingOptions {
	...
	public static final ConfigOption<Boolean> SKIP_MAX_PARALLELISM_CHECK =
			key("execution.checkpointing.skip-max-parallelism-check")
					.booleanType()
					.defaultValue(false)
					.withDescription("Skip checking the max parallelism when restoring from an old checkpoint. " +
	...
	public static final ConfigOption<Boolean> AUTO_ADJUST_MAX_PARALLELISM =
		key("execution.checkpointing.auto-adjust-max-parallelism")
			.booleanType()
			.defaultValue(false)
			.withDescription("adjust the max parallelism when restoring from an old checkpoint. " +
				"This option will not work when there are keyed operator subtask state in the checkpoint.");

	public static final ConfigOption<Boolean> ENABLE_REGIONAL_CHECKPOINT =
			key("execution.checkpointing.regional.enabled")
					.booleanType()
					.defaultValue(false)
					.withDescription("Enables regional checkpoints, which tolerates failures or expirations of some" +
							"checkpoint regions. The max tolerable failures or expirations of a single checkpoint " +
							"region, and the max failure or expiration ratio of checkpoint regions can be configured " +
							"by the users.");

	public static final ConfigOption<Integer> MAX_TOLERABLE_CONSECUTIVE_FAILURES_OR_EXPIRATIONS =
			key("execution.checkpointing.regional.max-tolerable-consecutive-failures-or-expirations")
					.intType()
					.defaultValue(3)
					.withDescription("The max tolerable consecutive failures or expirations of a single region");

	public static final ConfigOption<Double> MAX_TOLERABLE_FAILURE_OR_EXPIRATION_RATIO =
			key("execution.checkpointing.regional.max-tolerable-failure-or-expiration-ratio")
					.doubleType()
					.defaultValue(0.50)
					.withDescription("The max tolerable ratio of failures or expirations");

	public static final ConfigOption<Boolean> EXCLUDE_BROADCASTED_VERTEX =
			key("execution.checkpointing.regional.exclude-broadcasted-vertex")
					.booleanType()
					.defaultValue(false)
					.withDescription("Do not include broadcasted execution vertex when building checkpoint regions." +
							" Enabling this when the broadcasted execution vertex does not have state can make " +
							"the region pointwise without the risk of getting wrong computing result.");
	...
	public static final ConfigOption<Boolean> ENABLE_CHECKPOINT_INFO_LISTENER =
			key("execution.checkpointing.listener.enabled")
			    .booleanType()
				.defaultValue(false)
				.withDescription("Whether to enable checkpoint listener");
}
