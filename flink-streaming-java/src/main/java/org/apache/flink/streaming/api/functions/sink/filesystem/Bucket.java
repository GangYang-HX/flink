...
public class Bucket<IN, BucketID> {
	...
	private Bucket(
			...
			final BucketLifeCycleListener<IN, BucketID> bucketLifeCycleListener) {
	...
	private Bucket(
		...
		this(
				...
				rollingPolicy,
				outputFileConfig,
				new BucketLifeCycleListener<IN, BucketID>() {
					@Override
	...
	private Bucket(
			...
			final OutputFileConfig outputFileConfig,
			final BucketLifeCycleListener<IN, BucketID> bucketLifeCycleListener) throws IOException {
	...
	private void restoreInProgressFile(final BucketState<BucketID> state) throws IOException {
		LOG.info("bucketWriter name is {},IsSupportsResume {}，stateHasInProgressFileRecoverable {}",bucketWriter.getClass().getName(),bucketWriter.getProperties().supportsResume(),state.hasInProgressFileRecoverable());
	...
	private void commitRecoveredPendingFiles(final BucketState<BucketID> state) throws IOException {
		...
		for (List<InProgressFileWriter.PendingFileRecoverable> pendingFileRecoverables: state.getPendingFileRecoverablesPerCheckpoint().values()) {
			for (InProgressFileWriter.PendingFileRecoverable pendingFileRecoverable: pendingFileRecoverables) {
				...
				notifyTargetFileReady(pendingFileRecoverable);
			}
		}
	}
	...
	private Path assembleNewPartPath() {
		Path uriBucketPath = new Path(bucketPath.toUri().getPath());
		return new Path(uriBucketPath, outputFileConfig.getPartPrefix() + '-' + subtaskIndex + '-' + partCounter + outputFileConfig.getPartSuffix());
	}
	...
	private void notifyTargetFileReady(InProgressFileWriter.PendingFileRecoverable committable) {

		OutputStreamBasedPartFileWriter.OutputStreamBasedPendingFileRecoverable recoverable =
				(OutputStreamBasedPartFileWriter.OutputStreamBasedPendingFileRecoverable) committable;

		if (recoverable.getCommitRecoverable() instanceof HadoopFsRecoverable) {
			HadoopFsRecoverable hadoopFsRecoverable = (HadoopFsRecoverable) recoverable.getCommitRecoverable();
			if (bucketLifeCycleListener != null) {
				Path targetPath = new Path(hadoopFsRecoverable.targetFilePathString());
				String targetParent = targetPath.getParent().toString();
				String bucketPath = getBucketPath().toString();
				Path finalPath = targetParent.equals(bucketPath) ? targetPath : new Path(bucketPath + "/" + targetPath.getName());
				PartFileStatus<BucketID> partFileStatus = new PartFileStatus<>(finalPath,
																getBucketPath(),
																hadoopFsRecoverable.size(),
																bucketId,
																hadoopFsRecoverable.offset());

				bucketLifeCycleListener.bucketPartFileInactive(partFileStatus);
			}
		}
	}
	...
	static <IN, BucketID> Bucket<IN, BucketID> getNew(
			...
			final OutputFileConfig outputFileConfig,
			final BucketLifeCycleListener<IN, BucketID> bucketLifeCycleListener) {
		return new Bucket<>(subtaskIndex, bucketId, bucketPath, initialPartCounter, bucketWriter, rollingPolicy, outputFileConfig, bucketLifeCycleListener);
	}
	...
	static <IN, BucketID> Bucket<IN, BucketID> restore(
			...
			final OutputFileConfig outputFileConfig,
			final BucketLifeCycleListener<IN, BucketID> bucketLifeCycleListener) throws IOException {
		return new Bucket<>(subtaskIndex, initialPartCounter, bucketWriter, rollingPolicy, bucketState, outputFileConfig, bucketLifeCycleListener);
	}
	...
	static <IN, BucketID> Bucket<IN, BucketID> restore(
		...
		return new Bucket<>(subtaskIndex, bucketState.getBucketId(), bucketState.getBucketPath(), initialPartCounter, bucketWriter, rollingPolicy, outputFileConfig);
	}
}
