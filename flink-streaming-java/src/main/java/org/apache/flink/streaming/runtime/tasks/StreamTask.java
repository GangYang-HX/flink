...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
	...
	/** Timeout of {@link #disposeAllOperators(boolean)} method */
	private static final int DISPOSE_TIMEOUT_MINUTES = 3;
	...
	protected StreamTask(
		...
		this.systemTimerService = createTimerService("System Timer Trigger for " + getName());
	...
	private TimerService createTimerService(String threadName) {
		ThreadFactory timerThreadFactory =
				new DispatcherThreadFactory(TRIGGER_THREAD_GROUP, threadName);
		...
		actionExecutor.runThrowing(() -> {
			try {
				...
				try {
					...
					throw new RestoreCheckpointException(RestoreFailureReason.INFLIGHT_DATA_RESTORED_EXCEPTION, e);
				}
				...
				if (e instanceof OperatorOpenException) {
					ExceptionUtils.rethrowException(e.getCause());
				}
				TaskStateManager taskStateManager = getEnvironment().getTaskStateManager();
				if (taskStateManager == null || e instanceof RestoreCheckpointException) {
					throw e;
				}
				if (taskStateManager.isRestored()) {
					throw new RestoreCheckpointException(RestoreFailureReason.UNKNOWN_EXCEPTION, e);
				} else {
					throw e;
				}
			}
	...
	public final void invoke() throws Exception {
		try {
			...
			LOG.error("operator invoke exception", invokeException);
			failing = !canceled;
			try {
				if (!canceled) {
					try {
						LOG.info("Trying to cancel task as task failed instead of being canceled.");
						cancelTask();
					} catch (Exception ex) {
						invokeException = firstOrSuppressed(ex, invokeException);
					}
				}
				LOG.info("Start clean task {}", getName());
				cleanUpInvoke();
				LOG.info("Clean task over {}", getName());
	...
	protected void throughputCalculatorSetup() {
	...
	protected void afterInvoke() throws Exception {
		...
		actionExecutor.runThrowing(() -> {
			...
			// make sure no new system timers can come
		...
		// make sure all system timers finish
	...
	protected void cleanUpInvoke() throws Exception {
		...
		LOG.info("Start cleaning {}", getName());
		// stop all timers and threads
		tryShutdownTimerService();
		LOG.info("Step-1: shutdown timer over {}", getName());
		...
		LOG.info("Step-2: shutdown async threads over {}", getName());
		...
		LOG.info("Step-3: execute cleanup over {}", getName());
		// if the operators were not disposed before, do a hard dispose

		// Set a timer here to prevent rocksdb dispose in blocking.
		// See https://git.bilibili.co/datacenter/flink/-/issues/287
		Future<?> disposeFuture = Executors.newSingleThreadExecutor().submit(() -> {
			try {
				disposeAllOperators(true);
			} catch (Exception ignored) {}
		});
		try {
			disposeFuture.get(DISPOSE_TIMEOUT_MINUTES, TimeUnit.MINUTES);
		} catch (Exception ignored) {
			LOG.warn("Step-4: dispose all operators skipped! {}", getName());
		}
		LOG.info("Step-4: dispose all operators over {}", getName());
		...
		LOG.info("Step-5: release output resources over {}", getName());
		try {
			channelIOExecutor.shutdown();
		} catch (Throwable t) {
			LOG.error("Error during shutdown the channel state unspill executor", t);
		}
		LOG.info("Step-6: shutdown channelIOExecutor over {}", getName());
	...
	public final String getName() {
		return getEnvironment().getTaskInfo().getTaskNameWithSubtasks(true);
	}
	...
	public void triggerCheckpointOnBarrier(
		...
		long delay = System.currentTimeMillis() - checkpointMetaData.getTimestamp();
		getEnvironment().acknowledgeTriggerSnapshot(checkpointMetaData.getCheckpointId(), delay);
        ...
        if (!timerService.isTerminated()) {
			if (!timerService.shutdownServiceUninterruptible(timeoutMs)) {
				LOG.warn(
						...
						"timers. Will continue with shutdown predicate.",
						timeoutMs);
			}
		}
	}
	...
	private static <OUT> void registerBacklogMetricsForSubpartitions(
		...
		if (outputPartitioner instanceof RebalancePartitioner || outputPartitioner instanceof RescalePartitioner) {
			int numberOfSubpartitions = bufferWriter.getNumberOfSubpartitions();
			ResultSubpartition[] subpartitions = new ResultSubpartition[numberOfSubpartitions];
			for (int i = 0; i < numberOfSubpartitions; i++) {
				subpartitions[i] = bufferWriter.getSubpartition(i);
			}
