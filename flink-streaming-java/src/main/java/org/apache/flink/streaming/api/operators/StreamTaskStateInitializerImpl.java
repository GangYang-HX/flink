...
public class StreamTaskStateInitializerImpl implements StreamTaskStateInitializer {
	...
	public StreamOperatorStateContext streamOperatorStateContext(
		...
		try {
			...
			if (ex instanceof RestoreCheckpointException) {
				throw ex;
			} else if (taskStateManager.isRestored()) {
				LOG.error("Exception occurred when restore state: {}", ExceptionUtils.stringifyException(ex));
				throw new RestoreCheckpointException(RestoreFailureReason.UNKNOWN_EXCEPTION, ex);
			}

			LOG.error("Exception while creating StreamOperatorStateContext: {}", ExceptionUtils.stringifyException(ex));
	...
	protected OperatorStateBackend operatorStateBackend(
		...
		try {
			...
			if (e instanceof RestoreCheckpointException) {
				throw e;
			} else if (prioritizedOperatorSubtaskStates.isRestored()) {
				throw new RestoreCheckpointException(
					RestoreFailureReason.OPERATOR_STATE_RESTORED_EXCEPTION,
					e);
			} else {
				throw e;
			}
	...
	protected <K> AbstractKeyedStateBackend<K> keyedStatedBackend(
		...
		try {
			...
			if (e instanceof RestoreCheckpointException) {
				...
				throw new RestoreCheckpointException(
					RestoreFailureReason.KEYED_STATE_RESTORED_EXCEPTION,
					e);
			} else {
				throw e;
			}
