...
public class StreamingJobGraphGenerator {
	...
	private StreamingJobGraphGenerator(StreamGraph streamGraph, @Nullable JobID jobID) {
		...
		if (streamGraph.getExecutionConfig().useStreamGraphHasherV3()) {
			this.defaultStreamGraphHasher = new StreamGraphHasherV3();
		} else {
	...
	private JobGraph createJobGraph() {
		...
		try {
			vertexConfigs.values().forEach(streamConfig -> LOG.info(("operator id & name & type line: {} : {} : {}")
				, streamConfig.getOperatorID()
				, streamConfig.getOperatorName().substring(0, Math.min(300, streamConfig.getOperatorName().length()))
				, streamConfig.getOperatorType()));
		} catch (Exception e) {
			LOG.error("exception happen when log operator id", e);
		}
	...
	private List<StreamEdge> createChain(Integer currentNodeId, int chainIndex, OperatorChainInfo chainInfo) {
		...
		if (!builtVertices.contains(startNodeId)) {
			...
			if (currentNodeId.equals(startNodeId)) {
				...
				config.setOperatorType(node.getOperatorType());
				config.setOutEdgesInOrder(transitiveOutEdges);
				config.setOutEdges(node.getOutEdges());
	...
	private void connect(Integer headOfChain, StreamEdge edge) {
		...
		if (partitioner instanceof BroadcastPartitioner) {
			downStreamVertex.addBroadcastedInput(headVertex);
		}
	...
	private void configureCheckpointing() {
		...
		Map<OperatorID, Tuple2<String, String>> operatorNameAndTypes = vertexConfigs.values().stream()
			.collect(Collectors.toMap(StreamConfig::getOperatorID,
				config -> new Tuple2<>(config.getOperatorName(), config.getOperatorType())));
		...
		JobCheckpointingSettings settings = new JobCheckpointingSettings(
				...
				CheckpointCoordinatorConfiguration.builder()
						...
						.shouldSkipMaxParallelismCheck(cfg.shouldSkipMaxParallelismCheck())
						.enableAutoAdjustMaxParallelism(cfg.autoAdjustMaxParallelismEnable())
						.setRegionalCheckpointEnabled(cfg.isRegionalCheckpointEnabled())
                        .setMaxTolerableConsecutiveFailuresOrExpirations(
                        		cfg.getMaxTolerableConsecutiveFailuresOrExpirations())
						.setMaxTolerableFailureOrExpirationRatio(
								cfg.getMaxTolerableFailureOrExpirationRatio())
						.setExcludeBroadcastedVertex(cfg.getExcludeBroadcastedVertex())
						.setUnalignedCheckpointsEnabled(cfg.isUnalignedCheckpointsEnabled())
						.setAlignmentTimeout(cfg.getAlignmentTimeout())
						.setCheckpointInfoListenerEnabled(cfg.isCheckpointInfoListenerEnabled())
						.build(),
				serializedStateBackend,
				serializedHooks);
