...
import org.apache.flink.streaming.runtime.operators.RateLimiterWatermarkOperator;
...
public class DataStream<T> {
	...
	public DataStream<T> rebalance(boolean enableLoadBasedChannelSelector) {
		return setConnectionType(new RebalancePartitioner<T>(enableLoadBasedChannelSelector));
	}
	...
	@PublicEvolving
	public DataStream<T> rescale(boolean enableLoadBasedChannelSelector) {
		return setConnectionType(new RescalePartitioner<T>(enableLoadBasedChannelSelector));
	}
	...
	public SingleOutputStreamOperator<T> assignTimestampsAndWatermarks(
		...
		final RateLimiterWatermarkOperator<T> rateLimiterOperator = new RateLimiterWatermarkOperator<>(cleanedStrategy);

		// match parallelism to input, to have a 1:1 source -> timestamps/watermarks relationship and chain
		final int inputParallelism = getTransformation().getParallelism();

		return transform("Timestamps/Watermarks", "", getTransformation().getOutputType(), getExecutionConfig().isRateLimiter() && getExecutionConfig().getTolerateInterval() > 0 ? rateLimiterOperator : operator)
	 ...
	 * @see #transform(String, String, TypeInformation, OneInputStreamOperatorFactory)
	 */
	@PublicEvolving
	public <R> SingleOutputStreamOperator<R> transform(
		...
		return doTransform(operatorName, operatorType, outTypeInfo, SimpleOperatorFactory.of(operator));
	}
	...
	public <R> SingleOutputStreamOperator<R> transform(
		...
		return doTransform(operatorName, operatorType, outTypeInfo, operatorFactory);
	}
