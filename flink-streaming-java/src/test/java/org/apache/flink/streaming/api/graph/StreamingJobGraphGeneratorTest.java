...
public class StreamingJobGraphGeneratorTest extends TestLogger {
	...
	public void testBroadcast() {
		...
		DataStream<Integer> sourceDataStream1 = env.fromElements(1, 2, 3);
		DataStream<Integer> broadcastStream = sourceDataStream1.broadcast();

		DataStream<Integer> unionStream = mapDataStream.union(broadcastStream);
		unionStream.map(x -> x).print();
		...
		// it can not be chained with BATCH shuffle mode
		JobVertex broadcastVertex = verticesSorted.get(1);
		JobVertex printVertex = verticesSorted.get(2);

		assertNotNull(printVertex.getBroadcastedUpstreams());
		assertTrue(printVertex.getBroadcastedUpstreams().contains(broadcastVertex));
	}
