...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    private ApplicationReport startAppMaster(
        ...
        if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
            ...
            // Set number of application retries to 1 when not set HA, and don't use
            // YarnConfigOptions.APPLICATION_ATTEMPTS!!!
            // For example, if use YarnConfigOptions.APPLICATION_ATTEMPTS whose value is set to 2,
            // YARN will restart JM if JM failed.
            // But JM will not restart from the latest checkpoint(because we don't set HA, Flink
            // don't know the latest checkpoint).
            // Instead, JM will restart from the checkpoint that submitted to YARN in the first
