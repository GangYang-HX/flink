...
import static org.apache.flink.runtime.statistics.AbstractOnceSender.DIMENSION_YARN_APPLICATION_ID;
...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	public YarnClusterDescriptor(
		...
		if (userJarInclusion != YarnConfigOptions.UserJarInclusion.DISABLED) {
			flinkConfiguration.set(DeploymentOptionsInternal.SYSTEM_CLASSPATH_INCLUDES_USER_JAR, true);
		}
	...
	private ClusterClientProvider<ApplicationId> deployInternal(
		...
		flinkConfiguration.setDouble(
				JobManagerOptions.JOBMANAGER_VCORES,
		...
		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.CLIENT_START_JOB_REQUEST,
				true,
	...
	private ApplicationReport startAppMaster(
		...
		long submitJobManagerTimestamp = System.currentTimeMillis();
		StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.SUBMIT_JOB_MANAGER);
		StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.UPLOAD_FILES_TO_FILESYSTEM);
		...
		if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
			...
			// Set number of application retries to 1 when not set HA, and don't use YarnConfigOptions.APPLICATION_ATTEMPTS!!!
			// For example, if use YarnConfigOptions.APPLICATION_ATTEMPTS whose value is set to 2, YARN will restart JM if JM failed.
			// But JM will not restart from the latest checkpoint(because we don't set HA, Flink don't know the latest checkpoint).
			// Instead, JM will restart from the checkpoint that submitted to YARN in the first time, which is unexpected.
		...
		if (!CollectionUtil.isNullOrEmpty(jarUrls) && isApplicationOrSessionMode(yarnClusterEntrypoint)) {
			userJarFiles.addAll(jarUrls.stream().map(Path::new).collect(Collectors.toSet()));
			configuration.set(PipelineOptionsInternal.REMOTE_JARS, configuration.get(PipelineOptions.JARS));
		}

		final List<URI> classPaths = ConfigUtils.decodeListFromConfig(configuration, PipelineOptions.CLASSPATHS, URI::create);
		if (classPaths != null && YarnApplicationClusterEntryPoint.class.getName().equals(yarnClusterEntrypoint)) {
			userJarFiles.addAll(classPaths.stream().map(Path::new).collect(Collectors.toSet()));
		}
		...
		final List<String> userClassPaths = fileUploader.registerMultipleLocalResources(
			userJarFiles,
			ClusterEntrypointUtils.getRelativeUserLibDirectory(configuration));
        ...
        LOG.info("App master use systemClassPaths: {}", systemClassPaths);
		LOG.info("App master use userClassPaths: {}", userClassPaths);
		...
		if (jobGraph != null) {
			StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.UPLOAD_FILES_TO_FILESYSTEM,
				true,
				jobGraph.getJobID().toString(),
				Collections.singletonMap(DIMENSION_YARN_APPLICATION_ID, appId.toString()));
		} else {
			StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.UPLOAD_FILES_TO_FILESYSTEM,
				true,
		...
		appMasterEnv.put(StartupUtils.ENV_SUBMIT_JOB_MANAGER, String.valueOf(submitJobManagerTimestamp));
		...
		StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.PREPARE_RESOURCE_FOR_JOB_MANAGER);
		...
		StatisticsUtils.reportEndTracking(
			StartupUtils.StartupPhase.CLIENT_START_JOB,
			true,
			Collections.singletonMap(DIMENSION_YARN_APPLICATION_ID, appId.toString())
		);
		...
		loop: while (true) {
			...
			switch(appState) {
				...
				case RUNNING:
					...
					if (jobGraph != null) {
						StatisticsUtils.reportEndTracking(
							StartupUtils.StartupPhase.PREPARE_RESOURCE_FOR_JOB_MANAGER,
							true,
							jobGraph.getJobID().toString(),
							Collections.singletonMap(DIMENSION_YARN_APPLICATION_ID, appId.toString()));
					} else {
		...
		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.SUBMIT_JOB_MANAGER,
				true,
				Collections.emptyMap());
	...
	private boolean isApplicationOrSessionMode(String yarnClusterEntrypoint) {
		return YarnApplicationClusterEntryPoint.class.getName().equals(yarnClusterEntrypoint) ||
			YarnSessionClusterEntrypoint.class.getName().equals(yarnClusterEntrypoint);
	}
}
