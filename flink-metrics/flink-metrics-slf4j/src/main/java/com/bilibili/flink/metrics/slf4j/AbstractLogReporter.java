...
public abstract class AbstractLogReporter implements MetricReporter, CharacterFilter {
    ...
    // eg:-yD
    // metrics.reporter.prom.labels="saber_job_name=platform_custom_job_name;saber_job_owner=yanggang01@bilibili.com"
    ...
    public void open(MetricConfig config) {
        metricFilter =
                new Filter(config.getString(METRIC_FILTER.key(), METRIC_FILTER.defaultValue()));
    ...
    static String replaceInvalidChars(final String input) {
        return maxLabelValueLength > input.length()
                ? input
                : StringUtils.substring(input, 0, maxLabelValueLength / 2)
                        + StringUtils.substring(
                                input, input.length() - maxLabelValueLength / 2, input.length());
    }

    @Override
    public void notifyOfAddedMetric(Metric metric, String metricName, MetricGroup group) {
        final LogMeasurementInfo logMetricInfo =
                logMetricInfoProvider.getMetricInfo(
                        metricName, group, labelFilter, labelValueCharactersFilter);
        // todo filter
    ...
    protected Map<String, String> parseLabels(final String labelConfig) {
        ...
        if (!labelConfig.isEmpty()) {
            ...
            for (String kv : kvs) {
                ...
                if (org.apache.flink.util.StringUtils.isNullOrWhitespaceOnly(labelKey)
                        || org.apache.flink.util.StringUtils.isNullOrWhitespaceOnly(labelValue)) {
                    log.warn(
    ...
    public static class Filter {
        ...
        public String toString() {
            return "Filter{" + "filter=" + filter.get(0) + '}';
        }
    }
}
