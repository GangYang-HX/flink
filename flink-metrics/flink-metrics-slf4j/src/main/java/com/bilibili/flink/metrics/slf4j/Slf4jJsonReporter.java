...
/**
 * @author Dove
 * @Date 2021/8/25 12:24 下午
 */
@InstantiateViaFactory(factoryClassName = "com.bilibili.flink.metrics.slf4j.Slf4jJsonReporterFactory")
public class Slf4jJsonReporter extends AbstractLogReporter implements Scheduled {
	...
	private void tryReport() {
		try {
			...
			for (Map.Entry<Gauge<?>, LogMeasurementInfo> entry : gauges.entrySet()) {
				...
				String labelIndex = getLabelIndex(entry.getValue(), labelIndexMap, labelKVMap, atomicLabelIndex, atomicLabelKV);
			...
			for (Map.Entry<Histogram, LogMeasurementInfo> entry : histograms.entrySet()) {
				...
				String labelIndex = getLabelIndex(entry.getValue(), labelIndexMap, labelKVMap, atomicLabelIndex, atomicLabelKV);
				histogram.add(createHistogramJson(mapper, name + "_count", labelIndex, stats.size()));
				histogram.add(createHistogramJson(mapper, name + "_min", labelIndex, stats.getMin()));
				histogram.add(createHistogramJson(mapper, name + "_max", labelIndex, stats.getMax()));
				histogram.add(createHistogramJson(mapper, name + "_mean", labelIndex, stats.getMean()));
			}
	...
	private String getLabelIndex(LogMeasurementInfo logMeasurementInfo,
	...
	private String getLabelIndexValue(LogMeasurementInfo logMeasurementInfo, Map<String, String> labelKVMap, AtomicLong atomicLabelKV) {
	...
	private ObjectNode createHistogramJson(ObjectMapper mapper, String name, String labels, Object value) {
		ObjectNode node = mapper.createObjectNode();
		node.put("name", name);
