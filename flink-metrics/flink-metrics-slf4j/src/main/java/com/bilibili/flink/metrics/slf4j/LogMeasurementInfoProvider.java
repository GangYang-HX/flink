...
class LogMeasurementInfoProvider implements LogMetricInfoProvider {
    ...
    @Override
    public LogMeasurementInfo getMetricInfo(
            String metricName,
            MetricGroup group,
            AbstractLogReporter.Filter labelFilter,
            CharacterFilter labelValueCharactersFilter) {
        return new LogMeasurementInfo(
                getScopedName(metricName, group),
                getLabels(group, labelFilter, labelValueCharactersFilter));
    }
    ...
    private static Map<String, String> getLabels(
        ...
        for (Map.Entry<String, String> variable : group.getAllVariables().entrySet()) {
            ...
            if (labelFilter.containKey(name)) {
                String labelValue =
                        labelValueCharactersFilter.filterCharacters(variable.getValue());
    ...
    private static String getScopedName(String metricName, MetricGroup group) {
        return CHARACTER_FILTER.filterCharacters(
                getLogicalScope(group) + SCOPE_SEPARATOR + metricName);
    }
