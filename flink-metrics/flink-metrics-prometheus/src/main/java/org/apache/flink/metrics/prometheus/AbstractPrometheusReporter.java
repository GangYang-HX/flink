...
import static org.apache.flink.metrics.prometheus.PrometheusPushGatewayReporterOptions.MAX_LABEL_VALUE_LENGTH;
...
public abstract class AbstractPrometheusReporter implements MetricReporter {
	...
	public void open(MetricConfig config) {
		...
		log.info("prometheus reporter start, set the label value length to {}", maxLabelValueLength);
	}
	...
	static class HistogramSummaryProxy extends Collector {
		static final List<Double> QUANTILES = Arrays.asList(.5, .75, .95, .98, .99, .999, 1.0);
