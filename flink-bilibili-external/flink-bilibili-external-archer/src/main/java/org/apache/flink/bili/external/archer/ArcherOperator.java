...
import org.apache.flink.bili.external.archer.integrate.InstanceQuery;
...
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.JOB_CRON_LOG_DAY;
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.SINK_DATABASE_NAME_KEY;
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.SINK_PARTITION_KEY;
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.SINK_TABLE_NAME_KEY;
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.SYSTEM_USER_ID;
import static org.apache.flink.bili.external.archer.constant.ArcherConstants.USER;

/** The operator of Archer. */
public class ArcherOperator implements Serializable {
    ...
    private static final String LOG_HOUR = "log_hour";
    ...
    private void registerTableListToArcherIfNotExist(
            List<Pair<String, String>> databaseTablesPair) {
                ...
                ArcherMessage<ArcherCreateDummyJobResp> createDummyJobResp =
                        ArcherHandler.createDummyJob(
                                ...
                                properties.get(SINK_PARTITION_KEY).toString().contains(LOG_HOUR)
                                        ? ArcherConstants.JOB_CRON_LOG_HOUR
                                        : JOB_CRON_LOG_DAY,
                                createUser,
                                opUser);
    ...
    public boolean updatePartitionStatus(
            String database, String table, List<String> partitionKeys, List<String> partitionValues)
            throws Exception {
    ...
    private Tuple2<String, String> getPartitionTime(
            List<String> partitionKeys, List<String> partitionValues) throws Exception {
        LocalDateTime bizStartDayTime = null, bizEndDayTime = null;
        if (CollectionUtils.isNotEmpty(partitionValues)
                && CollectionUtils.isNotEmpty(partitionKeys)) {
    ...
    private boolean updatePartitionStatusToArcher(Long jobId, Tuple2<String, String> bizTimeTuple) {
        ...
        ArcherMessage<ArcherQueryInstanceResp> archerQueryInstanceResp =
                ArcherHandler.queryInstance(
                        String.valueOf(jobId), bizTimeTuple.f0, bizTimeTuple.f1, opUser);
        if (!ArcherMessage.isSuccessful(archerQueryInstanceResp)
            ...
            return false;
        } else if (ArcherMessage.isSuccessful(archerQueryInstanceResp)
                && CollectionUtils.isEmpty(archerQueryInstanceResp.getData().getInstances())) {
            LOG.warn(
                    "instance is empty while update partitions status, will return true instead. job id :{} ,bizTimeTuple:{}",
                    jobId,
                    bizTimeTuple);
            return true;
        }
        ...
        ArcherMessage<ArcherUpdateInstanceSuccessResp> updatePartitionInstanceSuccess =
                ArcherHandler.updatePartitionInstanceSuccess(instance.getInstanceId(), opUser);
    ...
    public boolean updateInstanceStatusByRelateUID(
            String relateUID, List<String> partitionKeys, List<String> partitionValues)
            throws Exception {
        ...
        Tuple2<String, String> bizTimeTuple = getPartitionTime(partitionKeys, partitionValues);
        ArcherMessage<ArcherQueryInstanceResp> archerQueryInstanceResp =
                ArcherHandler.queryInstanceByRelateUID(
        ...
        if (!ArcherMessage.isSuccessful(archerQueryInstanceResp)
            ...
            return false;
                ...
                && CollectionUtils.isEmpty(archerQueryInstanceResp.getData().getInstances())) {
            LOG.warn(
                    "instance is empty while update partitions status, will return true instead. relateUID id :{} ,bizTimeTuple:{}",
                    relateUID,
                    bizTimeTuple);
            return true;
        }
        ...
        if (archerQueryInstanceRespData.size() > 1) {
            LOG.warn(
                    "relateUID: {}, bizTimeTuple: {} get instances: {} more than one!",
                    relateUID,
                    bizTimeTuple,
                    archerQueryInstanceRespData);
        }
