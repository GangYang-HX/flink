    ...
    private Runnable reportMetadata() {
        return () -> {
            try {
                ...
                String sessionId;
                if ("kubernetes-session"
                        .equals(configuration
                                .getString(DeploymentOptions.TARGET))) {
                    sessionId = configuration.getString(ExcaliburConfigOptions.APPLICATION_NAME);
                } else {
                    sessionId = configuration.getString(HighAvailabilityOptions.HA_CLUSTER_ID);
                }
                ...
                sessionClusterMetaBuilder.setExecutionTarget(
                ...
                taskManagerOverviewFuture.thenAcceptBoth(
                        ...
                        (resourceOverview, multipleJobsDetails) -> {
                            ...
                            try {
                                for (String jobIdItem : jobIdList) {
                                    sessionClusterMetaBuilder.addJobId(jobIdItem);
                                }
                                for (String completedJobIdIdItem : completedJobIdList) {
                                    sessionClusterMetaBuilder.addCompletedJobId(
                                            completedJobIdIdItem);
                                }
                            } catch (Throwable e) {
                                log.error("add job list failed", e);
                            }
                            try {
