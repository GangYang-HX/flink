...
public class KeeperMessage<T> {
	...
	public static <T> KeeperMessage<T> requestFailedResponse() {
		return KeeperMessageBuilder.<T>anKeeperMessage().withCode(KeeperCode.REQUEST_FAILED.getCode()).build();
	...
	public static <T> String errorMsg(KeeperMessage<T> keeperMessage) {
		log.error("call keeper api use open api encounter exception,VoyagerRuntimeException,code:{},message:{}", keeperMessage.code, keeperMessage.message);
