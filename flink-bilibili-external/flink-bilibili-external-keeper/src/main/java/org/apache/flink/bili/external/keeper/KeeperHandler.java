...
import static org.apache.flink.bili.external.keeper.constant.KeeperConstants.*;
...
public class KeeperHandler {
	 ...
	 * @throws Exception
	 */
	public static KeeperMessage<KeeperGetTableInfoResp> getTableInfo(String datasource, String databaseName, String tableName) throws Exception {
		...
		String data = getTableInfoData(
				dbTypeCode,
				datasource,
				databaseName == null ? "" : databaseName,
				tableName);
		return commonKeeperRequestWithRetry(RETRY_TIMES,
				GROUP_NAME_KEEPER,
				API_GET_TABLE_INFO,
				data,
	...
	private static <T> KeeperMessage<T> commonKeeperRequestWithRetry(
		...
		KeeperApiReq req = KeeperApiReq.builder()
				.groupName(groupName)
				.apiName(apiName)
	...
	private static String post(
		...
		String signature =
				generateSignature(appId, groupName, apiName, account, requestId, secretKey, data);
	 ...
	 * @throws Exception
	 */
	private static <T, R> R funcRetry(int retryTimes, Function<T, R> func, T param) throws Exception {
		int i = 0;
