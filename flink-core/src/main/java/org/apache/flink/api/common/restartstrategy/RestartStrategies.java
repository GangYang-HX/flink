...
public class RestartStrategies {
	 ...
	 * @param restartAttempts Number of restart attempts for the FixedDelayRestartStrategy
	 * @param delayInterval Delay in-between restart attempts for the FixedDelayRestartStrategy
	 * @param resetRestartAttemptPeriod Period of reset attempt value to zero for the FixedDelayRestartStrategy
	 * @return FixedDelayRestartStrategy
	 */
	public static RestartStrategyConfiguration fixedDelayRestart(
		int restartAttempts, long delayInterval, long resetRestartAttemptPeriod) {
		return new FixedDelayRestartStrategyConfiguration(restartAttempts, Time.of(delayInterval, TimeUnit.MILLISECONDS),
			Time.of(resetRestartAttemptPeriod, TimeUnit.MILLISECONDS));
	}
	...
	public static final class FixedDelayRestartStrategyConfiguration extends RestartStrategyConfiguration {
		...
		private final Time resetRestartAttemptsPeriod;

		FixedDelayRestartStrategyConfiguration(int restartAttempts, Time delayBetweenAttemptsInterval) {
			this(restartAttempts, delayBetweenAttemptsInterval, Time.of(-1, TimeUnit.MILLISECONDS));
		}

		FixedDelayRestartStrategyConfiguration(
			int restartAttempts, Time delayBetweenAttemptsInterval, Time resetRestartAttemptsPeriod) {
			this.restartAttempts = restartAttempts;
			this.delayBetweenAttemptsInterval = delayBetweenAttemptsInterval;
			this.resetRestartAttemptsPeriod = resetRestartAttemptsPeriod;
		}
		...
		public Time getResetRestartAttemptsPeriod() {
			return resetRestartAttemptsPeriod;
		}
	...
	private static RestartStrategyConfiguration parseConfiguration(
		...
		switch (restartstrategyKind.toLowerCase()) {
			...
			case "fixed-delay":
				...
				Duration period = configuration.get(RestartStrategyOptions.RESTART_STRATEGY_FIXED_DELAY_RESET_ATTEMPTS_PERIOD);
				return fixedDelayRestart(attempts, delay.toMillis(), period.toMillis());
			case "failurerate":
			case "failure-rate":
