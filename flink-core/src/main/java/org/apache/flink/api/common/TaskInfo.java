...
public class TaskInfo {
	 ...
	 * {@link #getNumberOfParallelSubtasks()}.
	 *
	 * @param truncate Whether task name need to be truncated if it exceeds the maximum length of {@link #MAX_NAME_LENGTH}
	 * @return The name of the task, with subtask indicator.
	 */
	...
	private String getTruncatedTaskNameWithSubtasks() {
		if (this.taskNameWithSubtasks.length() >= MAX_NAME_LENGTH) {
			...
			sb.append(this.taskNameWithSubtasks, 0, MAX_NAME_LENGTH)
			  ...
			  .append("(").append(indexOfSubtask + 1).append('/')
			  .append(numberOfParallelSubtasks).append(')');
			return sb.toString();
		}
