...
public final class ExceptionUtils {
	...
	/**
	 * Get the root cause message of throwable.
	 * Return "(null)" if t is null.
	 *
	 * @param t the throwable
	 */
	public static String getRootCauseMessage(Throwable t) {
		return getTruncateRootCauseMessage(t, -1);
	}

	/**
	 * Get the truncated length root cause message of throwable.
	 * Return "(null)" if t is null.
	 *
	 * @param t the throwable
	 * @param truncatedLength the length of returned message
	 */
	public static String getTruncateRootCauseMessage(Throwable t, int truncatedLength) {
		if (t == null) {
			return STRINGIFIED_NULL_EXCEPTION;
		}
		String message = getRootCause(t).getMessage();
		if (message == null) {
			return "";
		}
		...
		return message.substring(0, Math.min(truncatedLength, message.length()));
	}
