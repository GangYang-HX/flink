...
public class MetricOptions {
	...
	public static final ConfigOption<Boolean> CGROUP_METRICS_COLLECT_ENABLED =
		key("metrics.cgroup-file.enabled")
			.booleanType()
			.defaultValue(true)
			.withDescription("Flag indicating whether collect cgroup metrics from the tm file.");

	public static final ConfigOption<String> CGROUP_METRICS_PATH =
		key("metrics.cgroup-file.path")
			.stringType()
			.noDefaultValue()
			.withDescription("cgroup metrics storage path. Path must contain '{container_id}', It will be automatically " +
				"replaced with the current container_id at runtime. eg:/a/b/{container_id}/cpu.txt");

	public static final ConfigOption<Long> CGROUP_METRICS_PROBING_INTERVEL_SEC =
		key("metrics.cgroup-file.probing-intervel")
			.longType()
			.defaultValue(15L)
			.withDescription("The intervel of collecting cgroup metrics.");
