...
public final class DelegatingConfiguration extends Configuration {
	...
	private static final String REGEX_PATTERN = "metrics.reporter.\\w+.labels";
	...
	@Override
	public void addGeneralLabelProperties(Properties props) {
		synchronized (backingConfig.confData) {
			backingConfig.confData.entrySet().stream()
				.filter(entry -> entry.getKey().matches(REGEX_PATTERN))
				.findAny()
				.ifPresent(entry -> {
					String newKey = prefix + "labels";
					if (!backingConfig.confData.containsKey(newKey)) {
						props.put(newKey, entry.getValue());
					}
				});
		}
	}
	...
	@Override
	public void addExecutionTargetProperty(Properties props) {
		synchronized (backingConfig.confData) {
			String executionTarget =
				String.valueOf(backingConfig.confData.get(DeploymentOptions.CLUSTER_TYPE.key()));
			props.setProperty(DeploymentOptions.CLUSTER_TYPE.key(), executionTarget);
		}
	}

	@Override
	public void addKubernetesClusterIdProperty(Properties props) {
		synchronized (backingConfig.confData) {
			String kubernetesClusterId =
				String.valueOf(backingConfig.confData.get(DeploymentOptions.KUBERNETES_CLUSTER_ID.key()));
			props.setProperty(DeploymentOptions.KUBERNETES_CLUSTER_ID.key(), kubernetesClusterId);
		}
	}

	@Override
	public void addYarnQueueNameProperty(Properties props) {
		synchronized (backingConfig.confData) {
			String kubernetesClusterId =
				String.valueOf(backingConfig.confData.get(DeploymentOptions.YARN_APPLICATION_QUEUE.key()));
			props.setProperty(DeploymentOptions.YARN_APPLICATION_QUEUE.key(), kubernetesClusterId);
		}
	}

	@Override
	public void addCustomCallerContextJobIdProperty(Properties props) {
		synchronized (backingConfig.confData) {
			String custerCallerContextJobId =
				String.valueOf(backingConfig.confData.get(ExecutionOptions.CUSTOM_CALLER_CONTEXT_JOB_ID.key()));
			props.setProperty(ExecutionOptions.CUSTOM_CALLER_CONTEXT_JOB_ID.key(), custerCallerContextJobId);
		}
	}
