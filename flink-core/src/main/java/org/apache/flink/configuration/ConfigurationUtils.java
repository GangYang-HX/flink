...
import static org.apache.flink.configuration.MetricOptions.CGROUP_METRICS_COLLECT_ENABLED;
import static org.apache.flink.configuration.MetricOptions.CGROUP_METRICS_PROBING_INTERVEL_SEC;
import static org.apache.flink.configuration.MetricOptions.CGROUP_METRICS_PATH;
...
public class ConfigurationUtils {
	...
	public static Optional<String> getCGroupMetricsPath(Configuration configuration) {
		if (!configuration.getBoolean(CGROUP_METRICS_COLLECT_ENABLED)) {
			return Optional.empty();
		} else {
			String path = configuration.getString(CGROUP_METRICS_PATH);
			return path == null ? Optional.empty() : Optional.of(path);
		}
	}

	public static Long getCGroupMetricsProbingIntervel(Configuration configuration) {
		return configuration.getLong(CGROUP_METRICS_PROBING_INTERVEL_SEC);
	}
