...
public class TaskManagerOptions {
	...
	public static final ConfigOption<Duration> BUFFER_DEBLOAT_PERIOD =
			key("taskmanager.network.memory.automatic-buffer-adjustment.period")
					.durationType()
					.defaultValue(Duration.ofMillis(500))
					.withDescription(
							"The minimum period of time after which the buffer size will be automatically " +
									"adjusted to a new value if required. The low value provides a fast " +
									"reaction to the load fluctuation but can influence the performance.");
	...
	public static final ConfigOption<Integer> BUFFER_DEBLOAT_THRESHOLD_PERCENTAGES =
			ConfigOptions.key("taskmanager.network.memory.buffer-debloat.threshold-percentages")
					...
					.defaultValue(50)
					.withDescription(
							"The minimum difference in percentage between the newly calculated buffer " +
									"size and the old one to announce the new value. Can be used to avoid " +
	...
	public static final ConfigOption<String> TASK_MANAGER_PRINT_STREAM_TYPE = ConfigOptions
		.key("taskmanager.print.stream.type")
		.stringType()
		.noDefaultValue()
		.withDescription("The type of use print stream in taskmanager, eg: log4j.");

	public static final ConfigOption<Double> TASKMANAGER_VCORES =
		key("taskmanager.vcores")
			.defaultValue(1.0)
			.withDescription("Display the number of taskmanager vcores");
