...
public class NettyShuffleEnvironmentOptions {
	...
	public static final ConfigOption<Integer> MAX_NUM_TCP_CONNECTIONS =
		key("taskmanager.network.max-num-tcp-connections")
			...
			.defaultValue(1)
			.withDescription(
				"The maximum number of tcp connections between taskmanagers for data communication.");
	...
	@Documentation.Section(Documentation.Sections.ALL_TASK_MANAGER_NETWORK)
	public static final ConfigOption<Boolean> RUNTIME_NETWORK_RETRY_ENABLED =
		key("taskmanager.network-retry-at-runtime.enabled")
			.defaultValue(false)
			.withDeprecatedKeys("taskmanager.network-retry-at-runtime.enabled")
			.withDescription("It's used for reconnecting when the connection between the client and the server " +
				"is broken during runtime. Note that this parameter may cause very small amount data loss.");
