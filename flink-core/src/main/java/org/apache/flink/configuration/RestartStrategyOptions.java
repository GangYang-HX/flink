...
public class RestartStrategyOptions {
	...
	public static final ConfigOption<Duration> RESTART_STRATEGY_FIXED_DELAY_RESET_ATTEMPTS_PERIOD = ConfigOptions
		.key("restart-strategy.fixed-delay.reset-attempts-period")
		.durationType()
		.defaultValue(Duration.ofHours(1))
		.withDescription(
			Description.builder()
				.text(
					"The period after which the number of retries is reset to 0 if %s has been set to %s.",
					code(RESTART_STRATEGY.key()),
					code("fixed-delay"))
				.build());
