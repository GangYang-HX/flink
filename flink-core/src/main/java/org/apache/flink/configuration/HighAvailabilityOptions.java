...
public class HighAvailabilityOptions {
	...
	/**
	 * If this option is enabled then reconciling jobs instead of restarting jobs after JobManager failover from HA.
	 */
	@Documentation.Section(Documentation.Sections.EXPERT_HIGH_AVAILABILITY)
	public static final ConfigOption<Boolean> HA_RECONCILE_ENABLED =
		key("high-availability.reconcile.enabled")
			.defaultValue(false)
			.withDescription("Whether enable reconciling when JobManager failover from HA, if this option is enabled" +
				"then tasks in TaskExecutor won't fail themselves when HA leader changed.");

	/**
	 * The timeout in milliseconds for reconciling a job after JobManager restart from HA.
	 */
	@Documentation.Section(Documentation.Sections.EXPERT_HIGH_AVAILABILITY)
	public static final ConfigOption<Long> HA_RECONCILE_TIMEOUT =
		key("high-availability.reconcile.timeout")
			.longType()
			.defaultValue(60 * 1000L)
			.withDescription("The timeout in milliseconds for reconciling jobs after JobManager restart from HA.");

	@Documentation.Section(Documentation.Sections.EXPERT_HIGH_AVAILABILITY)
	public static final ConfigOption<Long> HA_YARN_LIVENESS_MONITOR_EXPIRY_INTERVAL_MS =
		key("yarn.liveness-monitor.expiry-interval-ms")
			.longType()
			.defaultValue(180 * 1000L)
			.withDescription("The timeout in TaskExecutor for TaskManager when heartbeat timeout " +
				"between TaskExecutor with ResourceManager or JobManager. It's the minimum value of the " +
				"config `yarn.am.liveness-monitor.expiry-interval-ms` and `yarn.nm.liveness-monitor.expiry-interval-ms` in " +
				"yarn-site.xml.");
	...
	//  File system options
	// ------------------------------------------------------------------------
	/** Polling interval for file system to support high availability. */
	@Documentation.Section(Documentation.Sections.COMMON_HIGH_AVAILABILITY_FILESYSTEM)
	public static final ConfigOption<Integer> HA_FILESYSTEM_RETRIEVAL_INTERVAL =
		key("high-availability.filesystem.retrieval.interval")
			.defaultValue(30000)
			.withDescription("Leader retrieval interval for file system in high availability mode," +
				"this value should be less than key heartbeat.timeout.");

	@Documentation.Section(Documentation.Sections.COMMON_HIGH_AVAILABILITY_FILESYSTEM)
	public static final ConfigOption<Integer> HA_FILESYSTEM_MAX_RETRY_ATTEMPTS =
		key("high-availability.filesystem.max-retry-attempts")
			.defaultValue(3)
			.withDescription("Max retry times for file system in high availability mode.");

	@Documentation.Section(Documentation.Sections.COMMON_HIGH_AVAILABILITY_FILESYSTEM)
	public static final ConfigOption<Integer> HA_FILESYSTEM_RETRY_WAIT =
		key("high-availability.filesystem.retry.wait")
			.defaultValue(3000)
			.withDescription("Defines the pause between consecutive retries in ms.");
