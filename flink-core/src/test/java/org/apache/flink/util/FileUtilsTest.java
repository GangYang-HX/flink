...
public class FileUtilsTest extends TestLogger {
	...
	@Test
    public void testListFilesNotRecursively() throws IOException {
        final java.nio.file.Path testDir = tmp.newFolder("_test_1").toPath();

        // create files of current dir
        final Collection<java.nio.file.Path> filesOfCurrentDir = new ArrayList<>();
        filesOfCurrentDir.add(Files.createFile(testDir.resolve("file1.txt")));
        filesOfCurrentDir.add(Files.createFile(testDir.resolve("file2.txt")));
        filesOfCurrentDir.add(Files.createFile(testDir.resolve("file3.txt")));
        filesOfCurrentDir.add(Files.createFile(testDir.resolve("file4.txt")));

        // create sub dir and files under that
        final java.nio.file.Path subDir1 = Files.createDirectory(testDir.resolve("_sub_dir1"));
        Files.createFile(subDir1.resolve("file5.txt"));
        Files.createFile(subDir1.resolve("file6.txt"));

        final Collection<java.nio.file.Path> filesInDirectory =
                FileUtils.listFilesInDirectory(testDir, p -> true, false);

        assertThat(filesInDirectory, containsInAnyOrder(filesOfCurrentDir.toArray()));
    }
