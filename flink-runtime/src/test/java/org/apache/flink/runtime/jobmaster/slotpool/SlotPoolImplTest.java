...
public class SlotPoolImplTest extends TestLogger {
	...
	public void testCheckIdleSlot() throws Exception {
		...
		try (TestingSlotPoolImpl slotPool = createSlotPoolImpl(clock)) {
			...
			assertThat(slotPool.registerTaskManager(taskManagerLocation.getResourceID(), taskManagerLocation),
				Matchers.is(true));
	...
	public void testDiscardIdleSlotIfReleasingFailed() throws Exception {
		...
		try (TestingSlotPoolImpl slotPool = createSlotPoolImpl(clock)) {
			...
			assertThat(slotPool.registerTaskManager(taskManagerLocation.getResourceID(), taskManagerLocation), Matchers.is(true));
