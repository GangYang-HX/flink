...
public class PartitionRequestQueueTest {
	...
	public void testNotifyReaderNonEmptyOnEmptyReaders() throws Exception {
		...
		reader1.requestSubpartitionView((partitionId, index, availabilityListener, isRetry) ->
				new EmptyAlwaysAvailableResultSubpartitionView(), new ResultPartitionID(), 0, false);
		...
		reader2.requestSubpartitionView((partitionId, index, availabilityListener, isRetry) ->
	...
	public void testProducerFailedException() throws Exception {
		...
		ResultPartitionProvider partitionProvider =
				(partitionId, index, availabilityListener, isRetry) -> view;
		...
		seqView.requestSubpartitionView(partitionProvider, new ResultPartitionID(), 0, false);
	...
	private void testBufferWriting(ResultSubpartitionView view) throws IOException {
		...
		reader.requestSubpartitionView(partitionProvider, new ResultPartitionID(), 0, false);
...
/*
	...
	public void testNotifyNewBufferSize() throws Exception {
		...
		Buffer buffer = TestBufferFactory.createBuffer(128);
		byte[] data = new byte[65];
		for (int i = 0; i < 65; i++) {
			data[i] = 0;
		}
		buffer.getMemorySegment().put(0, data);
		channel.writeOutbound(
				new NettyMessage.BufferResponse(
				        buffer,
						0,
						receiverId,
						0));
...
*/
	...
	public void testEnqueueReaderByResumingConsumption() throws Exception {
		...
		BufferAvailabilityListener bufferAvailabilityListener = new NoOpBufferAvailablityListener();
		PipelinedSubpartitionView view = subpartition.createReadView(false, bufferAvailabilityListener );
		ResultPartitionProvider partitionProvider = (partitionId, index, availabilityListener, isRetry) -> view;
	...
	private void testCancelPartitionRequest(boolean isAvailableView) throws Exception {
		...
		reader.requestSubpartitionView(partitionManager, partition.getPartitionId(), 0, false);
