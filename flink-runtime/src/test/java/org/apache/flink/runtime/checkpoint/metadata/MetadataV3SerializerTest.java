...
public class MetadataV3SerializerTest {
	...
	public void testCheckpointWithNoState() throws Exception {
		...
		for (int i = 0; i < 100; ++i) {
			...
			final Map<OperatorID, Tuple2<String, String>> operatorNameAndTypes = Collections.emptyMap();

			testCheckpointSerialization(checkpointId, taskStates, masterStates, operatorNameAndTypes, null);
		}
	}
	...
	public void testCheckpointWithOnlyMasterState() throws Exception {
		...
		for (int i = 0; i < 100; ++i) {
			...
			testCheckpointSerialization(checkpointId, operatorStates, masterStates, operatorNameAndTypes, null);
		}
	}

	@Test
	public void testCheckpointWithOnlyOperatorName() throws Exception {
		final Random rnd = new Random();
		final int maxOperatorNum = 20;
		...
		for (int i = 0; i < 100; ++i) {
			...
			final Map<OperatorID, Tuple2<String, String>> operatorNameAndTypes =
				CheckpointTestUtils.createOperatorNameAndTypes(rnd, maxOperatorNum);
	...
	private void testCheckpointWithOnlyTaskState(String basePath) throws Exception {
		...
		for (int i = 0; i < 100; ++i) {
			...
			testCheckpointSerialization(checkpointId, taskStates, masterStates, operatorNameAndTypes, basePath);
		}
	}
	...
	private void testCheckpointWithMasterAndTaskState(String basePath) throws Exception {
		...
		for (int i = 0; i < 100; ++i) {
			...
			final int operatorNum = rnd.nextInt(maxOperatorNum) + 1;
			final Map<OperatorID, Tuple2<String, String>> operatorNameAndTypes =
				CheckpointTestUtils.createOperatorNameAndTypes(rnd, operatorNum);
	 ...
	 * @param operatorNameAndTypes the given names of all the operators.
	 */
	private void testCheckpointSerialization(
		...
		CheckpointMetadata metadata = new CheckpointMetadata(checkpointId, operatorStates, masterStates, operatorNameAndTypes);
		...
		Map<OperatorID, Tuple2<String, String>> deserializedOperatorNameAndTypes = deserialized.getOperatorNameAndTypes();
		assertEquals(operatorNameAndTypes.size(), deserializedOperatorNameAndTypes.size());
		for (Map.Entry<OperatorID, Tuple2<String, String>> entry : operatorNameAndTypes.entrySet()) {
			...
			assertTrue(deserializedOperatorNameAndTypes.containsKey(operatorID));
			assertEquals(operatorName, deserializedOperatorNameAndTypes.get(operatorID).f0);
			assertEquals(typeName, deserializedOperatorNameAndTypes.get(operatorID).f1);
		}
	}
}
