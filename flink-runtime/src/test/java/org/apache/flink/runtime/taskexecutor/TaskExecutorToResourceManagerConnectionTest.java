...
public class TaskExecutorToResourceManagerConnectionTest extends TestLogger {
	...
	@Test
	public void testRpcGatewayInvokeAfterConnectionClosed() throws Exception {
		final TaskExecutorToResourceManagerConnection resourceManagerRegistration = createTaskExecutorToResourceManagerConnection();
		...
		resourceManagerRegistration.start();
		// wait for connection registration success
		registrationSuccessFuture.get(TEST_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS);

		// Rpc Gateway invoke any one sync and async method after connecting success.
		ResourceManagerGateway resourceManagerGateway = resourceManagerRegistration.getTargetGateway();
		// choose one of async method to invoke from ResourceManagerGateway
        resourceManagerGateway.requestResourceOverview(Time.milliseconds(TEST_TIMEOUT_MILLIS));
		// choose one of sync method to invoke from ResourceManagerGateway
        resourceManagerGateway.disconnectJobManager(new JobID(), null);

		// Rpc Gateway invoke any one sync and async method after connecting close, and not throw an Exception
