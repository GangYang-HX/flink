	...
	public void testFailingCompletedCheckpointStoreAdd() throws Exception {
		...
		CheckpointCoordinator coord =
			new CheckpointCoordinatorBuilder()
				...
				.build();
		GlobalCheckpointHandler globalCheckpointHandler = new GlobalCheckpointHandler(coord, jid);
		coord.setCheckpointHandler(globalCheckpointHandler);
		...
		verify(operatorSubtaskState.getManagedOperatorState().iterator().next()).discardStateWithRetry();
		verify(operatorSubtaskState.getRawOperatorState().iterator().next()).discardStateWithRetry();
		verify(operatorSubtaskState.getManagedKeyedState().iterator().next()).discardStateWithRetry();
		verify(operatorSubtaskState.getRawKeyedState().iterator().next()).discardStateWithRetry();
		verify(operatorSubtaskState.getInputChannelState().iterator().next().getDelegate()).discardStateWithRetry();
		verify(operatorSubtaskState.getResultSubpartitionState().iterator().next().getDelegate()).discardStateWithRetry();
	}
