...
import org.apache.flink.runtime.highavailability.hybrid.HybridHaServicesFactory;
...
/**
 * Tests the hybrid leader election and retrieval.
 */
public class HybridLeaderRetrievalTest extends TestLogger{
	...
	public void before() throws Exception {
		...
		config.setString(HighAvailabilityOptions.HA_MODE, HybridHaServicesFactory.class.getName());
		...
		config.setString(HighAvailabilityOptions.HA_STORAGE_PATH, tmpDir.getAbsolutePath());

		Executor executor = Executors.newFixedThreadPool(2, new ExecutorThreadFactory("flink-io"));

		highAvailabilityServices = HighAvailabilityServicesUtils.createAvailableOrEmbeddedServices(config, executor);
	}
	 ...
	 * in case of an old leader address and a subsequent election of a new leader.
	 * The findConnectingAddress should block until the new leader has been elected and his
	 * address has been written to file system.
	 */
	@Test
