...
public class PendingCheckpointStats extends AbstractCheckpointStats {
	...
	private volatile int currentNumLogicallyMappedSubtasks;
	...
	public int getNumberOfLogicallyMappedSubtasks() {
		return currentNumLogicallyMappedSubtasks;
	}
	...
	/**
	 * Reports statistics for a single subtask whose state is logically mapped from a previously completed checkpoint.
	 *
	 * @param jobVertexId ID of the task/operator the subtask belongs to.
	 * @param subtask The statistics for the subtask.
	 * @return <code>true</code> if successfully reported or <code>false</code> otherwise.
	 */
	boolean reportLogicallyMappedSubtaskStats(JobVertexID jobVertexId, SubtaskStateStats subtask) {
		TaskStateStats taskStateStats = taskStats.get(jobVertexId);

		if (taskStateStats != null && taskStateStats.reportLogicallyMappedSubtaskStats(subtask)) {
			currentNumLogicallyMappedSubtasks += 1;
	...
	CompletedCheckpointStats.DiscardCallback reportCompletedCheckpoint(String externalPointer) {
		CompletedCheckpointStats completed = new CompletedCheckpointStats(
				...
				currentNumAcknowledgedSubtasks,
				currentNumLogicallyMappedSubtasks,
				currentStateSize,
