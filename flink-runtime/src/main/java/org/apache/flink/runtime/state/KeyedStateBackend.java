...
public interface KeyedStateBackend<K>
	...
	/**
	 * @return A stream of all key-stateName-values for the given state and namespace.
	 *
	 * @param state State variable for which existing keys will be returned.
	 * @param namespace Namespace for which existing keys will be returned.
	 */
	default <N> Stream<Tuple4<K, byte[], String, byte[]>> getKeyStateNameValues(String state, N namespace) {
	...
	default boolean isSupportedOnReadAndWrite() {
		return true;
	}
