...
/**
 * The counterpart to the {@link org.apache.flink.runtime.leaderelection.HybridLeaderElectionService}.
 * <p>
 * Normally, the leader address as well as the current leader session ID is retrieved from ZooKeeper.
 * If ZooKeeper catches some error, it will be retrieved from the external FileSystem.
 */
public class HybridLeaderRetrievalService
	...
	public HybridLeaderRetrievalService(
		...
		this.pollingInterval = configuration.getInteger(HighAvailabilityOptions.HA_FILESYSTEM_RETRIEVAL_INTERVAL);
	}

	@Override
	public void start(LeaderRetrievalListener listener) throws Exception {
		Preconditions.checkNotNull(listener, "Listener must not be null.");
		Preconditions.checkState(leaderListener == null, "HybridLeaderRetrievalService can only be started once.");
	...
	public void stop() throws Exception {
		...
		try {
			...
			throw new FlinkException("Could not properly stop the HybridLeaderRetrievalService.", e);
		}
	...
	private void notifyIfNewLeader(LeaderInformation leaderInformation) {
		if (!leaderInformation.equals(lastLeaderInformation)) {
			...
			leaderListener.notifyLeaderAddress(leaderInformation.getLeaderAddress(),
				...
				executor.execute(() -> {
					while (running && !zkAvailable) {
						try {
							...
							LOG.warn("Could not retrieve leader information from file system", e);
						}

						try {
							Thread.sleep(pollingInterval);
						} catch (InterruptedException e) {
							LOG.warn("Retrieve leader information from file system interrupted");
						}
					}
				});
			}
