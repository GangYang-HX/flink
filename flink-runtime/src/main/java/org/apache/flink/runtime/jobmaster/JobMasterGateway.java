...
public interface JobMasterGateway extends
	 ...
	 * @param payload task executor to job master's payload
	 */
	void heartbeatFromTaskManager(
		final ResourceID resourceID,
		final TaskExecutorHeartbeatPayloadForJobMaster payload);

	/**
	 * Report the {@link SpeculativeTasksOptions} to job master (
	 * {@link org.apache.flink.runtime.speculativeframe.SpeculativeTasksCoordinator}). Besides the
	 * options, this also report the corresponding relationship between task type (identified by
	 * the {@link UUID}) and the task managers.
     *
	 * NOTE: the relationship between task type and task managers will also be supplemented by
	 * 		 the time costs report.
	 */
	void reportSpeculativeTaskOptions(SpeculativeTasksOptions options);

	/**
	 * Report time costs of task(s) to job master (
	 * {@link org.apache.flink.runtime.speculativeframe.SpeculativeTasksCoordinator}). Currently,
	 * the data is reported one by one right after it is generated. But we use collections. This
	 * give us a chance to optimize the implementation to reduce the number of rpc calls.
	 */
	CompletableFuture<Acknowledge> reportSpeculativeTaskTimeCost(SpeculativeTaskTimeCosts timeCosts);
	...
	/**
	 * Triggers taking a full checkpoint of the executed job.
	 *
	 * @param timeout for the rpc call
	 * @return Future which is completed with a Boolean indicating whether the checkpoint successfully completed
	 */
	CompletableFuture<Long> triggerFullCheckpoint(
			@RpcTimeout final Time timeout,
	 ...
	 * Do the following:
     * <ol>
	 *     <li>Add the servers to blacklist in resource manager</li>
	 * 	   <li>restart the job</li>
     * </ol>
	 *
	 * @param blacklistedHosts blacklist of servers
	 * @param timeout for the rpc call
	 * @param removeHostsAfterRestart whether to remove hosts after job restarted or failed
	 * @return true if the restarting succeeds, else false
	 */
	default CompletableFuture<Boolean> restart(
	 ...
	 * Do the following:
	 * <ol>
	 * 	   <li>remove the servers from the blacklist</li>
	 * </ol>
	 *
	 * @param blacklistedHosts blacklist of servers
	 */
	default void removeFromBlackList(List<String> blacklistedHosts) {
		throw new UnsupportedOperationException();
	}
}
