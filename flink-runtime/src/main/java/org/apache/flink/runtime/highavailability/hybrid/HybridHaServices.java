...
/**
 * An implementation of the {@link HighAvailabilityServices} using Apache ZooKeeper and an external
 * FileSystem.
 *
 * <p>This implement should run on Yarn which only exists one running JobManager for one cluster, so
 * we don't consider concurrent race problem.
 *
 * <p>The services store data in ZooKeeper's nodes or FileSystem's path, there exists two type data:
 *
 * <ul>
 *   1. leader information: save to FileSystem is a necessary and try best to save to ZooKeeper at
 *   the same time.
 * </ul>
 *
 * <ul>
 *   2. Checkpoint, JobGraph, RunningJobsRegistry: only save to FileSystem.
 * </ul>
 *
 ...
 *     HighAvailabilityOptions#HA_ZOOKEEPER_ROOT} or {@link
 *     HighAvailabilityOptions#HA_STORAGE_PATH}. This makes sure Flink stores its data under
 *     specific subtrees, for example to accommodate specific permission.
 ...
 *     <p>In case of a "per-job-cluster" on YARN or Mesos, the cluster-id is generated and
 *     configured automatically by the client or dispatcher that submits the Job to YARN or Mesos.
 */
public class HybridHaServices implements HighAvailabilityServices {
    ...
    public LeaderRetrievalService getResourceManagerLeaderRetriever() {
        return new HybridLeaderRetrievalService(
                client, fs, executor, configuration, RESOURCE_MANAGER_LEADER_PATH);
    }

    @Override
    public LeaderRetrievalService getDispatcherLeaderRetriever() {
        return new HybridLeaderRetrievalService(
                client, fs, executor, configuration, DISPATCHER_LEADER_PATH);
    }
    ...
    public LeaderRetrievalService getClusterRestEndpointLeaderRetriever() {
        return new HybridLeaderRetrievalService(
                client, fs, executor, configuration, REST_SERVER_LEADER_PATH);
    }
    ...
    public JobResultStore getJobResultStore() throws Exception {
        return FileSystemJobResultStore.fromConfiguration(configuration);
    }
    ...
    public void close() throws Exception {
        ...
        if (exception != null) {
            ExceptionUtils.rethrowException(
                    exception, "Could not properly close the HybridHaServices.");
        }
    }
    ...
    public void closeAndCleanupAllData() throws Exception {
        ...
        if (exception != null) {
            ExceptionUtils.rethrowException(
                    exception,
                    "Could not properly close and clean up all data of HybridHaServices.");
        }
    }
