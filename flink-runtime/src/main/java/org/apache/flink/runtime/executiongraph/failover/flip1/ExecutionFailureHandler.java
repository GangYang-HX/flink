...
public class ExecutionFailureHandler {
	...
	private FailureHandlingResult handleFailure(
		...
		CauseType causeType = parseErrorType(cause);
		...
		if (cause instanceof TaskManagerFinishedException) {
			TaskManagerFinishedException taskManagerFinishedException = (TaskManagerFinishedException) cause;
			if (!taskManagerFinishedException.isAttempted()) {
				taskManagerFinishedException.setAttempted(true);
		...
		if (restartBackoffTimeStrategy.canRestart()) {
			...
			return FailureHandlingResult.restartable(
				...
				globalFailure).setCauseType(causeType);
		} else {
			return FailureHandlingResult.unrecoverable(
	...
	private CauseType parseErrorType(Throwable error) {
		if (error instanceof NoResourceAvailableException) {
			return CauseType.NO_RESOURCE;
		}
		// TODO: we need to distinguish between different error types in the future.
		return CauseType.FLINK_INTERNAL;
	}
	...
	static boolean isUnrecoverableError(Throwable cause) {
		...
		return unrecoverableError.isPresent() || UnrecoverableThrowableChecker.check(cause);
	}
