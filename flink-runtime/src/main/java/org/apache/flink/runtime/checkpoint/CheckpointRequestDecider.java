...
class CheckpointRequestDecider {
	 ...
	 */
	Optional<CheckpointTriggerRequest> chooseRequestToExecute(CheckpointTriggerRequest newRequest, boolean isTriggering, long lastCompletionMs, boolean needTriggerSequential) {
		if (queuedRequests.size() >= maxQueuedRequests && !queuedRequests.last().isPeriodic) {
			...
			Optional<CheckpointTriggerRequest> request = chooseRequestToExecute(isTriggering, lastCompletionMs, needTriggerSequential);
	 ...
	 * @return request that should be executed
	 */
	Optional<CheckpointTriggerRequest> chooseQueuedRequestToExecute(boolean isTriggering, long lastCompletionMs, boolean needTriggerSequential) {
		Optional<CheckpointTriggerRequest> request = chooseRequestToExecute(isTriggering, lastCompletionMs, needTriggerSequential);
		request.ifPresent(CheckpointRequestDecider::logInQueueTime);
		return request;
	}
	 ...
	 * @return request that should be executed
	 */
	private Optional<CheckpointTriggerRequest> chooseRequestToExecute(boolean isTriggering, long lastCompletionMs, boolean needTriggerSequential) {
		if (isTriggering || queuedRequests.isEmpty()) {
			return Optional.empty();
		}

		// disable concurrent checkpoint when restore mode is no claim
		int maxConcurrentCheckpointAttempt = needTriggerSequential ? 1 : maxConcurrentCheckpointAttempts;
		if (pendingCheckpointsSizeSupplier.get() >= maxConcurrentCheckpointAttempt) {
