...
public class RemoteInputChannel extends InputChannel {
	...
	/** The status indicates whether the RemoteInputChannel has reestablished a new connection with the server. */
	...
	public void removePartitionRequestClient() {
		connectionManager.destroyPartitionRequestClient(connectionId);
	}

	/**
	 * Retriggers a remote subpartition request after the connection between the producer and consumer broken.
	 */
	@Override
	public void reRequestSubpartition(int subpartitionIndex) throws IOException, InterruptedException {
		try {
			// Reset the expectedSequenceNumber when reconnecting.
		...
		partitionRequestClient.reRequestSubpartition(partitionId, subpartitionIndex, this);
	}
	...
	public void markReconnection() {
		inputGate.reconnectForInputChannel(this);
	}
	...
	public void markUnavailable() {
		synchronized (receivedBuffers) {
			try {
				appendEvent(new ChannelUnavailableEvent(this.getChannelIndex()));
			} catch (IOException e) {
				throw new RuntimeException("Failed to append channel unavailable event to received buffers.");
			}
		}
	}
	...
	public void onBuffer(Buffer buffer, int sequenceNumber, int backlog) throws IOException {
		...
		try {
			if (isRetry && sequenceNumber == 0) {
				LOG.info("The channel {} has been reconnected successfully.", this.id);
			}
	...
	private void appendEvent(AbstractEvent event) throws IOException {
		final boolean wasEmpty;
		wasEmpty = receivedBuffers.isEmpty();

		Buffer eventBuffer = EventSerializer.toBuffer(event, false);
		SequenceBuffer sequenceBuffer = new SequenceBuffer(eventBuffer, -1);
		receivedBuffers.add(sequenceBuffer);

		// After appending a ChannelUnavailableEvent, it is necessary to notify the owning {@link SingleInputGate}
