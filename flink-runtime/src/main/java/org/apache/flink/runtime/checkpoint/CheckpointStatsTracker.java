...
import org.apache.flink.runtime.checkpoint.listener.CheckpointResult;
import org.apache.flink.runtime.checkpoint.listener.CheckpointResultListener;
import org.apache.flink.runtime.checkpoint.listener.Sl4jOnceSenderListener;
...
public class CheckpointStatsTracker {
	...
	private volatile LastCheckpointStatus lastCheckpointStatus = LastCheckpointStatus.UNKNOWN;

	private final CheckpointResultListener listener;

	private volatile long lastCompletedCheckpointId;

	private volatile long lastFailedCheckpointId;
	...
	public CheckpointStatsTracker(
		...
		// In the future, we may register listeners through the discovery mechanism.
		listener = new Sl4jOnceSenderListener();
	}
	...
	void reportRestoredCheckpoint(RestoredCheckpointStats restored) {
		...
		try {
			...
			//reset status while restoring state.
			lastCheckpointStatus = LastCheckpointStatus.UNKNOWN;
			dirty = true;

			lastCompletedCheckpointId = restored.getCheckpointId();
		} finally {
			statsReadWriteLock.unlock();
		}
	}
	...
	private void reportCompletedCheckpoint(CompletedCheckpointStats completed) {
		...
		try {
			...
			listener.notifyCheckpointResult(CheckpointResult.createFromCheckpointStats(completed));
			dirty = true;

			// Handle out of order case. eg: ck-1 completed, ck-2 failed, and ck-2 reported before ck-1,
			// we should not reset the number of consecutive failed checkpoints.
			if (completed.checkpointId > lastFailedCheckpointId) {
				counts.resetConsecutiveFailedCheckpoints();
			}
			lastCheckpointStatus = LastCheckpointStatus.COMPLETED;
			lastCompletedCheckpointId = completed.checkpointId;
		} finally {
			statsReadWriteLock.unlock();
		}
	}
	...
	private void reportFailedCheckpoint(FailedCheckpointStats failed) {
		...
		try {
			...
			listener.notifyCheckpointResult(CheckpointResult.createFromCheckpointStats(failed));
			dirty = true;

			// Handle out of order case. eg: ck-2 completed, and ck-2 reported before ck-1,
			// ck-1 will be aborted, we also should not increment it because of the bigger checkpoint
			// has been completed.
			if (failed.checkpointId > lastCompletedCheckpointId) {
				counts.incrementConsecutiveFailedCheckpoints();
			}
			lastCheckpointStatus = LastCheckpointStatus.FAILED;
			lastFailedCheckpointId = failed.checkpointId;
		} finally {
			statsReadWriteLock.unlock();
		}
	}
	...
	static final String LATEST_CHECKPOINT_STATUS = "lastCheckpointStatus";

	static final String NUMBER_OF_CONSECUTIVE_FAILED_CHECKPOINTS_METRIC = "numberOfConsecutiveFailedCheckpoints";
	...
	private void registerMetrics(MetricGroup metricGroup) {
		...
		metricGroup.gauge(LATEST_CHECKPOINT_STATUS, new LastCheckpointGauge());
		metricGroup.gauge(NUMBER_OF_CONSECUTIVE_FAILED_CHECKPOINTS_METRIC, new ConsecutiveFailedCheckpointsCounter());
	}
	...
	private class LastCheckpointGauge implements Gauge<Integer> {

		@Override
		public Integer getValue() {
			return lastCheckpointStatus.code;
		}

	}

	private class ConsecutiveFailedCheckpointsCounter implements Gauge<Integer> {

		@Override
		public Integer getValue() {
			return counts.getNumberOfConsecutiveFailedCheckpoints();
		}
	}

	enum LastCheckpointStatus {
		UNKNOWN(-1),
		FAILED(0),
		COMPLETED(1);
