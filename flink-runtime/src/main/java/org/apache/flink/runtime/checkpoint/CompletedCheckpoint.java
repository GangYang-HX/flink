...
public class CompletedCheckpoint implements Serializable {
	...
	private final Map<Long, AtomicInteger> referenceCount = new HashMap<>();
	...
	public void reference(long checkpointId) {
		referenceCount.computeIfAbsent(checkpointId, v -> new AtomicInteger(0)).incrementAndGet();
	}

	public void dereference(long checkpointId) throws Exception {
		if (referenceCount.get(checkpointId) == null) {
			throw new IllegalStateException("should not happen for " + checkpointId);
		} else {
			if (referenceCount.get(checkpointId).decrementAndGet() == 0) {
				referenceCount.remove(checkpointId);
			}
		}
	}

	/**
	 * Get ids of all the checkpoints that reference this checkpoint.
	 */
	public Set<Long> getReferencingCheckpointIds() {
		return referenceCount.keySet();
	}

	public boolean isReferencedBy(long checkpointId) {
		if (referenceCount.get(checkpointId) == null) {
			return false;
		}
		return referenceCount.get(checkpointId).get() > 0;
	}
	...
	public boolean isReferenced() {
		...
		return referenceCount.entrySet().stream().anyMatch(entry -> entry.getValue().get() > 0);
	}
	...
	public boolean discardOnSubsume() throws Exception {
		if (props.discardOnSubsumed() && !isReferenced()) {
			doDiscard();
	...
	public void doDiscard() {
		...
		try {
			...
			LOG.warn("Failed to discard checkpoint {}", this.checkpointID, e);
		} finally {
	...
	public String toString() {
		return String.format(
				"Checkpoint %d @ %d for %s reference detail: %s",
				checkpointID,
				timestamp,
				job,
				referenceCount);
