...
public abstract class RecordWriter<T extends IOReadableWritable> implements AvailabilityProvider {
	...
	protected TimerGauge backPressuredTimePerSecond = new TimerGauge();
	...
	protected boolean copyFromSerializerToTargetChannel(int targetChannel) throws IOException, InterruptedException {
		...
		BufferBuilder bufferBuilder = appendDataForNewRecord(targetChannel, serializer);
		if (bufferBuilder == null) {
			return false;
		}

		while (serializer.hasSerializedData()) {
			finishBufferBuilder(targetChannel, bufferBuilder);
			bufferBuilder = requestNewBufferBuilderForRecordContinuation(targetChannel, serializer);
			if (bufferBuilder == null) {
				return false;
			}
		}
	...
	private BufferBuilder appendDataForNewRecord(
			int targetChannel,
			RecordSerializer<T> serializer) throws IOException, InterruptedException {
		BufferBuilder buffer = getBufferBuilder(targetChannel);
		if (buffer == null) {
			return null;
		}
		serializer.copyToBufferBuilder(buffer);
		return buffer;
	}
	...
	public void setMetricGroup(TaskIOMetricGroup metrics) {
		...
		backPressuredTimePerSecond = metrics.getBackPressuredTimePerSecond();
	}

	abstract void finishBufferBuilder(int targetChannel, BufferBuilder bufferBuilder);
	...
	/**
	 * When a record spans multiple buffers, one buffer is not enough to hold the full record,
	 * so we need to request a new {@link BufferBuilder} to put the rest of the record.
	 */
	abstract BufferBuilder requestNewBufferBuilderForRecordContinuation(
			int targetChannel,
			RecordSerializer<T> serializer) throws IOException, InterruptedException;
	...
	protected int addBufferConsumer(BufferConsumer consumer, int targetChannel) throws IOException {
		return targetPartition.addBufferConsumer(consumer, targetChannel);
	}

	protected int addBufferConsumer(
			BufferConsumer consumer,
			int targetChannel,
			int partialRecordLength) throws IOException {
		return targetPartition.addBufferConsumer(consumer, targetChannel, partialRecordLength);
	}
	...
	public BufferBuilder requestNewBufferBuilder(int targetChannel) throws IOException, InterruptedException {
		...
		if (builder == null) {
		    backPressuredTimePerSecond.markStart();
			builder = targetPartition.getBufferBuilder(targetChannel);
			backPressuredTimePerSecond.markEnd();
		}
		return builder;
	}
