...
public class TaskStateStats implements Serializable {
	...
	private int numLogicallyMappedSubtasks;
	...
	boolean reportLogicallyMappedSubtaskStats(SubtaskStateStats subtask) {
		checkNotNull(subtask, "Subtask stats");
		int subtaskIndex = subtask.getSubtaskIndex();
		...
		if (subtaskStats[subtaskIndex] == null) {
			...
			numLogicallyMappedSubtasks++;

			summaryStats.updateSummaryForLogicallyMappedSubtask(subtask);
	...
	public static class TaskStateStatsSummary implements Serializable {
		...
		void updateSummaryForLogicallyMappedSubtask(SubtaskStateStats subtaskStats) {
		}
