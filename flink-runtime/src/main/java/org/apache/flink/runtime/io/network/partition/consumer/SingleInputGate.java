...
public class SingleInputGate extends IndexedInputGate {
	...
	public void announceBufferSize(int bufferSize) {
		for (InputChannel channel : channels) {
			channel.announceBufferSize(bufferSize);
		}
	}
	...
	@Override
	public void reconnectForInputChannel(RemoteInputChannel inputChannel) {
		try {
			inputChannel.reRequestSubpartition(consumedSubpartitionIndex);
		} catch (IOException | InterruptedException e) {
			throw new RuntimeException("Failed to request partition for input channel " + inputChannel, e);
		}
	}
	...
	private BufferOrEvent transformEvent(
		...
		if (event.getClass() == EndOfPartitionEvent.class) {
			...
			LOG.info("{}: Received ChannelUnavailableEvent from {}", owningTaskName, this);
			int channelIndex = ((ChannelUnavailableEvent) event).getChannelIndex();
			if (channelIndex != currentChannel.getChannelIndex()) {
				throw new IllegalStateException(
						String.format(
								"Expected channel index: %d, got: %d",
								channelIndex,
