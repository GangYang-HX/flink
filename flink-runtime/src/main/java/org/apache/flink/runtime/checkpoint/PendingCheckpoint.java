...
import org.apache.flink.runtime.checkpoint.regional.CheckpointRegion;
...
public class PendingCheckpoint {
	...
	public static  final long DUMMY_ALIGNMENT_DURATION_MILLIS = -1;
	public static  final long DUMMY_CHECKPOINT_START_DELAY_MILLIS = -1;
	public static  final long DUMMY_SYNC_CHECKPOINT_DURATION = -1;
	public static  final long DUMMY_ASYNC_CHECKPOINT_DURATION = -1;
	...
	private CheckpointCoordinator.CheckpointCanceller canceller;

	private CheckpointException failureCause;

	private ExpiredCheckpointAnalyzer expiredCheckpointAnalyzer;
	...
	public PendingCheckpoint(
		...
		if (numOfSourceTasks != null) {
			this.expiredCheckpointAnalyzer = new ExpiredCheckpointAnalyzer(verticesToConfirm.size(), numOfSourceTasks);
		}
	}
	...
	public ExpiredCheckpointAnalyzer getExpiredCheckpointAnalyzer() {
		return expiredCheckpointAnalyzer;
	}
	 ...
	 */
	public boolean setCancellerHandle(
			ScheduledFuture<?> cancellerHandle,
			CheckpointCoordinator.CheckpointCanceller canceller) {
		synchronized (lock) {
	...
	public CheckpointCoordinator.CheckpointCanceller getCanceller() {
		return canceller;
	}

	public ScheduledFuture<?> getCancellerHandle() {
		return cancellerHandle;
	}
	...
	public CompletedCheckpoint finalizeCheckpoint(
		...
		synchronized (lock) {
			...
			try {
				...
				final CheckpointMetadata savepoint = new CheckpointMetadata(checkpointId, operatorStates.values(), masterStates, operatorNameAndTypes);
	...
	public void acknowledgeTriggerSnapshot(long delay) {
		synchronized (lock) {
			...
			if (expiredCheckpointAnalyzer != null) {
				expiredCheckpointAnalyzer.acknowledgeTriggerSnapshot(delay);
			}
		}
	}
	...
	public TaskAcknowledgeResult acknowledgeTask(
		...
		synchronized (lock) {
			...
			if (operatorSubtaskStates == null) {
				// We should create an empty state, otherwise the logical mapping will produce NPE.
				operatorIDs.forEach(oip -> {
					OperatorState operatorStateForThisCheckpoint = this.operatorStates.computeIfAbsent(
							oip.getGeneratedOperatorID(),
							oi -> new OperatorState(
									oi, vertex.getTotalNumberOfParallelSubtasks(), vertex.getMaxParallelism()));
					operatorStateForThisCheckpoint.putState(subtaskIndex, new OperatorSubtaskState());
				});
			} else {
	...
	/**
	 * Acknowledges all the tasks contained in the given {@link CheckpointRegion} and map the state in
	 * the given {@link CompletedCheckpoint} to this checkpoint.
	 *
	 * @param failedOrExpiredCheckpointRegion {@link CheckpointRegion} which will be mapped
	 * @param completedCheckpoint {@link CompletedCheckpoint} whose state will be mapped to this checkpoint
	 * @return TaskAcknowledgeResult of the operation
	 */
	public TaskAcknowledgeResult acknowledgeCheckpointRegion(
			CheckpointRegion failedOrExpiredCheckpointRegion,
			CompletedCheckpoint completedCheckpoint) {

		List<OperatorSubtaskState> statesToDiscard = new ArrayList<>();
		synchronized (lock) {
			if (discarded) {
				return TaskAcknowledgeResult.DISCARDED;
			}

			for (ExecutionAttemptID executionAttemptId : failedOrExpiredCheckpointRegion.getAllExecutionAttempts()) {
				ExecutionVertex vertex = notYetAcknowledgedTasks.remove(executionAttemptId);

				Set<OperatorInstanceID> operatorInstances;
				int subtaskIndex;
				if (vertex == null) {
					if (!acknowledgedTasks.contains(executionAttemptId)) {
						return TaskAcknowledgeResult.UNKNOWN;
					}
					acknowledgedTasks.remove(executionAttemptId);
					operatorInstances = failedOrExpiredCheckpointRegion.getOperatorInstances(executionAttemptId);
					vertex = failedOrExpiredCheckpointRegion.getExecutionVertex(executionAttemptId);
					subtaskIndex = vertex.getParallelSubtaskIndex();
					// Clean up states that are uploaded by tasks which belong to failed or expired
					// region.
					statesToDiscard.addAll(
							operatorInstances.stream()
									.map(
											oii -> {
												OperatorID operatorId = oii.getOperatorId();
												return operatorStates
														.get(operatorId)
					...
					subtaskIndex = vertex.getParallelSubtaskIndex();
					operatorInstances = new HashSet<>();
					List<OperatorIDPair> operatorIds = vertex.getJobVertex().getOperatorIDs();
					for (OperatorIDPair operatorIdPair : operatorIds) {
						operatorInstances.add(
								new OperatorInstanceID(subtaskIndex, operatorIdPair.getGeneratedOperatorID()));
					}
				}

				long mappingTimestamp = System.currentTimeMillis();
				long stateSize = 0L;

				for (OperatorInstanceID operatorInstanceId : operatorInstances) {
					OperatorID operatorId = operatorInstanceId.getOperatorId();
					OperatorState operatorStateFromLastCompletedCheckpoint =
							completedCheckpoint.getOperatorStates().get(operatorId);

					addOperatorSubtaskState(
							vertex,
							operatorId,
							subtaskIndex,
							operatorStateFromLastCompletedCheckpoint.getState(subtaskIndex),
							stateSize);
				}
				...
				reportSubtaskStats(
						...
						mappingTimestamp,
						stateSize,
						DUMMY_SYNC_CHECKPOINT_DURATION,
						DUMMY_ASYNC_CHECKPOINT_DURATION,
						DUMMY_ALIGNMENT_DURATION_MILLIS,
						DUMMY_CHECKPOINT_START_DELAY_MILLIS);
			}
	...
	private long addOperatorSubtaskState(
			ExecutionVertex vertex,
			OperatorID operatorId,
			int subtaskIndex,
			OperatorSubtaskState subtaskState,
			long stateSize) {
		int numSubtasks = vertex.getTotalNumberOfParallelSubtasks();
		int maxParallelism = vertex.getMaxParallelism();
		OperatorState operatorStateForThisCheckpoint =
				this.operatorStates.computeIfAbsent(
						operatorId, oi -> new OperatorState(oi, numSubtasks, maxParallelism));

		// if no real operatorSubtaskState was reported, we insert an empty state
		if (subtaskState == null) {
			subtaskState = OperatorSubtaskState.builder().build();
		}
		operatorStateForThisCheckpoint.putState(subtaskIndex, subtaskState);

		return stateSize + subtaskState.getStateSize();
	}

	private void reportSubtaskStats(
			boolean logicallyMapped,
		...
		if (statsCallback != null) {
			...
			if (logicallyMapped) {
				statsCallback.reportLogicallyMappedSubtaskStats(jobVertexId, subtaskStateStats);
			} else {
	...
	private void reportFailedCheckpoint(Exception cause) {
		...
		if (statsCallback != null) {
			...
			if (cause instanceof CheckpointException) {
				causeMessage = ExceptionUtils.getRootCauseMessage(cause);
			}
			statsCallback.reportFailedCheckpoint(failureTimestamp, new Exception(causeMessage));
		}
	}
