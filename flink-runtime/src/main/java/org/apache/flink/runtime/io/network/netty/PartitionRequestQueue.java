...
class PartitionRequestQueue extends ChannelInboundHandlerAdapter {
	...
	protected final ConcurrentMap<InputChannelID, NetworkSequenceViewReader> allReaders = new ConcurrentHashMap<>();
	...
	protected void releaseAllResources() throws IOException {
		// note: this is only ever executed by one thread: the Netty IO thread!
		for (NetworkSequenceViewReader reader : allReaders.values()) {
			LOG.info("To be released channel is " + reader.getReceiverId());
			releaseViewReader(reader);
		}
