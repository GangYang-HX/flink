 ...
 * {@link ResultSubpartition#createReadView(boolean, BufferAvailabilityListener)} of new data availability. Except by calling
...
public class PipelinedSubpartition extends ResultSubpartition
	...
	protected final PrioritizedDeque<BufferConsumerWithPartialRecordLength> buffers = new PrioritizedDeque<>();
	...
	protected long totalNumberOfBuffers;
	...
	@Nullable
	protected BacklogEventListener backlogEventListener;
	...
	public String toString() {
		...
		return String.format(
				"%s(%s)#%d [number of buffers: %d (%d bytes), number of buffers in backlog: %d, " +
						"finished? %s, read view? %s, blocked by checkpoint? %s]",
	...
	public void registerListener(BacklogEventListener backlogEventListener) {
		...
		LOG.info("Task: {}, subPartition index: {} register a listener {}", parent.getOwningTaskName(),
			getSubpartitionInfo().getSubPartitionIdx(), backlogEventListener);
	}
	...
	private void notifyChanged() {
		...
		BacklogEvent backlogChangedEvent = new BacklogEvent(getSubPartitionIndex(), getBuffersInBacklog());
	...
	@Override
	public int addBufferConsumer(BufferConsumer bufferConsumer) {
