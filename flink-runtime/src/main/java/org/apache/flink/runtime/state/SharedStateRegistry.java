...
public class SharedStateRegistry implements AutoCloseable {
	...
	/** All restored state objects by an artificial key */
	private final Map<SharedStateRegistryKey, SharedStateRegistry.SharedStateEntry> restoredStates;
	...
	public SharedStateRegistry(Executor asyncDisposalExecutor) {
		...
		this.restoredStates = new HashMap<>();
	...
	public Result unregisterReference(SharedStateRegistryKey registrationKey) {
		...
		synchronized (registeredStates) {
			boolean isRestoredState = false;

			entry = registeredStates.get(registrationKey);
            if (restoredStates.containsKey(registrationKey)) {
				entry = restoredStates.get(registrationKey);
			...
			if (entry.getReferenceCount() <= 0) {
				if (isRestoredState) {
					// Do not delete state which come from restored states.
					restoredStates.remove(registrationKey);
	...
	public void removeAllRestoredStates(Set<OperatorState> restoreOperatorStates) {

		Set<SharedStateRegistryKey> restoreSharedStateRegistryKeys = getAllSharedStateRegistryKeysFromOperatorStates(restoreOperatorStates);

		synchronized (registeredStates) {
			for (SharedStateRegistryKey restoreSharedStateRegistryKey : restoreSharedStateRegistryKeys) {
				SharedStateEntry sharedStateEntry = registeredStates.remove(restoreSharedStateRegistryKey);
                if (sharedStateEntry != null) {
					restoredStates.put(restoreSharedStateRegistryKey, sharedStateEntry);
					unregisterReference(restoreSharedStateRegistryKey);
				}
			}
		}
	}

	private Set<SharedStateRegistryKey> getAllSharedStateRegistryKeysFromOperatorStates(Set<OperatorState> operatorStates) {
		Set<SharedStateRegistryKey> restoreSharedStateRegistryKeys = new HashSet<>();

		if (operatorStates != null) {
			operatorStates.forEach(
				restoreOperatorState -> {
					restoreOperatorState.getSubtaskStates().values().forEach(subtaskState -> {
						subtaskState.getAllKeyedStates().forEach(
							keyedStateHandle -> {
								if (keyedStateHandle instanceof IncrementalRemoteKeyedStateHandle) {
									restoreSharedStateRegistryKeys.addAll(((IncrementalRemoteKeyedStateHandle) keyedStateHandle).getSharedStateRegistryKeys());
								}
							}
						);
					});
		...
		return restoreSharedStateRegistryKeys;
	}
	...
	private static final class AsyncDisposalRunnable implements Runnable {
		...
		public void run() {
			try {
				toDispose.discardStateWithRetry();
			} catch (Exception e) {
