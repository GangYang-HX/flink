...
public interface CheckpointStreamFactory {
	...
	/**
	 * Creates a new {@link CheckpointStateOutputStream} with interrupt ability.
	 * When the stream is closed, it returns a state handle that can retrieve the state back.
	 *
	 * @param scope The state's scope, whether it is exclusive or shared.
	 * @return An output stream that writes state for the given checkpoint.
	 *
	 * @throws IOException exception may occur while creating the stream and flushing to file because of interrupted.
	 */
	default CheckpointStateOutputStream createInterruptableCheckpointStateOutputStream(
			CheckpointedStateScope scope) throws IOException {
		throw new UnsupportedOperationException("This CheckpointStreamFactory doesn't support it!");
	}
	 ...
	 * is closed, it returns a state handle that can retrieve the state back.
	 *
	 * One single operator should create one single output stream, now only support FS output stream.
	 * The output stream create the dir like this:
	 * +- ${checkpoint-dir}
	 * |  \- meta
	 * |     +- ${operator-id}
	 * |     |  \- state-desc-file
	 *
	 * @param operatorID Operator's description.
	 * @return An output stream that writes state for the given checkpoint.
	 *
	 * @throws IOException Exceptions may occur while creating the stream and should be forwarded.
	 */
	default CheckpointStateOutputStream createCheckpointStateDescOutputStream(OperatorID operatorID) throws IOException {
