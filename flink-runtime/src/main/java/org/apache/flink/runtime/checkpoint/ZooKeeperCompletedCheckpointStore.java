...
public class ZooKeeperCompletedCheckpointStore implements CompletedCheckpointStore {
	...
	public void dereference(long referencingCheckpointId, CompletedCheckpoint referencedCheckpoint) {
		try {
			...
			LOG.warn("Failed to discard completed checkpoint", e);
		}
		...
		if (!referencedCheckpoint.isReferenced()) {
			if (completedCheckpoints.size() > maxNumberOfCheckpointsToRetain) {
				...
				do {
					CompletedCheckpoint completedCheckpoint = completedCheckpoints.removeFirst();
					if (completedCheckpoint == referencedCheckpoint) {
						tryRemoveCompletedCheckpoint(
								referencedCheckpoint,
								completedCheckpoint1 -> {
									// Nothing to do as the checkpoint has already been discarded in dereference.
								});
						break;
					} else {
				...
				while (!removedCheckpoints.isEmpty()) {
					completedCheckpoints.addFirst(removedCheckpoints.removeFirst());
				}
			}
		}
