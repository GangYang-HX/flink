...
import static org.apache.flink.runtime.state.StateBackendLoader.TAISHAN_STATE_BACKEND_NAME;
...
public final class KeyGroupRangeAssignment {
	...
	private static final int MIN_DEFAULT_MAX_PARALLELISM = 1024;
	private static final int MIN_DEFAULT_MAX_PARALLELISM_TAISHAN = 64;
	...
	public static int computeNewDefaultMaxParallelism(int operatorParallelism) {
		return Math.max(
			computeDefaultMaxParallelism(operatorParallelism * 10),
			MIN_DEFAULT_MAX_PARALLELISM);
	}

	public static int computeNewDefaultMaxParallelism(int operatorParallelism, int multiple, int minDefaultMaxParallelism) {
		return Math.max(
			computeDefaultMaxParallelism(operatorParallelism * multiple),
			minDefaultMaxParallelism);
	}

	public static int computeNewDefaultMaxParallelism(int operatorParallelism, String stateBackendName) {
		int multiple;
		int minDefaultMaxParallelism;
		switch (stateBackendName) {
			case TAISHAN_STATE_BACKEND_NAME:
				multiple = 1;
				minDefaultMaxParallelism = MIN_DEFAULT_MAX_PARALLELISM_TAISHAN;
				break;
			default:
				multiple = 10;
				minDefaultMaxParallelism = MIN_DEFAULT_MAX_PARALLELISM;
		}
		return computeNewDefaultMaxParallelism(operatorParallelism, multiple, minDefaultMaxParallelism);
	}
