...
public abstract class NettyMessage {
	...
	static class PartitionRequest extends NettyMessage {
		...
		PartitionRequest(
				...
				InputChannelID receiverId,
				int credit) {
			this(partitionId, queueIndex, receiverId, credit, false);
		}
		...
		ByteBuf write(ByteBufAllocator allocator) throws IOException {
			...
			try {
				result = allocateBuffer(allocator, ID, 20 + 16 + 4 + 16 + 4 + 1);
		...
		static PartitionRequest readFrom(ByteBuf buffer) {
			...
			return new PartitionRequest(partitionId, queueIndex, receiverId, credit, isRetry);
		}

		@Override
		public String toString() {
			return String.format("PartitionRequest(%s:%d:%d:%b)", partitionId, queueIndex, credit, isRetry);
		}
	}
	...
	static class CloseRequest extends NettyMessage {
		...
		static CloseRequest readFrom(@SuppressWarnings("unused") ByteBuf buffer) {
		...
		ByteBuf write(ByteBufAllocator allocator) throws IOException {
			...
			try {
				result = allocateBuffer(
						allocator,
						ID,
						Long.BYTES + InputChannelID.SIZE);
