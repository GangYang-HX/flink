...
/**
 * The CheckpointInfo is used to collect flink checkpoint information. It is mainly used to send
 * flink checkpoint information to external systems. It is worth noting that OperatorState is a
 * nested structure, and states are the result of flattening them. For some jobs, the number of
 * elements in the states list may be very large, and the implementer of the Listener needs to be
 * careful not to hold the reference of the list for a long time to prevent the gc pressure of the
 * job manager.
 */
public class CheckpointInfo {
    ...
    /**
     * The external system can judge the type (checkpoint、full checkpoint or savepoint) through this
     * field.
     */
    private OuterCheckpointType outerCheckpointType;

    /**
     * MetadataFileFullPath will set the metadata path (e.g. XX/_metadata) if the checkpoint was
     * triggered、completed or discarded, otherwise it will be set to null.
     */
    @Nullable private String metadataFileFullPath;

    private CheckpointResult result;

    @Nullable private CheckpointFailureReason failureReason;
    ...
    /**
     * StateInfo provides flink state basic information. It's worth noting that not all state have
     * state paths. Because if the state size is small, which will be saved in metadata.
     */
    public static class StateInfo {
        private final StateType type;
        @Nullable private final Path statePath;
    ...
    /** Include flink checkpoint result. */
    ...
    /**
     * A checkpoint type that only applies to external systems, to distinguish between
     * savepoint、full checkpoint and checkpoint.
     */
