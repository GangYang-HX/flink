...
public class Task implements Runnable, TaskSlotPayload, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
	...
	private TaskOperatorEventGateway operatorCoordinatorEventGateway;
	...
	/** The only one throughput calculator per subtask */
	private ThroughputCalculator throughputCalculator;

	private SpeculativeTasksManager speculativeTasksManager;

	/** The data sent from the job manager to restore this task. */
	private JobManagerTaskRestore jobManagerTaskRestore;

	/** The environment for user code to execute. */
	...
	public Task(
		...
		this.throughputCalculator = new ThroughputCalculator(
				...
				tmConfig.get(BUFFER_DEBLOAT_SAMPLES));
		int counter = 0;
		...
		executingThread = new Thread(TASK_THREADS_GROUP, this, taskInfo.getTaskNameWithSubtasks(true));

		// Add CallerContext job id, we may add more task information here in the future.
		HadoopUtils.setCallerContextJobID(jobId.toString(), tmConfig);
	...
	public void setSpeculativeTasksManager(SpeculativeTasksManager speculativeTasksManager) {
	...
	private void doRun() {
		...
		try {
			...
			invokable = loadAndInstantiateInvokable(userCodeClassLoader, nameOfInvokableClass, environment);
			...
			dimensions.put(StatisticsUtils.TASK_NAME, taskName.substring(0, Math.min(taskName.length(), 10)));
			dimensions.put(StatisticsUtils.TASK_INDEX, String.valueOf(taskInfo.getIndexOfThisSubtask()));
			...
			LOG.info("Received exception from StreamTask", t);
			...
			try {
				...
				// to failExternally()
				LOG.info("Start transition state {}, {}", this.executionState, getTaskInfo());
				while (true) {
					ExecutionState current = this.executionState;
				...
				LOG.info("Transition state over {}, {}", this.executionState, getTaskInfo());
			}
			catch (Throwable tt) {
			...
			try {
				...
				LOG.info("Notify final state for {} ({}).", taskNameWithSubtask, executionId);
				notifyFinalState();
			}
			catch (Throwable t) {
	...
	private ClassLoader createUserCodeClassloader() throws Exception {
		...
		StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.TASK_LOAD_JAR_FILES);

		// first add local jars url which are set in PipelineOptions.JARS to user classpaths
		Collection<URL> userClasspaths = ClusterEntrypointUtils.addUserJarsToClasspath(
			taskManagerConfig.getConfiguration(),
			requiredClasspaths);

		// triggers the download of all missing jar files from the job manager
		final ClassLoader userCodeClassLoader = classLoaderHandle.getOrResolveClassLoader(requiredJarFiles, userClasspaths);
		...
		StatisticsUtils.reportEndTracking(StartupUtils.StartupPhase.TASK_LOAD_JAR_FILES,
				true,
				jobId.toString(),
				metrics.getAllVariables());
	...
	public void setOperatorCoordinatorEventGateway(TaskOperatorEventGateway operatorCoordinatorEventGateway) {
		this.operatorCoordinatorEventGateway = operatorCoordinatorEventGateway;
	}
	...
	public void setPartitionProducerStateChecker(PartitionProducerStateChecker partitionProducerStateChecker) {
	...
	public JobManagerTaskRestore getJobManagerTaskRestore() {
