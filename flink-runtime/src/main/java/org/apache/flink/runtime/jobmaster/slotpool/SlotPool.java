...
public interface SlotPool extends AllocatedSlotActions, AutoCloseable {
	 ...
	 */
	boolean registerTaskManager(ResourceID resourceID, TaskManagerLocation taskManagerLocation);
	...
	/**
	 * Restore the {@link SlotPool} with initial slots.
	 *
	 * @param initialSlots allocated slots which will be added to this pool
	 */
	void restoreAllocatedSlots(Collection<AllocatedSlot> initialSlots);
	...
	/**
	 * Add the given hosts to the blacklist.
	 *
	 * @param blackListedHosts list of hosts to add
	 */
	default void addHostsToBlacklist(@Nonnull List<String> blackListedHosts) {
		throw new UnsupportedOperationException();
	}

	/**
	 * Remove the given hosts from the blacklist.
	 *
	 * @param blackListedHosts list of hosts to remove
	 */
	default void removeHostsFromBlacklist(List<String> blackListedHosts) {
		throw new UnsupportedOperationException();
	}

	default int getNumOfTaskManager() {
		return 0;
	}
}
