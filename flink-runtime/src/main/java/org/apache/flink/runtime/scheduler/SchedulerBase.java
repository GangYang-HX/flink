...
import org.apache.flink.runtime.metrics.sla.CauseType;
import org.apache.flink.runtime.metrics.sla.UnavailableMetricStoreStrategy;
...
import org.apache.flink.runtime.statistics.CheckpointRestoreInfoUtils;
import org.apache.flink.runtime.statistics.JobInfoUtils;
...
public abstract class SchedulerBase implements SchedulerNG {
	...
	private final Scheduler slotProvider;
	...
	protected final AtomicInteger unFinishedDeployment;

	// reconcile job
    protected final ArchivedExecutionGraphStore archivedExecutionGraphStore;

    protected ArchivedExecutionGraph archivedExecutionGraph;

	protected boolean shouldReconcile = false;

	private final Time reconcileTimeout;

	protected final UnavailableMetricStoreStrategy metricRecordsStrategy;
	...
	public SchedulerBase(
		...
		final boolean haReconcileEnabled,
		final Time reconcileTimeout) throws Exception {
		...
		this.reconcileTimeout = checkNotNull(reconcileTimeout);
		...
		this.metricRecordsStrategy = executionGraph.getMetricRecordsStrategy();
		...
		this.unFinishedDeployment = new AtomicInteger(0);
	}
	...
	private ExecutionGraph createAndRestoreExecutionGraph(
		...
		boolean haReconcileEnabled) throws Exception {

		ExecutionGraph newExecutionGraph = createExecutionGraph(currentJobManagerJobMetricGroup, shuffleMaster, partitionTracker);

        // first try to restore from ArchivedExecutionGraph
        shouldReconcile = haReconcileEnabled
			&& (archivedExecutionGraph = archivedExecutionGraphStore.get(getJobId())) != null;
        if (shouldReconcile) {
            restoreFromArchivedExecutionGraph(newExecutionGraph, archivedExecutionGraph);
		}

		// second we need also restore from checkpoint or savepoint
	...
	private void tryRestoreExecutionGraphFromSavepoint(ExecutionGraph executionGraphToRestore, SavepointRestoreSettings savepointRestoreSettings) throws Exception {
		if (savepointRestoreSettings.restoreSavepoint()) {
			...
			if (checkpointCoordinator != null) {
				try {
					CheckpointRestoreInfoUtils.registerCheckpointRestoreJob(
						getJobId(),
					...
					String restorePath = CheckpointRestoreInfoUtils.deregisterCheckpointRestoreJob(getJobId());
					if (StringUtils.isNotEmpty(restorePath)) {
						StatisticsUtils.reportCheckpointRestoreInfo(
							getJobId().toString(),
							restorePath,
							System.currentTimeMillis(),
							StatisticsUtils.FAILURE_STATUS,
							e
						);
					}
					throw e;
    ...
    /**
     * Restore all Execution of ExecutionGraph from ArchivedExecutionGraph.
     *
     * @param archivedExecutionGraph {@link ArchivedExecutionGraph} which helps restore current
     *     {@link ExecutionGraph}
     */
	@VisibleForTesting
    void restoreFromArchivedExecutionGraph(
        ...
        archivedExecutionGraph
                ...
                .forEach(
                        archivedExecutionVertex -> {
                            final Execution executionToRestore = executionGraphToRestore
								.getAllVertices()
								.get(archivedExecutionVertex.getJobVertexId())
								.getTaskVertices()[archivedExecutionVertex.getParallelSubtaskIndex()]
								.getCurrentExecutionAttempt();

                            executionToRestore.restore(
	...
	private void registerJobMetrics() {
        ...
        jobManagerJobMetricGroup.gauge(
			MetricNames.NUM_RECONCILE_FAILED,
			() -> executionGraph.getNumberOfReconcileFailed());
		jobManagerJobMetricGroup.gauge(
				MetricNames.NUM_RESTARTS_ATTEMPTS,
				() -> executionGraph.getNumberOfRestartAttempts());
	}
	...
	public void cancel() {
		...
		incrementVersionsOfAllVertices();
		metricRecordsStrategy.terminateHook(CauseType.CANCELLED, System.currentTimeMillis(), false);
	...
	public final boolean updateTaskExecutionState(final TaskExecutionState taskExecutionState) {
		checkState(executionGraph.getState() != JobStatus.RECONCILING);
		...
		if (updateSuccess) {
			...
			maybeHandleTaskRunning(taskExecutionState);
	...
	private void maybeHandleTaskRunning(final TaskExecutionState taskExecutionState) {
		if (taskExecutionState.getExecutionState() == ExecutionState.RUNNING) {
			handleTaskRunning(taskExecutionState);
		}
	}

	private void handleTaskRunning(final TaskExecutionState taskExecutionState) {

		// report single task deployment time
		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.DEPLOY_TASK,
				true,
				getJobId().toString(),
				jobManagerJobMetricGroup.getAllVariables());

		if (unFinishedDeployment.decrementAndGet() == 0) {
			handleAllTasksRunning();
		}
	}

	private void handleAllTasksRunning() {
		reportStatisticsAfterDeployment();

		// When all task return to RUNNING state, save its ArchivedExecutionGraph
		mainThreadExecutor.execute(
			() -> {
				ArchivedExecutionGraph newArchived = requestJob();
				try {
					archivedExecutionGraphStore.put(newArchived);
				} catch (IOException e) {
					// only log, not fail job here.
					log.error(
						"Put ArchivedExecutionGraph error, jobId={}",
						newArchived.getJobID(),
						e);
				}
			});
	}

	private void reportStatisticsAfterDeployment() {
		if (!StartupUtils.isRestartedJob(getJobId().toString())) {
			// Only report whole startup time for first start
			reportStartPhases();
		} else {
			reportRestartPhases();
		}
	...
	private void reportStartPhases() {
		// report every phases when first start
		// START_AND_RUN_JOB = CREATE_EXECUTION_AT_START + ALLOCATE_ALL_SLOTS_AT_START + DEPLOY_ALL_TASKS_AT_START

		Map<String, String> dimensions = jobManagerJobMetricGroup.getAllVariables();
		dimensions.put(
				StatisticsUtils.RESTART_COUNT,
				String.valueOf(StartupUtils.getRestartedCountByJobID(getJobId().toString())));

		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.DEPLOY_ALL_TASKS_AT_START,
				true,
				getJobId().toString(),
				dimensions);

		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.START_JOB,
				true,
				getJobId().toString(),
				dimensions);

		long requestTimestamp = jobMasterConfiguration.getLong(
			StartupUtils.StartupPhase.CLIENT_START_JOB.toString(),
			0);
		log.info("Get client start request timestamp from config: {}", requestTimestamp);
		if (requestTimestamp != 0) {
			StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.START_JOB_FROM_CLIENT, requestTimestamp);
			StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.START_JOB_FROM_CLIENT,
				true,
				getJobId().toString(),
				dimensions);
		}
	...
	private void reportRestartPhases() {
		Map<String, String> dimensions = jobManagerJobMetricGroup.getAllVariables();
		dimensions.put(
				StatisticsUtils.RESTART_COUNT,
				String.valueOf(StartupUtils.getRestartedCountByJobID(getJobId().toString())));
		...
		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.RESTART_JOB,
				true,
				getJobId().toString(),
				dimensions);

		StatisticsUtils.reportEndTracking(
				StartupUtils.StartupPhase.DEPLOY_ALL_TASKS_AT_RESTART,
				true,
				getJobId().toString(),
				dimensions);
	}
	...
	private void cancelJobAndReport() {
		Long globalModVersion = JobInfoUtils.deregisterStartedJob(getJobId());
		if (globalModVersion != null) {
			StatisticsUtils.reportJobInfo(
				getJobId().toString(),
				JobStatus.FINISHED,
				System.currentTimeMillis(),
				StatisticsUtils.SUCCESS_STATUS,
				globalModVersion,
				null);
		}
	}
	...
	@Override
	public CompletableFuture<Long> triggerFullCheckpoint(final boolean incremental, final String targetDirectory) {
		mainThreadExecutor.assertRunningInMainThread();
		...
		log.info("Triggering full checkpoint for job {}.", jobGraph.getJobID());

		return checkpointCoordinator
			.triggerFullCheckpoint(incremental, targetDirectory)
			.handleAsync((cp, throwable) -> cp.getCheckpointID(), mainThreadExecutor);
	}
	...
	public void acknowledgeTriggerSnapshot(AcknowledgeTriggerSnapshot acknowledgeTriggerSnapshot) {
		...
		if (checkpointCoordinator != null) {
			ioExecutor.execute(() -> {
				try {
					checkpointCoordinator.receiveAcknowledgeTriggerSnapshotMessage(acknowledgeTriggerSnapshot);
				} catch (Exception e) {
					log.error("Error in CheckpointCoordinator while processing {}", acknowledgeTriggerSnapshot, e);
				}
			});
		} else {
			String errorMessage = "Received acknowledgeTriggerSnapshot message for job {} with no CheckpointCoordinator";
			if (executionGraph.getState() == JobStatus.RUNNING) {
				log.error(errorMessage, jobGraph.getJobID());
			} else {
				log.debug(errorMessage, jobGraph.getJobID());
	...
	public CompletableFuture<Boolean> restart() {
	    executionGraph.transitionState(executionGraph.getState(), JobStatus.RESTARTING);
	...
	/**
	 * Reconcile the Job with its corresponding tasks.
	 */
	...
	@Override
	public void restoreTaskExecutionState(final TaskExecutionState taskExecutionState) {
		checkState(executionGraph.getState() == JobStatus.RECONCILING);
