...
public interface ArchivedExecutionGraphStore extends Closeable {
	...
	/**
	 * Remove the {@link ArchivedExecutionGraph} for the given job id. Or do nothing if it isn't stored.
	 *
	 * @param jobId identifying the serializable execution graph to retrieve
	 * @throws IOException if the serializable execution graph could not be removed in the store
	 */
	default void remove(JobID jobId) throws IOException {}
}
