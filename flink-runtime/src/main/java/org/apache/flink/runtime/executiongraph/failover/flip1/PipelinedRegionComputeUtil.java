...
public final class PipelinedRegionComputeUtil {
	...
	public static <V extends Vertex<?, ?, V, R>, R extends Result<?, ?, V, R>> Set<Set<V>> computePipelinedRegions(
		...
		for (V vertex : topology.getVertices()) {
			...
			for (R consumedResult : vertex.getConsumedResults()) {
				boolean isBroadcastConnected = false;
				if (vertex instanceof DefaultExecutionVertex) {
					DefaultExecutionVertex defaultExecutionVertex = ((DefaultExecutionVertex) vertex);
					JobVertex jobVertex = getJobVertex(defaultExecutionVertex);
					JobVertex producerJobVertex = getJobVertex((DefaultExecutionVertex) consumedResult.getProducer());
					isBroadcastConnected = (jobVertex.getBroadcastedUpstreams() != null) &&
							jobVertex.getBroadcastedUpstreams().contains(producerJobVertex);
				}
				if (consumedResult.getResultType().isPipelined() && !isBroadcastConnected) {
	...
	private static JobVertex getJobVertex(final DefaultExecutionVertex defaultExecutionVertex) {
		return defaultExecutionVertex.getExecutionVertex().getJobVertex().getJobVertex();
	}
