...
public class EventSerializer {
	...
	private static final int CHANNEL_UNAVAILABLE_EVENT = 8;
	...
	private static final int CHECKPOINT_TYPE_FULL_CHECKPOINT = 3;

	private static final int CHECKPOINT_TYPE_INCREMENTAL_FULL_CHECKPOINT = 4;
	...
	public static ByteBuffer toSerializedEvent(AbstractEvent event) throws IOException {
		...
		if (eventClass == EndOfPartitionEvent.class) {
            ...
            buf.flip();
            return buf;
        } else if (eventClass == ChannelUnavailableEvent.class) {
			ChannelUnavailableEvent channelUnavailableEvent = (ChannelUnavailableEvent) event;
			ByteBuffer buffer = ByteBuffer.allocate(8); // 4 bytes for channelIndex, 4 bytes for type
			buffer.putInt(0, CHANNEL_UNAVAILABLE_EVENT);
			buffer.putInt(4, channelUnavailableEvent.getChannelIndex());
			return buffer;
		} else {
			try {
	...
	public static AbstractEvent fromSerializedEvent(ByteBuffer buffer, ClassLoader classLoader) throws IOException {
		...
		try {
            ...
            } else if (type == CHANNEL_UNAVAILABLE_EVENT) {
				int channelIndex = buffer.getInt();
				return new ChannelUnavailableEvent(channelIndex);
	...
	private static ByteBuffer serializeCheckpointBarrier(CheckpointBarrier barrier) throws IOException {
		...
		} else if (checkpointType == CheckpointType.INCREMENTAL_FULL_CHECKPOINT) {
			typeInt =  CHECKPOINT_TYPE_INCREMENTAL_FULL_CHECKPOINT;
		} else {
	...
	private static CheckpointBarrier deserializeCheckpointBarrier(ByteBuffer buffer) throws IOException {
		...
		} else if (checkpointTypeCode == CHECKPOINT_TYPE_INCREMENTAL_FULL_CHECKPOINT) {
			checkpointType = CheckpointType.INCREMENTAL_FULL_CHECKPOINT;
		} else {
