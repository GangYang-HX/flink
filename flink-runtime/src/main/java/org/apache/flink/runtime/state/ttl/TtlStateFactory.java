...
public class TtlStateFactory<K, N, SV, TTLSV, S extends State, IS extends S> {
	...
	private <OIS extends State, TTLS extends State, V, TTLV> TtlStateContext<OIS, V>
		...
		checkStatebackendTtlUpdateType();
	...
	private void checkStatebackendTtlUpdateType() {
		Preconditions.checkArgument(!(!stateBackend.isSupportedOnReadAndWrite()
				&& ttlConfig.getUpdateType() == StateTtlConfig.UpdateType.OnReadAndWrite),
			"State backend is currently not supported with OnReadAndWrite Ttl.");
	}
