...
import org.apache.flink.runtime.rest.handler.resource.RequestResourceHandlers;
import org.apache.flink.runtime.rest.handler.resource.RequestResourceStatusHeaders;
import org.apache.flink.runtime.rest.handler.resource.RequestResourceTriggerHeaders;
import org.apache.flink.runtime.rest.handler.job.restart.RemoveFromBlackListHandler;
import org.apache.flink.runtime.rest.handler.job.restart.RemoveFromBlackListTriggerHeaders;
import org.apache.flink.runtime.rest.handler.job.restart.RestartHandlers;
import org.apache.flink.runtime.rest.handler.job.restart.RestartStatusHeaders;
import org.apache.flink.runtime.rest.handler.job.restart.RestartTriggerHeaders;
...
import org.apache.flink.runtime.rest.messages.job.fullcheckpoint.FullCheckpointTriggerHandlers;
...
public class WebMonitorEndpoint<T extends RestfulGateway>
	...
	protected List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(final CompletableFuture<String> localAddressFuture) {
		...
		final String defaultCheckpointDir = clusterConfiguration.getString(CheckpointingOptions.CHECKPOINTS_DIRECTORY);

		final FullCheckpointTriggerHandlers fullCheckpointTriggerHandlers = new FullCheckpointTriggerHandlers(defaultCheckpointDir);

		final FullCheckpointTriggerHandlers.FullCheckpointTriggerHandler fullCheckpointTriggerHandler =
			fullCheckpointTriggerHandlers.new FullCheckpointTriggerHandler(
				leaderRetriever,
				timeout,
				responseHeaders);

		final FullCheckpointTriggerHandlers.FullCheckpointStatusHandler fullCheckpointStatusHandler =
			fullCheckpointTriggerHandlers.new FullCheckpointStatusHandler(
				leaderRetriever,
				timeout,
				responseHeaders);
		...
		final RestartHandlers restartHandlers = new RestartHandlers();

		final RestartHandlers.RestartTriggerHandler restartTriggerHandler =
				restartHandlers.new RestartTriggerHandler(
						leaderRetriever,
						timeout,
						responseHeaders,
						RestartTriggerHeaders.getInstance());

		final RestartHandlers.RestartStatusHandler restartStatusHandler =
				restartHandlers.new RestartStatusHandler(
					leaderRetriever,
					timeout,
					responseHeaders,
					RestartStatusHeaders.getInstance());

		final RemoveFromBlackListHandler removeFromBlackListHandler =
			new RemoveFromBlackListHandler(
				leaderRetriever,
				timeout,
				responseHeaders,
				RemoveFromBlackListTriggerHeaders.getInstance());
		...
		final RequestResourceHandlers requestResourceHandlers = new RequestResourceHandlers();

		final RequestResourceHandlers.RequestResourceTriggerHandler requestResourceTriggerHandler =
				requestResourceHandlers.new RequestResourceTriggerHandler(
						leaderRetriever,
						timeout,
						responseHeaders,
						RequestResourceTriggerHeaders.getInstance());

		final RequestResourceHandlers.RequestResourceStatusHandler requestResourceStatusHandler =
				requestResourceHandlers.new RequestResourceStatusHandler(
						leaderRetriever,
						timeout,
						responseHeaders,
						RequestResourceStatusHeaders.getInstance());
		...
		handlers.add(Tuple2.of(fullCheckpointTriggerHandler.getMessageHeaders(), fullCheckpointTriggerHandler));
		handlers.add(Tuple2.of(fullCheckpointStatusHandler.getMessageHeaders(), fullCheckpointStatusHandler));
		...
		handlers.add(Tuple2.of(requestResourceTriggerHandler.getMessageHeaders(), requestResourceTriggerHandler));
		handlers.add(Tuple2.of(requestResourceStatusHandler.getMessageHeaders(), requestResourceStatusHandler));
		handlers.add(Tuple2.of(restartTriggerHandler.getMessageHeaders(), restartTriggerHandler));
		handlers.add(Tuple2.of(restartStatusHandler.getMessageHeaders(), restartStatusHandler));
		handlers.add(Tuple2.of(removeFromBlackListHandler.getMessageHeaders(), removeFromBlackListHandler));
