...
public interface RestfulGateway extends RpcGateway {
	...
	/**
	 * Triggers a full checkpoint.
	 *
	 * @param jobId           ID of the job for which the savepoint should be triggered.
	 * @param timeout         Timeout for the asynchronous operation
	 * @return A future to the checkpoint ID.
	 */
	default CompletableFuture<Long> triggerFullCheckpoint(
		JobID jobId,
		@RpcTimeout Time timeout,
		boolean incremental,
		String targetDirectory
	...
	/**
	 * Requests the specified resources (in slot unit) for the given jobId.
	 *
	 * @param jobId ID of job which will use the requested resources
	 * @param noTimeOut the allocated resources should not be released
	 * @param numSlots number of slots to request
	 * @param timeout for the asynchronous operation
	 * @return Future containing the request result
	 */
	default CompletableFuture<Integer> requestResourceWithSlots(
			Optional<JobID> jobId,
			boolean resourceCanBeShared,
			boolean noTimeOut,
			int numSlots,
			@RpcTimeout Time timeout) {
		throw new UnsupportedOperationException();
	...
	/**
	 * Requests the specified resources (in task manager unit) for the given jobId.
	 *
	 * @param jobId ID of job which will use the requested resources
	 * @param noTimeOut the allocated resources should not be released
	 * @param numTaskManagers number of task managers to request
	 * @param timeout for the asynchronous operation
	 * @return Future containing the request result
	 */
	default CompletableFuture<Integer> requestResourceWithTaskManagers(
			Optional<JobID> jobId,
			boolean resourceCanBeShared,
			boolean noTimeOut,
			int numTaskManagers,
	...
	/**
	 * Restart the job identified by the given {@link JobID}.
	 *
	 * @param jobId {@link JobID} identifying the job
	 * @param blacklistedHosts list of strings representing host of containers
	 * @param removeHostsAfterRestart whether to remove hosts after job restarted or failed
	 * @param timeout for the asynchronous operation
	 * @return true if the restarting succeeded, else false
	 */
	default CompletableFuture<Boolean> restartJob(
			JobID jobId,
			@Nullable List<String> blacklistedHosts,
	...
	/**
	 * Remove the given blacklistedHosts from the black list.
	 *  @param jobId the job id
	 * @param blacklistedHosts to be removed
	 */
	default void removeFromBlackList(JobID jobId, List<String> blacklistedHosts) {
		throw new UnsupportedOperationException();
	}
}
