...
public class AbstractFileSystemReaderWriter<T extends Serializable> implements FileSystemReaderWriter<T> {

	private static final Logger LOG = LoggerFactory.getLogger(AbstractFileSystemReaderWriter.class);
	...
	public AbstractFileSystemReaderWriter(FileSystem fs, Configuration configuration) {
		this.fs = checkNotNull(fs, "FileSystem");
		this.maxRetry = configuration
			.getInteger(HighAvailabilityOptions.HA_FILESYSTEM_MAX_RETRY_ATTEMPTS);
		this.retryInterval = configuration
			.getInteger(HighAvailabilityOptions.HA_FILESYSTEM_RETRY_WAIT);
	}
