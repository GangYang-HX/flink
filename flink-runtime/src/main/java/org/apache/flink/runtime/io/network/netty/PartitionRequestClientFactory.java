...
class PartitionRequestClientFactory {
	...
	NettyPartitionRequestClient createPartitionRequestClient(ConnectionID connectionId) throws IOException, InterruptedException {
		...
		connectionId = new ConnectionID(connectionId.getAddress(),
	...
	void closeOpenChannelConnections(ConnectionID connectionId) {
		ConnectionID closedConnectionId = new ConnectionID(connectionId.getAddress(),
			connectionId.getConnectionIndex() % maxNumberOfConnections);

		CompletableFuture<NettyPartitionRequestClient> entry = clients.get(closedConnectionId);

		if (entry != null && !entry.isDone()) {
			entry.thenAccept(client -> {
				if (client.disposeIfNotUsed()) {
					clients.remove(closedConnectionId, entry);
				}
			});
		}
	}
	...
	void destroyPartitionRequestClient(ConnectionID connectionId) {
		ConnectionID destroyedConnectionId = new ConnectionID(connectionId.getAddress(),
			connectionId.getConnectionIndex() % maxNumberOfConnections);

		final CompletableFuture<NettyPartitionRequestClient> future = clients.get(destroyedConnectionId);
		if (future != null && future.isDone()) {
			clients.remove(destroyedConnectionId, future);
			LOG.info("Remove the PartitionRequestClient successfully.");
		}
	}
}
