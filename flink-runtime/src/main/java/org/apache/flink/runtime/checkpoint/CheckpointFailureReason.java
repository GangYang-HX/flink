...
public enum CheckpointFailureReason {
	...
	MAX_CONSECUTIVE_FAILURES_OR_EXPIRES_OF_CHECKPOINT_REGION_EXCEEDED(
			false,
			"Consecutive failures or expires of checkpoint region reached max allowed value"),

	MAX_FAILED_OR_EXPIRED_RATIO_OF_CHECKPOINT_REGIONS_EXCEEDED(
			false,
			"Ratio of failed or expired regions reached max allowed value"),

	NO_SUCCEEDED_CHECKPOINT_FOR_REGION(
			false,
			"There is no succeeded checkpoint for region");
