...
/**
 * Leader election service for Yarn. The election is more or less useless, because
 * there always exists at most one running JobManager on a Yarn cluster which will always be elected.
 * <p>
 * The real usage for this election service is both writing data into Zookeeper and the external FileSystem,
 * with the fact that writing into the FileSystem is a must while writing into Zookeeper is a try-best.
 * <p>
 * We don't need to set lock in this class, because only one instance will be existed, and if existing more than one
 * instance, it means wrong HA config and need to be rightly configured.
 */
public class HybridLeaderElectionService implements LeaderElectionService, NodeCacheListener {
	...
	public void start(LeaderContender contender) throws Exception {
		...
		LOG.info("Grant leadership to contender {} with session ID {}.",
			leaderContender.getDescription(), leaderSessionID);

		leaderContender.grantLeadership(leaderSessionID);

		LOG.info("Starting HybridLeaderElectionService, contender={}, session ID={}.",
			contender, leaderSessionID);
	...
	public void stop() throws Exception {
		...
		LOG.info("Stopping HybridLeaderElectionService, contender={}.", leaderContender);
	}
	...
	/**
	 * Writes the current leader's address as well the given leader session ID to FileSystem and ZooKeeper.
	 * Writing to the FileSystem will block while writing to ZooKeeper will run asynchronously and if ZooKeeper
	 * catch error, it will retry until writing succeed.
	 */
	...
	private void writeLeaderInformationToFileSystem(LeaderInformation leaderInformation){
		...
		int maxRetry = configuration.getInteger(HighAvailabilityOptions.HA_FILESYSTEM_MAX_RETRY_ATTEMPTS);
		int retryInterval = configuration.getInteger(HighAvailabilityOptions.HA_FILESYSTEM_RETRY_WAIT);

		int numRetries = 0;
		while (numRetries < maxRetry) {
			try (FSDataOutputStream outputStream = fs.create(leaderPath, FileSystem.WriteMode.OVERWRITE);
				...
				numRetries += 1;
				LOG.error("write leader information to file system error, retry={}, maxRetry={}",
					numRetries, maxRetry, e);
				...
				if (numRetries < maxRetry) {
					...
					leaderContender.handleError(
						new FlinkException("Could not write leader address and leader session ID to FileSystem.", e));
				}
	...
	private void writeLeaderInformationToZooKeeper(LeaderInformation leaderInformation) {
		...
		while (running && !dataWrittenToZK) {
			try(ByteArrayOutputStream baos = new ByteArrayOutputStream();
				...
				dataWrittenToZK = ZooKeeperUtils.writeEphemeralNode(client, pathSuffix, baos.toByteArray());
			} catch (Throwable e) {
	...
	/**
	 * <p>Normally, node only changed once when the first time writing data to
	 * the ephemeral leader node, and this method do nothing for this case.
	 *
	 * <p>But there exists little chance that this session with ZooKeeper might
	 * be lost because of accidental network instability, thus the ephemeral
	 * leader node would be deleted. For this case, we must ensure rewrite the
	 * leader information to the leader node.
	 *
	 */
	@Override
