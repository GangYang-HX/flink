...
public class SchedulerImpl implements Scheduler {
    ...
    @Override
    public Collection<AllocatedSlot> restoreSlots(
            ExecutionGraph executionGraph,
            TaskManagerLocation taskManagerLocation,
            TaskManagerGateway taskManagerGateway,
            Collection<SlotOffer> initialOffers) {
		Collection<AllocatedSlot> restoredAllocatedSlots = new ArrayList<>();

		initialOffers.forEach(
			slotOffer -> {
				List<Execution> executionsToRestore =
					executionGraph
						.getExecutionsToRestore()
						.get(slotOffer.getAllocationId());

				if (CollectionUtil.isNullOrEmpty(executionsToRestore)) {
					executionGraph.completeReconciling(
						new FlinkRuntimeException("Executions to restore is empty when reconciling"));
				}

				executionsToRestore.forEach(
					execution -> {
						...
						final SlotSharingGroup sharingGroup =
							execution.getVertex().getJobVertex().getSlotSharingGroup();
						...
						if (!allocatedSlot.tryAssignPayload(logicalSlot)
						...
						restoredAllocatedSlots.add(allocatedSlot);
					});
			});

		return restoredAllocatedSlots;
	}
