...
import org.apache.flink.state.api.input.RawKeyedStateInputFormat;
import org.apache.flink.state.api.input.operator.KeyedStateReaderOperator;
import org.apache.flink.state.api.input.operator.RawKeyedStateReaderOperator;
...
public class ExistingSavepoint extends WritableSavepoint<ExistingSavepoint> {
	 ...
	 * @param uid The uid of the operator.
	 * @param function The {@link KeyedStateReaderFunction} that is called for each key in state.
	 * @param keyTypeSerializer The TypeSerializer of the key in state.
	 * @param outTypeInfo The type information of the output of the transform reader function.
	 * @param <K> The type of the key in state.
	 * @param <OUT> The output type of the transform function.
	 * @return A {@code DataSet} of objects read from keyed state.
	...
	public <K, OUT> DataSet<OUT> readKeyedState(
		...
		KeyedStateInputFormat<K, VoidNamespace, OUT> inputFormat = new KeyedStateInputFormat<>(
			...
			stateBackend,
			env.getConfiguration(),
			new KeyedStateReaderOperator<>(function, keyTypeSerializer));
	...
	public <K, OUT> DataSet<OUT> readKeyedState(
		...
		OperatorState operatorState = metadata.getOperatorState(operatorID);
		KeyedStateInputFormat<K, VoidNamespace, OUT> inputFormat = new KeyedStateInputFormat<>(
			operatorState,
			stateBackend,
			env.getConfiguration(),
	 ...
	 * Read keyed state from an operator in a {@code Savepoint}.
	 * @param operatorID The operatorID of the operator.
	 * @param function The {@link RawKeyedStateReaderFunction} that is called for each key in state.
	 * @param keyTypeSerializer The TypeSerializer of the key in state.
	 * @param outTypeInfo The type information of the output of the transform reader function.
	 * @param <K> The type of the key in state.
	 * @param <OUT> The output type of the transform function.
	 ...
	 * @throws IOException If the savepoint does not contain operator state with the given uid.
	 */
	public <K, OUT> DataSet<OUT> readKeyedStateWithoutDeserialization(
		OperatorID operatorID,
		RawKeyedStateReaderFunction function,
		TypeSerializer keyTypeSerializer,
		TypeInformation<OUT> outTypeInfo,
		int parallelism) throws IOException {

		OperatorState operatorState = metadata.getOperatorState(operatorID);
		RawKeyedStateInputFormat<K, VoidNamespace, OUT> inputFormat = new RawKeyedStateInputFormat<>(
			operatorState,
			stateBackend,
			env.getConfiguration(),
			new RawKeyedStateReaderOperator<>(function, keyTypeSerializer));

		return env.createInput(inputFormat, outTypeInfo).setParallelism(parallelism);
	}
