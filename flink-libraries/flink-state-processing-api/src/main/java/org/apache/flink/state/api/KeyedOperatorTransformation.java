...
public class KeyedOperatorTransformation<K, T> {
	 ...
	 */
	public BootstrapTransformation<T> transform(KeyedStateBootstrapFunction processFunction) {
	...
	public BootstrapTransformation<T> transform(SavepointWriterOperatorFactory factory) {
		...
		Preconditions.checkArgument((keyType != null && keySerializer == null)
				|| (keyType == null && keySerializer != null),
			"only one of keyType and keySerializer can be init!");
		if (keyType != null) {
			return new BootstrapTransformation<>(dataSet, operatorMaxParallelism, factory, keySelector, keyType);
		} else {
			return new BootstrapTransformation<>(dataSet, operatorMaxParallelism, factory, keySelector, keySerializer);
		}
	}
}
