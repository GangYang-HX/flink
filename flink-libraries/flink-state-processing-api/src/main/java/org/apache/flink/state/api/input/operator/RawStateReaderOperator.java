...
/**
 * Base class for executing functions that read raw keyed state.
 *
 ...
 */
@Internal
public abstract class RawStateReaderOperator<F extends Function, KEY, N, OUT> implements KeyContext, Serializable {
	...
	private transient InternalTimeServiceManager timerServiceManager;

	protected RawStateReaderOperator(F function, TypeInformation keyType, TypeSerializer<N> namespaceSerializer) {
	...
	protected RawStateReaderOperator(F function, TypeSerializer keyTypeSerializer, TypeSerializer<N> namespaceSerializer) {
		Preconditions.checkNotNull(function, "The user function must not be null");
	...
	public abstract void processElement(KEY deserializedKey, byte[] key, VoidNamespace namespace, String cf, byte[] value, Collector<OUT> out) throws Exception;

	public abstract Iterator<Tuple5<KEY, byte[], VoidNamespace, String, byte[]>> getKeysAndNamespacesAndColumnFamiliesAndValues(
		SavepointRuntimeContext ctx) throws Exception;

	public final void setup(
		ExecutionConfig executionConfig,
		KeyedStateBackend keyKeyedStateBackend,
		InternalTimeServiceManager timerServiceManager,
	...
	public final KeyedStateBackend getKeyedStateBackend() {
		return keyedStateBackend;
	}

	public final TypeInformation getKeyType() {
		return keyType;
	}
