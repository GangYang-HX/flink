...
public class SavepointMetadata {
	 ...
	 * @return Operator state for the given UID.
	 *
	 * @throws IOException If the savepoint does not contain operator state with the given operatorID.
	 */
	public OperatorState getOperatorState(OperatorID operatorID) throws IOException {
		...
		if (operatorState == null || operatorState.isNewStateTransformation()) {
			throw new IOException("Savepoint does not contain state with operatorID " + operatorID);
		}
	...
	public void addOperator(String uid, BootstrapTransformation<?> transformation) {
		...
		addOperator(id, transformation);
	}

	public void addOperator(OperatorID operatorID, BootstrapTransformation<?> transformation) {
		if (operatorStateIndex.containsKey(operatorID)) {
			throw new IllegalArgumentException("The savepoint already contains operatorID " + operatorID + ". All operatorID's must be unique");
		}

		operatorStateIndex.put(operatorID, OperatorStateSpec.newWithTransformation(new BootstrapTransformationWithID<>(operatorID, transformation)));
	}
