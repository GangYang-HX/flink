...
public class PbCodegenRowDeserializer implements PbCodegenDeserializer {
	...
	public String codegen(String returnInternalDataVarName, String pbGetStr)
		...
		String methodString = "Method[] " + fieldNamesVar + " =" + pbMessageVar + ".getClass().getDeclaredMethods()";
		...
		appender.appendSegment("for (Method name : " + fieldNamesVar + ") {" +
			"            " + fieldNameListVar + ".add(name.getName());" +
			"        }");
		int index = 0;
		for (String fieldName : rowType.getFieldNames()) {
			...
			if (ignoreNullRows(formatConfig, elementFd)) {
				String sb = "if(!" + pbMessageVar + ".has" + strongCamelFieldName + "()" + "){" +
					rowDataVar + ".setField(" + index + "," + "null" + ");" +
					"}else{";
				...
				appender.appendSegment(
					"if(!" + fieldNameListVar + ".contains(\"has" + strongCamelFieldName + "\")){");
				appender.appendLine("flag" + subUid + "= false");
				appender.appendSegment("} else {");
				String invokeString = "try{ flag" + subUid + " = !(Boolean)" + pbMessageVar +
					".getClass().getDeclaredMethod(\"has" + strongCamelFieldName + "\").invoke(" + pbMessageVar + ")";
				appender.appendLine(invokeString);
				appender.appendSegment(
					"} catch (NoSuchMethodException noSuchMethodException" + subUid +
						"){} catch (IllegalAccessException illegalAccessException" + subUid +
						"){} catch (InvocationTargetException invocationTargetException" + subUid +
						"){}");
				appender.appendSegment("}");
	...
	private boolean ignoreNullRows(PbFormatConfig formatConfig, FieldDescriptor elementFd) {
		return !formatConfig.isIgnoreNullRows() && FieldDescriptor.Type.MESSAGE.equals(elementFd.getType()) && !elementFd.isRepeated();
	}
