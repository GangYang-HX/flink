...
public class RetryKafkaWriter<IN> extends KafkaWriter<IN> {
    ...
    protected RetryKafkaWriter(
        ...
        if (partitioner instanceof RandomRetryPartitioner) {
            this.retryPartitioner = (RandomRetryPartitioner) partitioner;
        }
        if (this.retry) {
        ...
        if (Boolean.parseBoolean(
                userDefinedProps.getProperty(KafkaConnectorOptions.BLACKLIST_ENABLE.key()))) {
            initBlacklistPartitioner();
        }
    }

    private void initBlacklistPartitioner() {
        this.retryPartitioner.initBlacklistPartitioner(
                userDefinedProps, sinkInitContext.getSubtaskId(), runtimeContext);
    }
