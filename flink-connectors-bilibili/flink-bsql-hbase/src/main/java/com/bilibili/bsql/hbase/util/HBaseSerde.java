...
public class HBaseSerde {
	...
	private final Integer versionRowIdx;
	private final Integer versionColIdx;
	private final LogicalType versionDataType;

	public HBaseSerde(HBaseTableSchema hbaseSchema, final String nullStringLiteral, Boolean skipWAL,
					  Integer versionRowIdx, Integer versionColIdx, LogicalType versionDataType) {
		this.skipWAL = skipWAL;
		this.versionRowIdx = versionRowIdx;
		this.versionColIdx = versionColIdx;
		...
		for (int i = 0; i < fieldLength; i++) {
			if (i != rowkeyIndex) {
				...
				for (int q = 0; q < this.qualifiers[f].length; q++) {
					...
					if (versionColIdx != -1 && versionRowIdx == i && versionColIdx == q) {
						Long versionTs = 0L;
						if (versionDataType.getTypeRoot() == VARCHAR) {
							String versionString = familyRow.getString(versionColIdx).toString();
							versionTs = Long.valueOf(versionString);
						} else if (versionDataType.getTypeRoot() == INTEGER) {
							Integer versionString = familyRow.getInt(versionColIdx);
							versionTs = Long.valueOf(versionString);
						} else if (versionDataType.getTypeRoot() == BIGINT) {
							versionTs = familyRow.getLong(versionColIdx);
						}
						versionTs = versionTs >= 0? versionTs: 0;
						put.addColumn(familyKey, qualifier, versionTs, value);
					}else{
