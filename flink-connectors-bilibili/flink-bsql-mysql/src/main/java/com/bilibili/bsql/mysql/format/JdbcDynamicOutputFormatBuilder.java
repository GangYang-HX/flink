 ...
 *
 * copied from {@link org.apache.flink.connector.jdbc.table.JdbcDynamicOutputFormatBuilder}
 *
 * <p> Logics modified as follows:
 *     a) add {@link JdbcConnectionProvider} assignment entrypoint
 * 	   b) remove the append-only condition
 * </p>
 */
public class JdbcDynamicOutputFormatBuilder implements Serializable {
	...
	public JdbcDynamicOutputFormatBuilder setJdbcConnectionProvider(JdbcConnectionProvider jdbcConnectionProvider) {
	...
	public JdbcBatchingOutputFormat<RowData, ?, ?> build() {
		...
		if (dmlOptions.getKeyFields().isPresent() && dmlOptions.getKeyFields().get().length > 0) {
			...
			// append only query: throw execption
			throw new RuntimeException("primary key fields should be assigned when delete enabled!");
		}
	}

	public static JdbcBatchStatementExecutor<RowData> createKeyedRowExecutor(
			JdbcDialect dialect,
			int[] pkFields,
			LogicalType[] pkTypes,
			String sql,
	...
	public static Function<RowData, RowData> createRowKeyExtractor(LogicalType[] logicalTypes, int[] pkFields) {
	...
	public static JdbcBatchStatementExecutor<RowData> createSimpleRowDataExecutor(JdbcDialect dialect, String sql, LogicalType[] fieldTypes, RuntimeContext ctx, TypeInformation<RowData> rowDataTypeInfo) {
		final TypeSerializer<RowData> typeSerializer = rowDataTypeInfo.createSerializer(ctx.getExecutionConfig());
	 ...
	 */
	public static JdbcStatementBuilder<RowData> createRowDataJdbcStatementBuilder(JdbcDialect dialect, LogicalType[] types) {
