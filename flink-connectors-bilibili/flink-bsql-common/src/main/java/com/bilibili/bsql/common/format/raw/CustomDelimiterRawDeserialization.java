 ...
 *
 * @author zhouxiaogang
 * @version $Id: CustomDelimiterRawDeserialization.java, v 0.1 2021-03-18 19:56
zhouxiaogang Exp $$
 */
public class CustomDelimiterRawDeserialization extends AbstractDeserializationSchema<RowData> {
	...
	public CustomDelimiterRawDeserialization(TypeInformation<RowData> typeInfo, RowType rowType,
											 String delimiterKey) {
		...
		for (int i = 0; i < rowType.getFieldCount(); i++) {
			...
			switch (type.getTypeRoot()) {
				...
				case INTERVAL_DAY_TIME:
					toInternalConverters[i] = (segments, offset, sizeInBytes) ->
						LongParser.parseField(segments[0].getArray(), offset, sizeInBytes);
					toInternalDefault[i] = 0L;
					break;
				case TIMESTAMP_WITHOUT_TIME_ZONE:
					toInternalConverters[i] = (segments, offset, sizeInBytes) -> {
						BinaryStringData timeString = new BinaryStringData(segments, offset, sizeInBytes);
				...
				case BOOLEAN:
					toInternalConverters[i] = (segments, offset, sizeInBytes) ->
						"true".equalsIgnoreCase(new BinaryStringData(segments, offset, sizeInBytes).toString());
					toInternalDefault[i] = false;
					break;
				case INTEGER:
				case INTERVAL_YEAR_MONTH:
					toInternalConverters[i] = (segments, offset, sizeInBytes) ->
						IntParser.parseField(segments[0].getArray(), offset, sizeInBytes);
					toInternalDefault[i] = 0;
					break;
				case TINYINT:
					toInternalConverters[i] = (segments, offset, sizeInBytes) ->
						ByteParser.parseField(segments[0].getArray(), offset, sizeInBytes);
					toInternalDefault[i] = (byte) 0;
					break;
				case SMALLINT:
					toInternalConverters[i] = (segments, offset, sizeInBytes) ->
						ShortParser.parseField(segments[0].getArray(), offset, sizeInBytes);
				...
				case FLOAT:
					toInternalConverters[i] = (segments, offset, sizeInBytes) -> {
						...
						return FloatParser.parseField(segments[0].getArray(), offset, sizeInBytes);
					};
				...
				case DOUBLE:
					toInternalConverters[i] = (segments, offset, sizeInBytes) -> {
						...
						return DoubleParser.parseField(segments[0].getArray(), offset, sizeInBytes);
					};
				...
				case VARBINARY:
					toInternalConverters[i] = (segments, offset, sizeInBytes) -> {
						...
						System.arraycopy(segments[0].getArray(), offset, result, 0, sizeInBytes);
						return result;
					};
					toInternalDefault[i] = new byte[0];
					break;
				default:
					throw new UnsupportedOperationException(type.toString() + " is not supported in raw source format");
			}
		}
	}
	...
	public static TimestampData convertToTimestamp(String timeStampStr){
		if (StringUtils.isNumeric(timeStampStr)) {
			...
			if (timeStampStr.length() == 20) {
				...
				throw new RuntimeException(String.format("Can not format timestamp %s", timeStampStr));
			}
		} else {
