...
public class ProducerCache {
    ...
    private void openMetrcis(KafkaProducer<byte[], byte[]> producer, String brokers) {
        if (!Boolean.parseBoolean(properties.getProperty("flink.disable-metrics", "false"))) {
            Map<MetricName, ? extends Metric> metrics = producer.metrics();
            if (metrics == null) {
                LOG.warn("Producer implementation does not support metrics");
            } else {
                final MetricGroup producerMetricGroup =
                        metricGroup.addGroup("DiversionKafkaProducer", getSingleBroker(brokers));
                for (Map.Entry<MetricName, ? extends Metric> metric : metrics.entrySet()) {
                    producerMetricGroup.gauge(
