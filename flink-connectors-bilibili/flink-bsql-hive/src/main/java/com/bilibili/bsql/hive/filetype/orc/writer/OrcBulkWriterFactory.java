...
public class OrcBulkWriterFactory<T> implements BulkWriter.Factory<T> {
	private static final Logger LOG = LoggerFactory.getLogger(OrcBulkWriterFactory.class);
    ...
    private static final String ORC_ROWS_BETWEEN_MEMORY_CHECKS = "orc.rows.between.memory.checks";
	...
	public OrcBulkWriterFactory(Vectorizer<T> vectorizer, Properties writerProperties, Configuration configuration) {
		this(vectorizer, writerProperties, configuration, null);
	}

	public OrcBulkWriterFactory(Vectorizer<T> vectorizer, Properties writerProperties, Configuration configuration, Properties bufferProperties) {
		...
		if (bufferProperties != null) {
			this.bufferSize = bufferProperties.containsKey(BSQL_HIVE_BUFFER_SIZE.key())
				? (Integer) bufferProperties.get(BSQL_HIVE_BUFFER_SIZE.key())
				: VectorizedRowBatch.DEFAULT_SIZE;

			this.batchSize = bufferProperties.containsKey(BSQL_HIVE_BATCH_SIZE.key())
				? (Integer) bufferProperties.get(BSQL_HIVE_BATCH_SIZE.key())
				: VectorizedRowBatch.DEFAULT_SIZE;

			if (bufferProperties.containsKey(BSQL_ORC_ROWS_BETWEEN_MEMORY_CHECKS.key())) {
				LOG.info(ORC_ROWS_BETWEEN_MEMORY_CHECKS +": {}", bufferProperties.get(BSQL_ORC_ROWS_BETWEEN_MEMORY_CHECKS.key()).toString());
				confMap.put(ORC_ROWS_BETWEEN_MEMORY_CHECKS,bufferProperties.get(BSQL_ORC_ROWS_BETWEEN_MEMORY_CHECKS.key()).toString());
			}
		}
    ...
    public BulkWriter<T> create(FSDataOutputStream out) throws IOException {
		...
		return new OrcBulkWriter<>(vectorizer, new WriterImpl(null, FIXED_PATH, opts), bufferSize, batchSize);
	}
