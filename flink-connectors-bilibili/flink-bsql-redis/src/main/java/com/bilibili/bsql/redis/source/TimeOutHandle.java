...
public class TimeOutHandle {
	private final static Logger LOG = LoggerFactory.getLogger(TimeOutHandle.class);
	...
	public void scheduleHandle(TimeOutFuture timeOutFuture)  {
        if (!timeOutFuture.getFutureForValue().isDone() || timeOutFuture.getException() != null) {
			...
			LOG.info("redis query timeout. key :{}  retry:{}", timeOutFuture.getQueryKey().toString(), timeOutFuture.getRetry() + 1 );


			RedisFuture redisFuture = timeOutFuture.getKeyGenerator().queryRedis(timeOutFuture.getQueryKey());
			TimeOutFuture retryTimeOutFuture = new TimeOutFuture(
			...
			redisFuture.thenAccept((Object redisValue) -> {
				try {
					timeOutFuture.getRedisRowDataOutputer().handleRedisQuery(retryTimeOutFuture.getQueryKey(), retryTimeOutFuture.getResultFuture(), retryTimeOutFuture.getTime() - timeout, redisValue);
			...
			redisFuture.exceptionally(o -> handleException(o, retryTimeOutFuture, timeOutFuture.getQueryKey().toString()));
	...
	public Object handleException(Object o, TimeOutFuture timeOutFuture, String key) {
		if (o instanceof RedisCommandExecutionException) {
			...
			timeOutFuture.getResultFuture().completeExceptionally((RedisCommandExecutionException) o);
			...
			LOG.info("redis asyn return CompletionException, input-key: {}, will retry again!", key);
			...
			LOG.info("redis asyn return unhandle exception: {}, input-key: {}", o.getClass().getName(), key);
		}
		return o;
	}
	...
	public void addTimeOutHandle(TimeOutFuture timeOutFuture) {
		if (timeOutFuture.getRetry() >= maxRetry) {
			Throwable throwable = new RuntimeException(
				String.format("redis query error. key :%s  retry:%s",
					timeOutFuture.getQueryKey().toString(),
					timeOutFuture.getRetry())
			);
			timeOutFuture.getResultFuture().completeExceptionally(throwable);
			LOG.error("redis query error. key :{}  retry:{}", timeOutFuture.getQueryKey().toString(), timeOutFuture.getRetry());
			hasError = true;
			error = throwable;
			return;
		}

		ScheduledFuture<?> scheduledFuture = timerService.schedule(new Runnable() {
			@Override
			public void run() {
