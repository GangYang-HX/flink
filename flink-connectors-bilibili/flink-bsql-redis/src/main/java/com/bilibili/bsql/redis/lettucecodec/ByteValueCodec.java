...
/** @version ByteValueCodec.java */
public class ByteValueCodec
        implements RedisCodec<String, byte[]>, ToByteBufEncoder<String, byte[]> {
    ...
    private ByteBuffer encodeAndAllocateBuffer(String key) {
        if (key == null) {
            ...
            CharsetEncoder encoder = CharsetUtil.encoder(this.charset);
            ByteBuffer buffer =
                    ByteBuffer.allocate((int) (encoder.maxBytesPerChar() * (float) key.length()));
