...
public class PartitionPathUtils {
	...
	private static final Pattern PARTITION_WITHOUT_KEY_HOUR_PATTERN = Pattern.compile("(\\d{4}\\d{1,2}\\d{1,2})/(\\d{1,2})$");
	private static final Pattern PARTITION_WITHOUT_KEY_DAY_PATTERN = Pattern.compile("(\\d{4}\\d{1,2}\\d{1,2})$");
	...
	public static LinkedHashMap<String, String> extractPartitionSpecFromPathWithoutPartitionKey(List<String> partitionKeys, Path currPath) {
		LinkedHashMap<String, String> fullPartSpec = new LinkedHashMap<>();
		Pattern pattern = partitionKeys.size() == 1 ? PARTITION_WITHOUT_KEY_DAY_PATTERN : PARTITION_WITHOUT_KEY_HOUR_PATTERN;
		Matcher m = pattern.matcher(currPath.getPath());
		if (m.find()) {
			for (int i = 0; i < partitionKeys.size(); i++) {
				LOG.info("extractPartition match group:{}",m.group(i+1));
				fullPartSpec.put(partitionKeys.get(i), m.group(i+1));
			}
		}
		return fullPartSpec;
	}
