...
/** Report topic partition lag info to zookeeper. */
public class ZkReporter implements Watcher, Serializable {
    ...
    private void initZkClient(String zkHost, String blacklistPath) {
        try {
            ...
            LOG.info(
                    "create zk client for topics {} success, zkHost = {}, jobId = {}",
                    this.topics,
                    zkHost,
                    this.jobId);
        } catch (Exception e) {
            LOG.error(
                    "init zk client for topics {} error, zkHost = {}, jobId = {}",
                    this.topics,
                    zkHost,
                    this.jobId,
                    e);
    ...
    public void createZkNodeForTopic(String topic) throws Exception {
        if (null == zkClient.exists(this.blacklistPath + "/" + topic, this)) {
            ...
            LOG.info(
                    "create persistent node for topic {} in zk {}, jobId = {}",
                    topic,
                    this.zkHost,
                    this.jobId);
        }
    ...
    public void registerBlackListTp(String targetTopic, Map<Tuple2<String, Integer>, Long> tpLags) {
        ...
        try {
            List<String> registeredTp =
                    zkClient.getChildren(this.blacklistPath + "/" + targetTopic, this);
            // Filter out registered nodes
            List<String> upToRegisterTp =
                    blacklistTps.stream()
            ...
            if (0 == upToRegisterTp.size()) {
                LOG.info(
                        "high lagTps already registered, topic = {}, registeredTp = {}, registerBySelf = {}, {}'s blacklistTps = {}",
                        targetTopic,
                        registeredTp,
                        this.registerBySelf,
                        targetTopic,
            ...
            for (String tp : upToRegisterTp) {
                ...
                LOG.info("topic = {} add partition = {} to blacklist", targetTopic, tp);
                this.registerBySelf.getOrDefault(targetTopic, Lists.newArrayList()).add(tp);
            }
            LOG.info(
                    "register new blacklistTps success, topic = {}, upToRegisterTp = {}, registeredTp = {}, jobId = {}, current blacklistTps = {}",
                    targetTopic,
                    upToRegisterTp,
                    registeredTp,
                    this.jobId,
            ...
            LOG.error(
                    "register new blacklistTp failed, topic = {}, jobId = {}, current blacklistTps = {}",
                    targetTopic,
                    this.jobId,
                    blacklistTps,
                    e);
    ...
    private void deleteZkNode(String topic, List<String> blacklistTps) {
        ...
        List<String> tryToDeleteTpNode =
                this.registerBySelf.getOrDefault(topic, Collections.emptyList()).stream()
                        .filter(tp -> !blacklistTps.contains(tp))
                        .collect(Collectors.toList());
        if (tryToDeleteTpNode.size() > 0) {
            LOG.info(
                    "try to delete zk node topic = {}, tryToDeleteTpNode = {}",
                    topic,
                    tryToDeleteTpNode);
        }
        tryToDeleteTpNode.forEach(
                tp -> {
                    try {
                        this.zkClient.delete(blacklistPath + "/" + topic + "/" + tp, -1);
                        this.registerBySelf.getOrDefault(topic, Lists.newArrayList()).remove(tp);
                        LOG.info(
                                "delete tp Node success, topic = {}, deletedTp = {}, jobId = {}, registerBySelf = {}",
                                topic,
                                tp,
                                this.jobId,
                                this.registerBySelf);
                        ...
                        LOG.error(
                                "delete tp Node error, topic = {}, deletedTp = {}, jobId = {}, registerBySelf = {}",
                                topic,
                                tp,
                                this.jobId,
                                this.registerBySelf,
        ...
        if (tryToDeleteTpNode.size() > 0) {
            LOG.info(
                    "delete Zk node completed, tryToDeleteTpNode = {}, registerBySelf = {}, {}'s blacklistTps = {}",
                    tryToDeleteTpNode,
                    this.registerBySelf,
                    topic,
                    blacklistTps);
    ...
    protected List<String> calculateBlacklistTp(
            String targetTopic, Map<Tuple2<String, Integer>, Long> tpLags) {
        // filter targetTopic's lag data.
        Map<Tuple2<String, Integer>, Long> filterTp =
        ...
        double averageLag =
                filterTp.values().stream().mapToDouble(lag -> lag).average().getAsDouble();

        double median =
                getMedian(
                        filterTp.values().stream()
                                .map(lag -> Double.valueOf(String.valueOf(lag)))
                                .collect(Collectors.toList()));

        LOG.info(
                "filterTps = {}, averageLag = {}, median = {},lagThreshold = {}",
                filterTp,
                averageLag,
                median,
                lagThreshold);
        Map<Tuple2<String, Integer>, Long> kickTps =
                filterTp.entrySet().stream()
                        .filter(
                                tpLag ->
                                        tpLag.getValue() > Math.min(averageLag, median) * lagTimes
                                                && tpLag.getValue() > lagThreshold)
                        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

        List<String> kickTpList =
                kickTps.entrySet().stream()
                        .sorted(
                                Map.Entry.<Tuple2<String, Integer>, Long>comparingByValue()
                                        .reversed())
                        .map(kickTp -> kickTp.getKey().f1.toString())
                        .collect(Collectors.toList());

        int maxKickNum =
                this.kickThreshold == -1
                        ? Math.max((int) (filterTp.size() * 0.1), 1)
                        : this.kickThreshold;
        ...
        if (kickTpList.size() == 0) {
            LOG.info(
                    "no tp to kick, topic = {}, tpLags = {}, kickThreshold = {}",
                    targetTopic,
                    tpLags,
                    maxKickNum);
        } else {
            LOG.info(
                    "topic = {} 's kickTpList = {}, kickThreshold = {}",
                    targetTopic,
                    kickTpList,
                    maxKickNum);
        }
