...
import org.apache.flink.streaming.connectors.kafka.blacklist.ReportAggFunc;
...
public class FlinkKafkaConsumer010<T> extends FlinkKafkaConsumerBase<T> {
    ...
    private void updateAgg(){
        if (super.kafkaFetcher() == null
            || ((Kafka010Fetcher) super.kafkaFetcher()).getConsumerThread() == null
            || ((Kafka010Fetcher) super.kafkaFetcher()).getSubscribedPartitionStates() == null){
            return;
        }
	    Map<TopicPartition, Long> tpLags = ConsumerBlacklistTpUtils.monitorTpLags(
			((Kafka010Fetcher<T>) super.kafkaFetcher()).getConsumerThread().getKafkaConsumerMetrics(),
		    ((Kafka010Fetcher<T>) super.kafkaFetcher()).getSubscribedPartitionStates()
			    .stream()
			    .map(state-> new TopicPartition(state.getTopic(), state.getPartition())).collect(Collectors.toList()),
		...
		Properties inputProps = ConsumerBlacklistTpUtils.generateInputProps(
            this.properties,
            (super.kafkaFetcher())
                .getSubscribedPartitionStates()
                .stream()
                .map(tp-> tp.getKafkaTopicPartition().getTopic())
        ...
        try {
            ...
            LOG.error("subtask {} execute agg error", getRuntimeContext().getIndexOfThisSubtask(), e);
        }
    }

	@Override
	public void open(Configuration configuration) throws Exception {
		//make the client id only one in process
		assignConsumeId(properties, getRuntimeContext());
		LOG.info("open id :{}", properties.getProperty(ConsumerConfig.CLIENT_ID_CONFIG));
		super.open(configuration);
        if (Boolean.parseBoolean(this.properties.getProperty(ConsumerBlacklistTpUtils.BLACKLIST_ENABLE))
            && this.properties.containsKey(ConsumerBlacklistTpUtils.BLACKLIST_ZK_HOST)) {
            ...
            aggScheduler = new ScheduledThreadPoolExecutor(1,
                new ThreadFactoryBuilder().setNameFormat("KafkaConsumer010-agg-scheduler-" + getRuntimeContext().getIndexOfThisSubtask()).build());
            ...
            ConsumerBlacklistTpUtils.printProps(this.properties);
        }
	}
}
