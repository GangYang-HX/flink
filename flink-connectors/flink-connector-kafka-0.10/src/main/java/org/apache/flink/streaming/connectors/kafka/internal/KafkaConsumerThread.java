...
public class KafkaConsumerThread<T> extends Thread {
	...
	/** kafka metrics wrapper */
	private KafkaMetricWrapper kafkaMetricWrapper;

	private KafkaMetricConfig kafkaMetricConfig;
	...
	public KafkaConsumerThread(
		...
		this.kafkaMetricConfig = new KafkaMetricConfig();

		this.consumerReassignmentLock = new Object();
		this.nextOffsetsToCommit = new AtomicReference<>();
		this.running = true;
		this.kafkaMetricWrapper = new KafkaMetricWrapper()
			.setCompactionTime(this.consumerMetricGroup.histogram(KafkaConsumerMetricConstants.POLLS_TIME_METRICS_GROUP,
				new DescriptiveStatisticsHistogram(KafkaMetricWrapper.DEFAULT_WINDOW_SIZE)))
			.setPollIdleRatio(this.consumerMetricGroup.histogram(KafkaConsumerMetricConstants.POLLS_IDLE_RATIO_METRICS_GROUP,
				new DescriptiveStatisticsHistogram(KafkaMetricWrapper.DEFAULT_WINDOW_SIZE)));
		if (rateLimiter != null) {
	...
	public void run() {
		...
		try {
			kafkaProperties.setProperty(ConsumerConfig.CLIENT_ID_CONFIG,
				kafkaProperties.getProperty(ConsumerConfig.CLIENT_ID_CONFIG) + "_"
					+ System.nanoTime());
			...
			if (useMetrics) {
				...
				if (metrics == null) {
					...
					for (Map.Entry<MetricName, ? extends Metric> metric: metrics.entrySet()) {
						...
						if (kafkaMetricConfig.contains(name)) {
							consumerMetricGroup.gauge(name, new KafkaMetricWrapper(metric.getValue()));

							// TODO this metric is kept for compatibility purposes; should remove in the future
							subtaskMetricGroup.gauge(name, new KafkaMetricWrapper(metric.getValue()));
						}
					}
				}
			}
    ...
    public Map<MetricName, Metric> getKafkaConsumerMetrics() {
        ConcurrentHashMap<MetricName, Metric> metrics = new ConcurrentHashMap<>();
        if (null != this.consumer && null != this.consumer.metrics()) {
            metrics = new ConcurrentHashMap<MetricName, Metric>(this.consumer.metrics());
        }
        return metrics;
    }
	...
	protected ConsumerRecords<byte[], byte[]> getRecordsFromKafka() {
		...
		kafkaMetricWrapper.recordPollRatio(start);
		kafkaMetricWrapper.recordPollTimeMs(start, stop);
		kafkaMetricWrapper.pollsTimeUse(start, stop);
		if (rateLimiter != null) {
