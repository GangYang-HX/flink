...
import org.apache.flink.connectors.hoodie.HoodieUtils;
...
import org.apache.flink.table.catalog.hive.util.MaterializeViewUtils;
...
public class HiveCatalog extends AbstractCatalog implements MaterializeSupport {
    ...
    public static final String HOODIE_METAFOLDER_NAME = ".hoodie";
    ...
    private MaterializeViewUtils materializeViewUtils = null;
    ...
    public HiveCatalog(
        ...
        this(
                catalogName,
                defaultDatabase,
                createHiveConf(hiveConfDir, hadoopConfDir),
                hiveVersion,
                false,
                configuration);
    }
    ...
    protected HiveCatalog(
        ...
        try {
            fs = new Path("/department/bigdata/flink/materialize").getFileSystem(hiveConf);
        } catch (Exception e) {
            throw new RuntimeException("init fs error", e);
        }

        boolean materializeEnabled =
                RuntimeExecutionMode.BATCH.equals(configuration.get(ExecutionOptions.RUNTIME_MODE))
                        && configuration.get(
                                OptimizerConfigOptions.TABLE_OPTIMIZER_MATERIALIZATION_ENABLED);
        if (materializeEnabled) {
            materializeViewUtils = new MaterializeViewUtils(configuration);
        }
    ...
    static HiveConf createHiveConf(@Nullable String hiveConfDir, @Nullable String hadoopConfDir) {
        ...
        // user doesn't provide hive conf dir, we first try to find it in HADOOP_CONF_DIR
        if (hiveConfDir == null) {
    ...
    public CatalogBaseTable getTable(ObjectPath tablePath)
        ...
        LOG.info("get table from hive catalog, objectPath: {}", tablePath);
    ...
    CatalogBaseTable instantiateCatalogTable(Table hiveTable) {
        ...
        if (HoodieUtils.isHoodieTable(inputFormat)) {
            Properties props =
                    fetchConfigs(
                            fs,
                            hiveTable.getSd().getLocation()
                                    + File.separator
                                    + HOODIE_METAFOLDER_NAME);
            properties.put(CONNECTOR.key(), HoodieTableFactory.FACTORY_ID);
            properties.put(FlinkOptions.PATH.key(), location);
            properties.put(FlinkOptions.TABLE_TYPE.key(), HoodieUtils.getTableType(hiveTable));
            properties.put(
                    FlinkOptions.RECORD_KEY_FIELD.key(),
                    props.getProperty(HoodieTableConfig.RECORDKEY_FIELDS.key()));
            if (partitionKeys.size() > 0) {
                properties.put(
                        FlinkOptions.PARTITION_PATH_FIELD.key(), String.join(",", partitionKeys));
            }
            properties.put(FlinkOptions.HIVE_STYLE_PARTITIONING.key(), "true");
            if (materializeViewUtils != null) {
                Optional<Integer> readTasks =
                        materializeViewUtils
                                .getConfiguration()
                                .getOptional(FlinkOptions.READ_TASKS);
                if (readTasks.isPresent()) {
                    properties.put(FlinkOptions.READ_TASKS.key(), String.valueOf(readTasks.get()));
                }

                Optional<Integer> readMaxTasks =
                        materializeViewUtils
                                .getConfiguration()
                                .getOptional(FlinkOptions.READ_MAX_TASKS);
                if (readMaxTasks.isPresent()) {
                    properties.put(
                            FlinkOptions.READ_MAX_TASKS.key(), String.valueOf(readMaxTasks.get()));
                }
            }
            properties.put(FlinkOptions.READ_COW_CREATE_SPLIT_ASYNC_MIN_PARALLELISM.key(), "48");
            properties.put(FlinkOptions.READ_COW_CREATE_SPLIT_ASYNC_MAX_PARALLELISM.key(), "48");

            LOG.info("hoodie table: {}", properties.toString());
        }
    ...
    @Override
    public List<Tuple3<ObjectIdentifier, String, Long>> listAllMaterializeViews(Long flagTime) {
        try {
            List<Tuple3<String, String, Long>> allMVs =
                    materializeViewUtils.listAllMVsFromMySQL(flagTime);
            List<Tuple3<ObjectIdentifier, String, Long>> ret = new ArrayList<>(allMVs.size());
            for (Tuple3<String, String, Long> tuple3 : allMVs) {
                String mvSinkTableName = tuple3.f0;
                ObjectPath objectPath = ObjectPath.fromString(mvSinkTableName);
                ObjectIdentifier id =
                        ObjectIdentifier.of(
                                getName(),
                ...
                String mvSql = tuple3.f1;
                LOG.info("mv table: {}, ctime: {}, mvSql: '{}'", mvSinkTableName, tuple3.f2, mvSql);
                ret.add(new Tuple3<>(id, mvSql, tuple3.f2));
            }
            return ret;
        } catch (Exception e) {
    ...
    public Optional<String> getMaterializeViewWatermark(String fullTableName) {
        LOG.info("start get mv table watermark.");
        try {
            Table hiveTable =
                    getHiveTable(
                            new ObjectPath(
                                    fullTableName.split("\\.")[0], fullTableName.split("\\.")[1]));
            HoodieTableMetaClient metaClient =
                    HoodieTableMetaClient.builder()
                            .setConf(hiveConf)
                            .setBasePath(hiveTable.getSd().getLocation())
                            .build();
            HoodieTimeline timeline = metaClient.getCommitsTimeline();
            AtomicReference<Optional<String>> watermark = new AtomicReference<>();
            timeline.getInstants()
                            ...
                            i -> {
                                try {
                                    return HoodieCommitMetadata.fromBytes(
                                                    timeline.getInstantDetails(i).get(),
                    ...
                    .filter(m -> m.containsKey(ExtraMetadataKey.WATERMARK.value()))
                    .reduce((first, second) -> second)
                    .ifPresent(
                            w ->
                                    watermark.set(
                                            Optional.of(
                                                    w.get(ExtraMetadataKey.WATERMARK.value()))));
            LOG.info(
                    "hoodie table {} watermark: {}",
                    fullTableName,
                    watermark.get().isPresent() ? watermark.get() : "null");
            return watermark.get();
        } catch (TableNotExistException e) {
            LOG.error("get watermark of table {} failed", fullTableName, e);
        }
        return Optional.empty();
    }

    /** fetch hoodie table props from hoodie.properties. */
    private Properties fetchConfigs(FileSystem fs, String metaPath) {
        Path cfgPath = new Path(metaPath, HOODIE_PROPERTIES_FILE);
        Properties props = new Properties();
        try (FSDataInputStream is = fs.open(cfgPath)) {
            props.load(is);
        } catch (IOException ioe) {
            LOG.error("Failed to get properties from hoodie.properties: {}", cfgPath);
            throw new CatalogException(
                    "Failed to get properties from hoodie.properties: " + cfgPath);
        }
        return props;
    }
