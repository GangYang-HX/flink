...
import org.apache.flink.bilibili.catalog.util.HiveTableUtil;
...
//		keeperRpc.open();
	...
	public CatalogBaseTable getTable(ObjectPath tablePath) throws CatalogException {
		...
		try {
			...
			TableDto tableDto = keeperRpc.getTable(currentDataSource, tablePath.getDatabaseName(), tablePath.getObjectName());
	...
	public boolean tableExists(ObjectPath tablePath) throws CatalogException {
		return keeperRpc.tableExist(currentDataSource, tablePath.getDatabaseName(), tablePath.getObjectName());
	}
	...
	private Map<String, String> constructTableProps(TableDto tableDto) {
		...
		switch (dbType) {
			case Hive:
				return HiveTableUtil.constructTablePropsMap(tableDto);
			case Kafka:
			...
			case Clickhouse:
				//获取数据源的写配置信息
				DatasourceConfig clickhouseConfigs = keeperRpc.getDsConfigsByDsIdAndUser(tableDto.getDatabase().getDsId(), DbType.Clickhouse, DsOpTypeEnum.WRITE);
			...
			case TiDB:
				//获取数据源的写配置信息
				DatasourceConfig mysqlConfigs = keeperRpc.getDsConfigsByDsIdAndUser(tableDto.getDatabase().getDsId(), dbType, DsOpTypeEnum.WRITE);
