...
public class BilibiliCatalog extends AbstractBilibiliCatalog {
	...
	public CatalogBaseTable getTable(ObjectPath tablePath) throws CatalogException {
		try {
            ...
            return instantiateCatalogTable(tableDto);
		} catch (Exception e) {
			extractKeeperEx(e);
    ...
    CatalogBaseTable instantiateCatalogTable(TableDto tableDto) {
        if (HoodieUtils.isHoodieTable(tableDto)) {
            return instantiateHudiCatalogTable(tableDto);
        } else {
            return instantiateCommonCatalogTable(tableDto);
        }
    }

    private CatalogBaseTable instantiateHudiCatalogTable(TableDto tableDto) {
        TableSchema.Builder builder = TableSchema.builder();
        List<ColumnDto> columnDtos = HoodieUtils.schemaFilter(tableDto);
        if (CollectionUtils.isNotEmpty(columnDtos)) {
            for (ColumnDto columnDto : columnDtos) {
                DataType dataType;
                if (columnDto.getIsPrimary()) {
                    dataType = convertDataType(
                            columnDto,
                            tableDto.getDatabase().getDbType()).notNull();
                } else {
                    dataType = convertDataType(
                            columnDto,
                            tableDto.getDatabase().getDbType());
                }
                TableColumn tableColumn = TableColumn.of(columnDto.getColName(),
                        dataType);
                builder.add(tableColumn);
            }
        }
        String[] primaryKeyList = CatalogUtil.getPrimaryKeyArray(tableDto);
        /*根据keeper返回的业务主键作为primary key，可能为空，之后在hudi factory会检查用户是否通过配置set进去*/
        if (primaryKeyList.length > 0) {
            builder.primaryKey(primaryKeyList);
        }
        //连接属性
        Map<String, String> props = constructTableProps(tableDto);
        List<String> partitionKeyList = CatalogUtil.getHivePartitionKeyList(tableDto);
        if (CollectionUtils.isNotEmpty(partitionKeyList)) {
            return new CatalogTableImpl(builder.build(), partitionKeyList, props, "");
        } else {
	...
	private Map<String, String> constructTableProps(TableDto tableDto) {
		...
		switch (dbType) {
			case Hive:
                return CatalogUtil.constructTablePropsMap(tableDto);
			case Kafka:
