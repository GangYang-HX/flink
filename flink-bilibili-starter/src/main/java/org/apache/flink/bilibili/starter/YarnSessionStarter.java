...
//		flinkConfFile = new File(args[1]);
        ...
        LOG.info("YarnSessionStarter context: {}", context);
        ...
        LOG.info("YarnSessionStarter properties: {}", properties);
    ...
    private static void handleUnSupportConfigFileSettings(StreamTableEnvironment tabEnv, Properties properties, JsonNode jobContext, String extraConfig) throws JsonProcessingException {
		...
		Map<String, String> jobParamMap = tabEnv.getConfig().getConfiguration().get(PipelineOptions.GLOBAL_JOB_PARAMETERS);
		if (Objects.isNull(jobParamMap)) {
			jobParamMap = new HashMap<>();
		}
		//extraConfig
		if (StringUtils.isNotBlank(extraConfig)) {
			jobParamMap.put(ExcaliburConfigOptions.JOB_CUSTOM_EXTRA_CONFIG.key(), extraConfig);
		}
		//job id
		if (Objects.nonNull(jobContext.get(CUSTOM_JOB_ID))) {
			jobParamMap.put(ExecutionOptions.CUSTOM_CALLER_CONTEXT_JOB_ID.key(), jobContext.get(CUSTOM_JOB_ID).asText());
		}
		tabEnv.getConfig().getConfiguration().set(PipelineOptions.GLOBAL_JOB_PARAMETERS, jobParamMap);

        tabEnv.getConfig().setIdleStateRetentionTime(Time.hours(48), Time.hours(50));
