...
import static org.apache.flink.configuration.StateBackendOptions.ROCKSDB_INTERFACE_COMPRESS_VALUE_ENABLED;
import static org.apache.flink.contrib.streaming.state.RocksDBConfigurableOptions.*;
...
public class DefaultConfigurableOptionsFactory implements ConfigurableRocksDBOptionsFactory {
	...
	private static final Logger LOG = LoggerFactory.getLogger(DefaultConfigurableOptionsFactory.class);
	...
	@Override
	public ColumnFamilyOptions createColumnOptionsWithStateName(ColumnFamilyOptions currentOptions,
														 Collection<AutoCloseable> handlesToClose,
														 String ColumnName) {
//		LOG.info("create option, column family is {}", ColumnName);
		return createColumnOptions(currentOptions, handlesToClose);
	}
	...
	public ColumnFamilyOptions createColumnOptions(ColumnFamilyOptions currentOptions, Collection<AutoCloseable> handlesToClose) {
		...
		if (isOptionConfigured(COMPRESSION_TYPE)) {
			if (Objects.equals(configuredOptions.get(ROCKSDB_INTERFACE_COMPRESS_VALUE_ENABLED.key()), "true")) {
				currentOptions.setCompressionType(CompressionType.getCompressionType(null));
			}
			else {
		...
		if (isOptionConfigured(BOTTOMMOST_COMPRESSION_TYPE)) {
			if (Objects.equals(configuredOptions.get(ROCKSDB_INTERFACE_COMPRESS_VALUE_ENABLED.key()), "true")) {
				currentOptions.setBottommostCompressionType(CompressionType.getCompressionType(null));
			}
			else {
				currentOptions.setBottommostCompressionType(getBottommostCompressionType());
			}
        }
		...
		if (isOptionConfigured(BLOOM_FILTER_LENGTH)) {
			int bloomFilterLength = getBloomFilterLength();
			if (bloomFilterLength > 0) {
				final BloomFilter bloomFilter = new BloomFilter(bloomFilterLength);
    ...
    // --------------------------------------------------------------------------
    // The compression type for cf.
    // --------------------------------------------------------------------------
    private CompressionType getCompressionType() {
        return CompressionType.getCompressionType(getInternal(COMPRESSION_TYPE.key()));
    }

    public DefaultConfigurableOptionsFactory setCompressionType(String compressionType) {
        setInternal(COMPRESSION_TYPE.key(), compressionType);
        return this;
    }

    // --------------------------------------------------------------------------
    // The bottommost compression type for cf.
    // --------------------------------------------------------------------------
    private CompressionType getBottommostCompressionType() {
        return CompressionType.getCompressionType(getInternal(BOTTOMMOST_COMPRESSION_TYPE.key()));
    }

    public DefaultConfigurableOptionsFactory setBottommostCompressionType(String compressionType) {
        setInternal(BOTTOMMOST_COMPRESSION_TYPE.key(), compressionType);
        return this;
    }
	...
	//--------------------------------------------------------------------------
	// Whether to use a Bloom filter for the rocksdb
	//--------------------------------------------------------------------------

	private int getBloomFilterLength() {
		return Integer.parseInt(getInternal(BLOOM_FILTER_LENGTH.key()));
	}

	public DefaultConfigurableOptionsFactory setBloomFilterLength(int value) {
		configuredOptions.put(BLOOM_FILTER_LENGTH.key(), String.valueOf(value));
		return this;
	}
	...
	private long getFIFOTableSize() {
		return MemorySize.parseBytes(getInternal(FIFO_TTL_SIZE.key()));
	}

	public DefaultConfigurableOptionsFactory setFIFOTableSize(long value) {
		configuredOptions.put(FIFO_TTL_SIZE.key(), String.valueOf(value));
		return this;
	}
	...
	private static final ConfigOption<?>[] CANDIDATE_CONFIGS = new ConfigOption<?>[] {
        ...
        METADATA_BLOCK_SIZE,
		BLOCK_CACHE_SIZE,
		ROCKSDB_INTERFACE_COMPRESS_VALUE_ENABLED
	};
