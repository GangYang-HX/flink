...
class RocksDBMapState<K, N, UK, UV>
	...
	public UV get(UK userKey) throws IOException, RocksDBException {
		...
		takeReadMetrics(start, rawKeyBytes, rawValueBytes);
		rawValueBytes = decompress(rawValueBytes);
	...
	public boolean isEmpty() {
		...
		try (RocksIteratorWrapper iterator = RocksDBOperationUtils.getRocksIterator(
				backend.db,
				columnFamily,
			...
			takeSeekMetrics(start, prefixBytes, iterator);
	...
	public void clear() {
		try {
			...
			try (RocksIteratorWrapper iterator = RocksDBOperationUtils.getRocksIterator(
				...
				long seekStart = 0;
				if (catchSample) {
					seekStart = System.nanoTime();
				}
				iterator.seek(keyPrefixBytes);
				takeSeekMetrics(seekStart, keyPrefixBytes, iterator);
				...
				while (iterator.isValid()) {
					...
					long nextStart = 0;
					if (catchSample) {
						nextStart = System.nanoTime();
					}
					iterator.next();
					takeNextMetrics(nextStart, iterator);
				}
			}
		} catch (Exception e) {
	...
	private class RocksDBMapEntry implements Map.Entry<UK, UV> {
		...
		public UV setValue(UV value) {
			...
			try {
				...
				db.put(columnFamily, writeOptions, rawKeyBytes, compressedValue);
				takeWriteMetrics(start, rawKeyBytes, compressedValue);
	...
	private abstract class RocksDBMapIterator<T> implements Iterator<T> {
		...
		private void loadCache() {
			...
			try (RocksIteratorWrapper iterator = RocksDBOperationUtils.getRocksIterator(
				...
				takeSeekMetrics(seekStart, startBytes, iterator);
				...
				if (currentEntry != null && !currentEntry.deleted) {
					...
					iterator.next();
					takeNextMetrics(firstStart, iterator);
				...
				while (true) {
					...
					byte[] keyRawBytes = iterator.key();
					byte[] valRawBytes = iterator.value();
					valRawBytes = decompress(valRawBytes);
					...
					iterator.next();
					takeNextMetrics(secondStart, iterator);
				}
			}
		}
	}
