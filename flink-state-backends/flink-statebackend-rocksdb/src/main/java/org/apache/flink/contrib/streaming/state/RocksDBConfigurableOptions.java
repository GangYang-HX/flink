...
public class RocksDBConfigurableOptions implements Serializable {
    ...
    public static final ConfigOption<String> COMPRESSION_TYPE =
            key("state.backend.rocksdb.compression.type")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(Description.builder().text("rocksdb compression type, snappy,lz4 etc")
                            .build());

    public static final ConfigOption<String> BOTTOMMOST_COMPRESSION_TYPE =
            key("state.backend.rocksdb.bottommost.compression.type")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(Description.builder().text("rocksdb bottommost compression type, snappy,lz4 etc")
                            .build());
	...
	public static final ConfigOption<Integer> BLOOM_FILTER_LENGTH =
		key("state.backend.rocksdb.bloom-filter.length")
		.intType()
		.defaultValue(0)
		.withDescription("The length of the rocksdb bloom filter, 0 means no filter is enabled");

	public static final ConfigOption<MemorySize> FIFO_TTL_SIZE =
		key("state.backend.rocksdb.fifo-ttl-size")
		.memoryType()
		.defaultValue(new MemorySize(8 * 1024L * 1024L * 1024L))
		.withDescription("The rocksdb size after which will delete obsolete sst files");
}
