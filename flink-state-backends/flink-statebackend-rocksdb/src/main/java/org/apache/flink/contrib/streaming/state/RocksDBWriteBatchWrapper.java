...
public class RocksDBWriteBatchWrapper implements AutoCloseable {
	...
	private void flushIfNeeded() throws RocksDBException {
		...
		if (needFlush) {
			...
			flush();
			if (null != writeHist) {
				long perDataRt = (System.nanoTime() - start) / getDataSize();
				writeHist.update(perDataRt);
			}
		}
	}
