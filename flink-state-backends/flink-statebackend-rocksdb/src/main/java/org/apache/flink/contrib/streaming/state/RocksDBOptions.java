...
public class RocksDBOptions {
	...
	public static final ConfigOption<String> DISTRIBUTE_MODE = ConfigOptions
		.key("state.backend.rocksdb.distribute")
		.defaultValue("yarn")
		.withDescription("three mode: config(use configurate), yarn(can spread over all yarn)" +
	...
	@Documentation.Section(Documentation.Sections.EXPERT_ROCKSDB)
	public static final ConfigOption<Boolean> CHECKPOINT_INCREMENT_STATE_UPLOAD_SPECULATED_ENABLED = ConfigOptions
		.key("state.backend.rocksdb.speculative-state-uploading.enabled")
		.booleanType()
		.defaultValue(false)
		.withDescription("Whether to upload incremental state using speculative execution.");

	@Documentation.Section(Documentation.Sections.EXPERT_ROCKSDB)
	public static final ConfigOption<Boolean> CHECKPOINT_STATE_DOWNLOAD_SPECULATED_ENABLED = ConfigOptions
		.key("state.backend.rocksdb.speculative-state-downloading.enabled")
		.booleanType()
		.defaultValue(false)
		.withDescription("Whether to download state using speculative execution.");

	@Documentation.Section(Documentation.Sections.EXPERT_ROCKSDB)
	public static final ConfigOption<Boolean> CHECKPOINT_TRANSFER_DOWNLOAD_TIMEOUT_ENABLE = ConfigOptions
		.key("state.backend.rocksdb.checkpoint.transfer.download-timeout.enabled")
		.booleanType()
		.defaultValue(false)
		.withDescription("Whether to open the timeout mechanism.");

	@Documentation.Section(Documentation.Sections.EXPERT_ROCKSDB)
	public static final ConfigOption<Long> CHECKPOINT_TRANSFER_DOWNLOAD_TIMEOUT_MS = ConfigOptions
		.key("state.backend.rocksdb.checkpoint.transfer.download-timeout.ms")
		.longType()
		.defaultValue(300_000L)
		.withDescription("The time used to control download files threshold in RocksDBStateBackend.");
