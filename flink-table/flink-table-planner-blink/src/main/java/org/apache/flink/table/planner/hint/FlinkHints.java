...
public abstract class FlinkHints {
	...
	/**
	 * options for latency join.
	 */
	public static final String LATENCY_JOIN_OPTIONS = "LATENCY_JOIN_OPTIONS";
	public static final String LATENCY_JOIN_TYPE = "LATENCY_JOIN_TYPE";
	...
	/** Returns the LATENCY_JOIN_OPTIONS hint options from the
	 * given list of table hints {@code tableHints}, never null. */
	@Nonnull
	public static Map<String, String> getHintedJoinOptions(List<RelHint> tableHints) {
		return tableHints.stream().filter(hint ->
				hint.hintName.equalsIgnoreCase(LATENCY_JOIN_OPTIONS))
				.findFirst()
				.map(hint -> hint.kvOptions)
				.orElse(Collections.emptyMap());
	}

	/** Returns the LATENCY_JOIN_TYPE hint options from the
	 * given list of table hints {@code tableHints}, never null. */
	@Nonnull
	public static Map<String, String> getHintedJoinType(List<RelHint> tableHints) {
		return tableHints.stream().filter(hint ->
				hint.hintName.equalsIgnoreCase(LATENCY_JOIN_TYPE))
				.findFirst()
				.map(hint -> hint.kvOptions)
				.orElse(Collections.emptyMap());
	}
