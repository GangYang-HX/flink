...
public class PushProjectIntoTableSourceScanRule extends RelOptRule {
	...
	public void onMatch(RelOptRuleCall call) {
		...
		TableSchema schema = oldTableSourceTable.catalogTable().getSchema();
		List<TableColumn> tableColumns = schema.getTableColumns();
		List<String> fieldNames = new ArrayList<>();
		for (int i = 0; i < usedFields.length; ++i) {
			int usedField = usedFields[i];
			String fileName = scan.getRowType().getFieldNames().get(usedField);
			int physicalUsedField = tableColumns.indexOf(schema.getTableColumn(fileName).get());
			projectedFields[i] = new int[] { physicalUsedField };
