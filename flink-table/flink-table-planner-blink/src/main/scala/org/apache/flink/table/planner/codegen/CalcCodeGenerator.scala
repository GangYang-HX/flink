...
import org.apache.flink.table.runtime.generated.{CompileUtils, GeneratedFunction}
...
object CalcCodeGenerator extends Logging {
  ...
  private[flink] def generateCalcOperator(
    ...
    LOG.debug(s"Compiling CalcCodeGenerator Code:\n${genOperator.getCode}")

    if (ctx.preCompileCalcCodeGenEnable) {
      CompileUtils.compile(Thread.currentThread().getContextClassLoader, genOperator.getClassName, genOperator.getCode)
    }
