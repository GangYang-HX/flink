...
import org.apache.flink.table.connector.source.{AsyncTableFunctionProvider, AsyncTableFunctionProviderWithParallel, LookupTableSource, TableFunctionProvider}
...
import org.apache.flink.table.planner.plan.utils.{JoinTypeUtil, KeySelectorUtil, RelExplainUtil}
import org.apache.flink.table.planner.utils.TableConfigUtils.getMillisecondFromConfigDuration
import org.apache.flink.table.runtime.connector.source.LookupRuntimeProviderContext
import org.apache.flink.table.runtime.operators.join.lookup.{LookupJoinRunner, LookupJoinWithCalcRunner}
...
import org.apache.flink.streaming.api.operators.async.keyed.KeyedAsyncWaitOperatorFactory
import org.apache.flink.table.runtime.operators.join.lookup.hash.{KeyedAsyncLookupJoinRunner, KeyedAsyncLookupJoinWithCalcRunner}
 ...
 */
abstract class CommonKeyedLookupJoin(
    ...
    temporalTable match {
      case t: TableSourceTable =>
        ...
        provider match {
          ...
          case atfp: AsyncTableFunctionProviderWithParallel[_] => {
            atfp.createAsyncTableFunction()
          }
        }
  ...
  def translateToPlanInternal(
    ...
    val operatorFactory = if (isAsyncEnabled) {
      ...
      val retryBackoff= config.getConfiguration
        .getInteger(ExecutionConfigOptions.TABLE_EXEC_ASYNC_LOOKUP_RETRY_BACKOFF)
      val maxRetry = config.getConfiguration.getInteger(
        ExecutionConfigOptions.TABLE_EXEC_ASYNC_LOOKUP_RETRY_TIMES)
      ...
      val asyncFunc = if (calcOnTemporalTable.isDefined) {
        ...
        new KeyedAsyncLookupJoinWithCalcRunner(
          generatedFetcher,
          generatedCalc,
        ...
        val generatedResultFuture = LookupJoinCodeGenerator.generateTableAsyncCollector(
          ...
          rightRowType,
          remainingCondition)
        new KeyedAsyncLookupJoinRunner(
          generatedFetcher,
          generatedResultFuture,
          producedTypeInfo,
          RowDataTypeInfo.of(rightRowType),
      ...
      // bsql are all in unordered mode, so change here to the unorder mode
      new KeyedAsyncWaitOperatorFactory(asyncFunc, asyncTimeout, asyncBufferCapacity,retryBackoff,maxRetry,
    ...
    val lookupParallel:Int = temporalTable match {
      case t: TableSourceTable =>
        val indices = lookupKeyIndicesInOrder.map(Array(_))
        val tableSource = t.tableSource.asInstanceOf[LookupTableSource]
        val providerContext = new LookupRuntimeProviderContext(indices)
        val provider = tableSource.getLookupRuntimeProvider(providerContext)
        provider match {
          case atfp: AsyncTableFunctionProviderWithParallel[_] => atfp.getConfiguredParallel()
          case _ => 0
        }
      case t: LegacyTableSourceTable[_] => 0
    }

    val parallel = if (lookupParallel > 0) {
      lookupParallel
    } else {
      inputTransformation.getParallelism
    }

    val transformation = ExecNode.createOneInputTransformation(
      inputTransformation,
    ...
    val leftType = RowDataTypeInfo.of(inputRowType)


    val selector = KeySelectorUtil.getRowDataSelector(joinInfo.leftKeys.toIntArray,leftType)
    transformation.setStateKeyType(leftType)
