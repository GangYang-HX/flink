...
import org.apache.flink.table.planner.dataview.RowTypeUtils
import org.apache.flink.table.planner.functions.sql.{FlinkSqlOperatorTable, SqlWindowTableFunction}
import org.apache.flink.table.planner.plan.`trait`.RelWindowProperties
import org.apache.flink.table.planner.plan.logical.{CumulativeWindowSpec, HoppingWindowSpec, TimeAttributeWindowingStrategy, TumblingWindowSpec, WindowingStrategy}
import org.apache.flink.table.planner.plan.metadata.FlinkRelMetadataQuery
import org.apache.flink.table.planner.plan.utils.AggregateUtil.inferAggAccumulatorNamesV2
...
import org.apache.flink.table.runtime.groupwindow.{NamedWindowProperty, ProctimeAttribute, RowtimeAttribute, WindowEnd, WindowStart}
...
object WindowUtil {
  ...
  def checkEmitConfiguration(tableConfig: TableConfig): Unit = {
    if (tableConfig.getConfiguration.get(TABLE_EXEC_EMIT_EARLY_FIRE_ENABLED) ||
      tableConfig.getConfiguration.get(TABLE_EXEC_EMIT_LATE_FIRE_ENABLED)) {
  ...
  def deriveLocalWindowAggregateRowType(
    ...
    val accFieldNames = inferAggAccumulatorNamesV2(aggInfoList)
