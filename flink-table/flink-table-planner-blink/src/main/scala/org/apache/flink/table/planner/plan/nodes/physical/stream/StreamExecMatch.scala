...
class StreamExecMatch(
  ...
  override protected def translateToPlanInternal(
    ...
    if (logicalMatch.allRows) {
      ...
      val transformation = new OneInputTransformation[RowData, RowData](timestampedInput, getRelDetailedDescription, getFlinkRelTypeName, operator, outputRowTypeInfo, timestampedInput.getParallelism)
  ...
  private def translateOrder(
    ...
    val timestampedInputTransform =
      if (FlinkTypeFactory.isRowtimeIndicatorType(timeOrderField.getType)) {
        ...
        val transformation = new OneInputTransformation(inputTransform, s"rowtime field: ($timeOrderField)", getFlinkRelTypeName, new ProcessOperator(new RowtimeProcessFunction(timeIdx, inputTypeInfo, precision)), inputTypeInfo, inputTransform.getParallelism)
        if (inputsContainSingleton()) {
          transformation.setParallelism(1)
          transformation.setMaxParallelism(1)
        }
