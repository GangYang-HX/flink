...
object CodeGenUtils {
  ...
  /**
   * externalResult(javaResult)'s conversion class
   */
  def externalResultConversionClass(dataType: DataType): Class[_] = {
    val clazz = dataType.getConversionClass
    clazz.getCanonicalName match {
      case "boolean" | "byte" | "short" | "int" | "long" | "float" | "double" => dataType.getLogicalType.getDefaultConversion
      case _ => dataType.getConversionClass
    }
  }
