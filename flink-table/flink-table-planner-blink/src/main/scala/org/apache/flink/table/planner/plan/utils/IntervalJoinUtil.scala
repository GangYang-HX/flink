...
object IntervalJoinUtil {
  ...
  private[flink] def extractLatencyJoinPeriodFromPredicate(
      predicate: RexNode,
      leftLogicalFieldCnt: Int,
      joinRowType: RelDataType,
      rexBuilder: RexBuilder,
      config: TableConfig): (Option[WindowBounds], Option[WindowBounds] , Option[RexNode]) = {
    ...
    timePreds match {
      case Seq(_, _, _, _) =>
      // 只接收四个条件的场景，一定是两个between
      case _ =>
        return (None, None, Some(predicate))
    }

    val timePredsLeft = timePreds.
      filter(a => a.leftInputOnLeftSide)
    val timePredsRight = timePreds.
      filter(a => !a.leftInputOnLeftSide)

    // assemble window bounds from predicates
    val streamTimeOffsetsLeft = timePredsLeft.
      map(computeWindowBoundFromPredicate(_, rexBuilder, config))
    val streamTimeOffsetsRight = timePredsRight.
      map(computeWindowBoundFromPredicate(_, rexBuilder, config))

    val (leftLowerBound, leftUpperBound) =
      streamTimeOffsetsLeft match {
        case Seq(Some(x: WindowBound), Some(y: WindowBound)) if x.isLeftLower && !y.isLeftLower =>
          // two range predicates
          (x.bound, y.bound)
        case Seq(Some(x: WindowBound), Some(y: WindowBound)) if !x.isLeftLower && y.isLeftLower =>
    ...
    val (rightLowerBound, rightUpperBound) =
      streamTimeOffsetsRight match {
        case Seq(Some(x: WindowBound), Some(y: WindowBound)) if x.isLeftLower && !y.isLeftLower =>
          // two range predicates
          (x.bound, y.bound)
        case Seq(Some(x: WindowBound), Some(y: WindowBound)) if !x.isLeftLower && y.isLeftLower =>
    ...
    val boundsLeft =
      Some(WindowBounds(
        timePredsLeft.head.isEventTime,
        leftLowerBound,
        leftUpperBound,
        timePredsLeft.head.leftTimeIdx,
        timePredsLeft.head.rightTimeIdx))
    val boundsRight =
      Some(WindowBounds(
        timePredsRight.head.isEventTime,
        rightLowerBound,
        rightUpperBound,
        timePredsRight.head.leftTimeIdx,
        timePredsRight.head.rightTimeIdx))

    (boundsLeft, boundsRight, remainCondition)
  }
