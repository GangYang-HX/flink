...
import org.apache.flink.table.connector.sink.{DynamicTableSink, OutputFormatProvider, SinkFunctionProvider, SinkFunctionProviderWithParallel}
...
class CommonPhysicalSink (
  ...
  protected def createSinkTransformation(
    ...
    val sinkParallel: Int = runtimeProvider match {
      case provider: SinkFunctionProviderWithParallel =>
        provider.getConfiguredParallel
      case _ => //
    ...
    val transformation = new SinkTransformation(inputTransformation, getRelDetailedDescription, getFlinkRelTypeName, SimpleOperatorFactory.of(operator), sinkParallel).asInstanceOf[Transformation[Any]]

    val slotGroup = catalogTable.getOptions.get(SlotGroupOptions.SLOT_SLOT.key())
    if(slotGroup != null){
