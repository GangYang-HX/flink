...
object ExecNode {
  ...
  def setManagedMemoryWeight[T](
    ...
    // Careful about zero
    if (memoryBytes > 0) {
      val weightInMebibyte = Math.max(1, (memoryBytes >> 20).toInt)
      val previousWeight = transformation.declareManagedMemoryUseCaseAtOperatorScope(ManagedMemoryUseCase.OPERATOR, weightInMebibyte)
      if (previousWeight.isPresent) throw new TableException("Managed memory weight has been set, this should not happen.")
    }
    transformation
  }
  ...
  def createOneInputTransformation[T](
    ...
    val ret = new OneInputTransformation[T, T](input, name, operatorType, operatorFactory, outputType, parallelism)
  ...
  def createTwoInputTransformation[T](
    ...
    val ret = new TwoInputTransformation[T, T, T](input1, input2, name, operatorType, operatorFactory, outputType, parallelism)
    setManagedMemoryWeight(ret, memoryBytes)
    ret
  }
}
