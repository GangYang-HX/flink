...
class ExtractCallGen(method: Method)
  ...
  override def generate(
    ...
    // FIXME: Clock.systemDefaultZone is the default time zone,
    //  otherwise the UTC time zone will be 8 hours slower
    val newClock = Clock.systemDefaultZone
    val defaultTimeZone = newClock.getZone.getRules
      .getOffset(newClock.instant()).getTotalSeconds * 1000L
            ...
            return generateCallIfArgsNotNull(ctx, returnType, operands) {
              (terms) =>
                s"""
                   ...
                   |    (${terms(1)}.getMillisecond() + $defaultTimeZone) /
    ...
    generateCallIfArgsNotNull(ctx, returnType, operands) {
      (terms) => {
        ...
        val longTerm = tpe.getTypeRoot match {
          case LogicalTypeRoot.TIMESTAMP_WITHOUT_TIME_ZONE =>
            s"(${terms(1)}.getMillisecond() + $defaultTimeZone)"
          case _ => s"${terms(1)}"
        }
        unit match {
