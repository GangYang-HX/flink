...
import org.apache.flink.configuration.{Configuration, OptimizerOptions}
...
object FlinkStreamProgram {
  ...
  def buildProgram(config: Configuration): FlinkChainedProgram[StreamOptimizeContext] = {
    ...
    val logicalProgram: FlinkVolcanoProgramBuilder[StreamOptimizeContext] =
      FlinkVolcanoProgramBuilder.newBuilder
        .add(FlinkStreamRuleSets.LOGICAL_OPT_RULES)
        .setRequiredOutputTraits(Array(FlinkConventions.LOGICAL))
    if (config.getBoolean(OptimizerOptions.SCAN_SOURCE_PROJECT_PUSH_DOWN)) {
      logicalProgram.add(FlinkStreamRuleSets.LOGICAL_PUSH_DOWN_EXTENSION)
    }
    chainedProgram.addLast(LOGICAL, logicalProgram.build())
    ...
    chainedProgram.addLast(
      ...
      FlinkVolcanoProgramBuilder.newBuilder
        ...
        .add(
          if (config.getBoolean(OptimizerConfigOptions.TABLE_OPTIMIZER_JOIN_LOOKUP_KEYED_ENABLED)) {
            FlinkStreamRuleSets.PHYSICAL_LOOKUP_JOIN_KEYED_RULES
          } else {
            FlinkStreamRuleSets.PHYSICAL_LOOKUP_JOIN_RULES
          })
