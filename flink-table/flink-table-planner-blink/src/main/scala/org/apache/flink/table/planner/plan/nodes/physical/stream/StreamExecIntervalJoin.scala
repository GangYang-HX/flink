...
import org.apache.flink.table.runtime.operators.join.interval.config.IntervalJoinConfig
...
class StreamExecIntervalJoin(
  ...
  private def createNegativeWindowSizeJoin(
    ...
    val padRightStream = new OneInputTransformation[RowData, RowData](rightPlan, "pad right input transformation", getFlinkRelTypeName+"PadRight", new StreamMap[RowData, RowData](rightPadder), returnTypeInfo, rightParallelism)
    flinkJoinType match {
      case FlinkJoinType.INNER =>
