  ...
  def generateWatermarkGenerator(
    ...
    val funcCode =
      ...
      public final class $funcName
        ...
        private org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger($funcName.class);
        ...
        public Long currentWatermark($ROW_DATA row) throws Exception {
          ...
          if (${generatedExpr.nullTerm}) {
            return null;
             // FIXME: The calculated watermark will not be less than the current time
          } else if (${generatedExpr.resultTerm}.getMillisecond() > System.currentTimeMillis()) {
            LOG.warn("watermark has been discarded,watermark is:{}.",${generatedExpr.resultTerm}.getMillisecond());
            return null;
          } else {
