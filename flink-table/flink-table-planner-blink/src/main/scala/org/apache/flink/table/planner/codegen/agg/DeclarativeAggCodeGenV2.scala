...
class DeclarativeAggCodeGenV2(
  ...
  def accumulate(generator: ExprCodeGenerator): String = {
    ...
    val resolvedExprs = function.accumulateExpressions
      .map(_.accept(ResolveReferenceV2(isDistinctMerge = isDistinctMerge)))
    ...
    val codes = exprs.zipWithIndex.map { case (expr, index) =>
      if (ctx.udfMetricOpened) {
        s"""|long start_${function}_$index = System.nanoTime();
            |${expr.code}
            |${expr.copyResultTermToTargetIfChanged(ctx, bufferTerms(index))};
            |${bufferNullTerms(index)} = ${expr.nullTerm};
            |long end_${function}_$index = System.nanoTime();
            |this.udfEvalHist_${function.toString}.update(end_${function}_$index - start_${function}_$index);
            |""".stripMargin
      } else {
        s"""|${expr.code}
            |${expr.copyResultTermToTargetIfChanged(ctx, bufferTerms(index))};
            |${bufferNullTerms(index)} = ${expr.nullTerm};""".stripMargin
      }
    }
  ...
  def merge(generator: ExprCodeGenerator): String = {
    val exprs = function.mergeExpressions
      .map(_.accept(ResolveReferenceV2(isMerge = true)))
      .map(_.accept(rexNodeGen)) // rex nodes
      .map(generator.generateExpression) // generated expressions
   ...
   */
  private case class ResolveReferenceV2(
