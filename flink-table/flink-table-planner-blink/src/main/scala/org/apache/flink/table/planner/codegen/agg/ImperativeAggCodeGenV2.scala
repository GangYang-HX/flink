...
import org.apache.flink.table.planner.plan.utils.AggregateInfoV2
 ...
 */
class ImperativeAggCodeGenV2(
  ctx: CodeGeneratorContext,
  aggInfo: AggregateInfoV2,
  ...
  val viewSpecs: Array[DataViewSpecV2] = aggInfo.viewSpecs
  ...
  def accumulate(generator: ExprCodeGenerator): String = {
    ...
    val call = if(ctx.udfMetricOpened) {
      s"""
         |long start_$functionTerm = System.nanoTime();
         |$functionTerm.accumulate($parameters);
         |long end_$functionTerm = System.nanoTime();
         |this.udfEvalHist_${function.toString}.update(end_$functionTerm - start_$functionTerm);
         |""".stripMargin
    }
    else {
      s"$functionTerm.accumulate($parameters);"
    }
