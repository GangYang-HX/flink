...
object ScalarOperatorGens {
  ...
  def generateCast(
    ...
    // Timestamp -> Date
    case (TIMESTAMP_WITHOUT_TIME_ZONE, DATE) =>
      // FIXME: Clock.systemDefaultZone is the default time zone, otherwise the UTC time zone will be 8 hours slower
      val newClock = Clock.systemDefaultZone
      val defaultTimeZone = newClock.getZone.getRules.getOffset(newClock.instant()).getTotalSeconds * 1000L
      val targetTypeTerm = primitiveTypeTermForType(targetType)
      generateUnaryOperatorIfNotNull(ctx, targetType, operand) {
        operandTerm =>
          s"""
             |($targetTypeTerm) (($operandTerm.getMillisecond() + $defaultTimeZone) /
    ...
    case (TIMESTAMP_WITHOUT_TIME_ZONE, TIME_WITHOUT_TIME_ZONE) =>
      ...
      generateUnaryOperatorIfNotNull(ctx, targetType, operand) {
        operandTerm =>
          s"($targetTypeTerm) (($operandTerm.getMillisecond() + $defaultTimeZone)% " +
