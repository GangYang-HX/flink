...
class BridgingSqlFunctionCallGen(call: RexCall) extends CallGenerator {
  ...
  private def generateFunctionAwareCall(
    ...
    if (function.getDefinition.getKind == FunctionKind.TABLE) {
      ...
      if (ctx.reuseUdfEnable) {
        generateScalarFunctionCallReuseUdf(ctx, functionTerm, externalOperands, outputDataType, operands)
      } else {
  ...
  private def generateTableFunctionCall(
    ...
    // function call
    val functionCallCode = if (ctx.udfMetricOpened) {
      s"""
        |${externalOperands.map(_.code).mkString("\n")}
        |long start_$functionTerm = System.nanoTime();
        |$functionTerm.eval(${externalOperands.map(_.resultTerm).mkString(", ")});
        |long end_$functionTerm = System.nanoTime();
        |this.udfEvalHist_${udf.toString}.update(end_$functionTerm - start_$functionTerm);
        |""".stripMargin
    } else {
      s"""
         |${externalOperands.map(_.code).mkString("\n")}
  ...
  private def generateScalarFunctionCall(
    ...
    // function call
    internalExpr.copy(code = if (ctx.udfMetricOpened) {
      s"""
        |${externalOperands.map(_.code).mkString("\n")}
        |long start_$functionTerm = System.nanoTime();
  ...
  private def generateScalarFunctionCallReuseUdf(
      ctx: CodeGeneratorContext,
      functionTerm: String,
      externalOperands: Seq[GeneratedExpression],
      outputDataType: DataType,
    ...
    // result conversion
    val externalResultClass = externalResultConversionClass(outputDataType)
    val externalResultTypeTerm = typeTerm(externalResultClass)
    // Janino does not fully support the JVM spec:
    // boolean b = (boolean) f(); where f returns Object
    // This is not supported and we need to box manually.
    ...
    val externalResultTerm = if (udf.isDeterministic) {
      ctx.addReusableLocalVariable(externalResultTypeTerm, "externalResult", "null")
    } else {
      ctx.addReusableLocalVariable(externalResultTypeTerm, "externalResult")
    }
    val internalExpr = genToInternalConverterAll(ctx, outputDataType, externalResultTerm)
    val funcEvalCode = s"$externalResultCasting $functionTerm.$SCALAR_EVAL(${externalOperands.map(_.resultTerm).mkString(", ")})"

    val newCode = if (udf.isDeterministic) {

      val funcOperandsExpr = ctx.functionOperandsExpr(functionTerm, operands)
      val reusableFuncExpr = ctx.reuseScalarFuncExpr(funcOperandsExpr)

      ctx.addReusableResultTerm(internalExpr.resultTerm, funcOperandsExpr)
      ctx.addReusableResultTerm(externalResultTerm, funcOperandsExpr)

      val setResult = if (reusableFuncExpr.equals(funcOperandsExpr)) {
        ctx.addReusableScalarFuncExpr(funcOperandsExpr, externalResultTerm)
        s"$externalResultTerm = $funcEvalCode;"
      } else {
        ctx.addActualReusableResultTerm(reusableFuncExpr)
        s"$externalResultTerm = $reusableFuncExpr == null ? $funcEvalCode : $reusableFuncExpr;"
      }
    ...
    // function call
    internalExpr.copy(code = newCode)
  }
