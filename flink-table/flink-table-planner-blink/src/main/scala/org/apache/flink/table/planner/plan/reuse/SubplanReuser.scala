...
import org.apache.flink.table.planner.plan.utils.{CorrelationShuttle, DefaultRelShuttle, FlinkRelOptUtil, SameRelObjectShuttle}
...
object SubplanReuser {
  ...
  /**
    ...
    */
  def reuseDuplicatedSubplan(rels: Seq[RelNode], tableConfig: TableConfig, rexBuilder: RexBuilder): Seq[RelNode] = {
    ...
    //merge correlationId
    val correlShuttle = new CorrelationShuttle(rexBuilder)
    val relsMergedCorrel = relsWithoutSameObj.map(_.accept(correlShuttle))
    ...
    val context = new SubplanReuseContext(tableSourceReuseEnabled, relsMergedCorrel: _*)
    val reuseShuttle = new SubplanReuseShuttle(context)
    relsMergedCorrel.map(_.accept(reuseShuttle))
  }
