...
class StreamExecLegacyTableSourceScan(
  ...
  override protected def translateToPlanInternal(
    ...
    if(streamTransformation != inputTransform ){
      streamTransformation.setSlotSharingGroup(inputTransform.getSlotSharingGroup)
    }
    ...
    if(withWatermarks != ingestedTable){
      withWatermarks.getTransformation.setSlotSharingGroup(ingestedTable.getTransformation.getSlotSharingGroup)
    }
