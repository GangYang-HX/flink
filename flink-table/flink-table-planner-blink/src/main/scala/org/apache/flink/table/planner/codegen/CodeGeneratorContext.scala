...
import org.apache.flink.table.functions.{FunctionContext, AggregateFunction, UserDefinedFunction}
...
class CodeGeneratorContext(val tableConfig: TableConfig) {
  ...
  private val actualReusableTerms: mutable.LinkedHashSet[String] =
    mutable.LinkedHashSet[String]()
  ...
  def reuseUdfEnable: Boolean = tableConfig.getConfiguration.getBoolean(TableConfigOptions.TABLE_REUSE_USER_DEFINE_FUNCTION_ENABLED)

  def preCompileCalcCodeGenEnable: Boolean = tableConfig.getConfiguration.getBoolean(TableConfigOptions.TABLE_PRE_COMPILE_CALC_CODEGEN_ENABLED)

  def udfMetricOpened: Boolean = tableConfig.getConfiguration.getBoolean(TableConfigOptions.TABLE_UDF_METRIC_OPENED)
  ...
  def addReusableLocalVariable(fieldTypeTerm: String, fieldName: String, defaultValue: String): String = {
    val fieldTerm = newName(fieldName)
    reusableLocalVariableStatements
      .getOrElse(currentMethodNameForLocalVariables, mutable.LinkedHashSet[String]())
  ...
  private def addUdfMetricObject(
      udfName: String,
      udfClassName: String): Unit = {
    reusableMemberStatements.add(s"private final $udfClassName udfOpenHist_$udfName;")
    reusableMemberStatements.add(s"private final $udfClassName udfEvalHist_$udfName;")
  }
  ...
  def addReusableScalarFuncExpr(funcOperandsExpr: String, term: String): Unit = {
    if (!reusableScalarFuncExprs.contains(funcOperandsExpr)) {
      reusableScalarFuncExprs.put(funcOperandsExpr, term)
    }
  }

  def functionOperandsExpr(functionReference: String, operands: Seq[GeneratedExpression]): String = {
    functionReference + ":" + operands.map(_.resultTerm).map(reuseResultTerm).mkString(",")
  }
  ...
  def addActualReusableResultTerm(term:String): Unit ={
    actualReusableTerms.add(term)
  }

  /**
   * Clear global variable reuse values
   */
  def clearReusableLocalVariable(): String = {
    if (isCodeSplitMap.nonEmpty) {
      actualReusableTerms.map(a => a + " = null;").mkString("\n")
    } else {
      ""
    }
  }

  def reuseScalarFuncExpr(funcOperandsExpr: String): String = {
    reusableScalarFuncExprs.getOrElse(funcOperandsExpr, funcOperandsExpr)
  }

  def reuseResultTerm(term: String): String = {
    reusableResultTerms.getOrElse(term, s"($term)")
  }
  ...
  def addReusableFunction(
    ...
    val functionName = function.toString

    val udfMetricInit = if (udfMetricOpened) {
      s"""
         |
         |this.udfOpenHist_${functionName} = this.metricGroup.histogram("udfOpenRt_${functionName}", new ${className[DescriptiveStatisticsHistogram]}(1000));
         |if (this.udfOpenHist_${functionName} == null){
         |  throw new RuntimeException("udfOpenRt_${functionName} is null");
         |}
         |
         |this.udfEvalHist_${functionName} = this.metricGroup.histogram("udfEvalRt_${functionName}", new ${className[DescriptiveStatisticsHistogram]}(1000));
         |if (this.udfEvalHist_${functionName} == null){
         |  throw new RuntimeException("udfEvalRt_${functionName} is null");
         |}
       """.stripMargin
    } else {
      s"""\n""".stripMargin
    ...
    val funcSuperClassName = function.getClass.getSuperclass.getSimpleName

    if (udfMetricOpened && funcSuperClassName != "AsyncSideFunction") {
      addUdfMetricObject(functionName, className[Histogram])
    }
    ...
    val openFunction = if (contextTerm != null) {
      if (udfMetricOpened && funcSuperClassName != "AsyncSideFunction") {
        s"""
           |${udfMetricInit}
           |long start_$fieldTerm = System.nanoTime();
           |$fieldTerm.open(new ${functionContextClass.getCanonicalName}($contextTerm));
           |long end_$fieldTerm = System.nanoTime();
           |this.udfOpenHist_${functionName}.update(end_$fieldTerm - start_$fieldTerm);
       """.stripMargin
      } else {
       s"""
           |$fieldTerm.open(new ${functionContextClass.getCanonicalName}($contextTerm));
      ...
      if (udfMetricOpened && funcSuperClassName != "AsyncSideFunction") {
       ...
       """.stripMargin
        ...
        s"""$fieldTerm.open(new ${functionContextClass.getCanonicalName}(getRuntimeContext()));""".stripMargin
      }
    }
