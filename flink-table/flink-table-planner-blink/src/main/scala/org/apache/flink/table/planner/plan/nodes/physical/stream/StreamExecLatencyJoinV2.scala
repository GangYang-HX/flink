...
import org.apache.flink.table.planner.plan.utils.{ExpandUtil, IntervalJoinUtil, JoinTypeUtil, KeySelectorUtil}
import org.apache.flink.table.runtime.generated.GeneratedFunction
import org.apache.flink.table.runtime.operators.join.latency.v2.ProcTimeLatencyJoin
import org.apache.flink.table.runtime.operators.join.latency.v2.RowTimeLatencyJoin
...
class StreamExecLatencyJoinV2(
  ...
  override protected def translateToPlanInternal(
    ...
    val configuration = planner.getExecEnv.getConfiguration
    val keyedStateTtlEnable = ExpandUtil.keyedStateTtlEnable(configuration)
    joinConfig.setKeyedStateTtlEnable(keyedStateTtlEnable);
    flinkJoinType match {
  ...
  private def createNegativeWindowSizeJoin(
    ...
    val filterAllLeftStream = new OneInputTransformation[RowData, RowData](leftPlan, "filter all left input transformation", getFlinkRelTypeName+"FilterLeft", new StreamFlatMap[RowData, RowData](allFilter), returnTypeInfo, leftParallelism)

    val filterAllRightStream = new OneInputTransformation[RowData, RowData](rightPlan, "filter all right input transformation", getFlinkRelTypeName+"FilterRight", new StreamFlatMap[RowData, RowData](allFilter), returnTypeInfo, rightParallelism)

    val padLeftStream = new OneInputTransformation[RowData, RowData](leftPlan, "pad left input transformation", getFlinkRelTypeName+"PadLeft", new StreamMap[RowData, RowData](leftPadder), returnTypeInfo, leftParallelism)

    val padRightStream = new OneInputTransformation[RowData, RowData](rightPlan, "pad right input transformation", getFlinkRelTypeName+"PadRighr", new StreamMap[RowData, RowData](rightPadder), returnTypeInfo, rightParallelism)
    flinkJoinType match {
      case FlinkJoinType.INNER =>
  ...
  private def createProcTimeJoin(
      ...
      leftKeys: Array[Int],
      rightKeys: Array[Int],
      joinConfig: LatencyJoinConfig): Transformation[RowData] = {
    ...
    val procJoinFunc = new ProcTimeLatencyJoin(
      flinkJoinType,
      leftLowerBound,
      leftUpperBound,
      leftTypeInfo,
  ...
  private def createRowTimeJoin(
    ...
    val leftTypeInfo = leftPlan.getOutputType.asInstanceOf[RowDataTypeInfo]
    val rightTypeInfo = rightPlan.getOutputType.asInstanceOf[RowDataTypeInfo]
    val rowJoinFunc = new RowTimeLatencyJoin(
      flinkJoinType,
      leftLowerBound,
      leftUpperBound,
      0L,
    ...
    val ret = new TwoInputTransformation[RowData, RowData, RowData](leftPlan, rightPlan, getRelDetailedDescription, getFlinkRelTypeName, new KeyedCoProcessOperatorWithWatermarkDelay(rowJoinFunc, rowJoinFunc.getMaxOutputDelay)
            .asInstanceOf[TwoInputStreamOperator[RowData,RowData,RowData]], returnTypeInfo, leftPlan.getParallelism)
