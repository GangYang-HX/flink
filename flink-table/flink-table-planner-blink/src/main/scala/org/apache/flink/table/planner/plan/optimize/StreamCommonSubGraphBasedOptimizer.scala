...
import org.apache.calcite.rex.{RexBuilder, RexCall, RexOver, RexVisitorImpl}
...
class StreamCommonSubGraphBasedOptimizer(planner: StreamPlanner)
  ...
  override protected def doOptimize(roots: Seq[RelNode]): Seq[RelNodeBlock] = {
    ...
    checkAndEnableMiniBatch(roots, config)
  ...
  private var windowAggMatched = false
  private var allGroupAggMatched = true
  private var groupAggMatched = false

  private def checkAndEnableMiniBatch(nodes: Seq[RelNode], config: TableConfig): Unit = {
    if (config.getConfiguration.getBoolean(ExecutionConfigOptions.TABLE_EXEC_MINIBATCH_ENABLED)) {
      checkAggRelNodes(nodes)
      enableMiniBatch(config)
    }
  }

  /**
   * 检查是否有特定的relNodes，决定是否关闭mini-batch（当前默认配置为true，代码逻辑为适当的时候置为false）
   * 当前主要检查windowAgg和groupAgg
   */
  private def checkAggRelNodes(relNodes: Seq[RelNode]): Unit = {
    val checkAggRelNodeShuttle: RelShuttleImpl = new RelShuttleImpl() {
      final val visitor = new RexVisitorImpl[Void](true) {
        override def visitCall(call: RexCall): Void = {
          call.getOperator match {
            case FlinkSqlOperatorTable.TUMBLE_OLD | FlinkSqlOperatorTable.HOP_OLD | FlinkSqlOperatorTable.SESSION_OLD =>
              windowAggMatched = true
            case _ =>
          }
          super.visitCall(call)
        }

        override def visitOver(over: RexOver): Void = {
          groupAggMatched = true
    ...
    relNodes.foreach( relNode => {
      groupAggMatched = false
      relNode.accept(checkAggRelNodeShuttle)
      allGroupAggMatched &&= groupAggMatched
    })
  }

  private def enableMiniBatch(config: TableConfig): Unit = {
    // window agg优先级更高，可以覆盖group agg设置的true
    if (windowAggMatched) {
      config.getConfiguration.setBoolean(ExecutionConfigOptions.TABLE_EXEC_MINIBATCH_ENABLED, false)
      return
    }

    // 从所有sink开始的visit都遇到group agg，才可以保持mini-batch开启
    if (!allGroupAggMatched) {
      config.getConfiguration.setBoolean(ExecutionConfigOptions.TABLE_EXEC_MINIBATCH_ENABLED, false)
    }
  }
