...
class StreamExecLegacySink[T](
  ...
  override protected def translateToPlanInternal(
    ...
    val resultTransformation = sink match {
      case streamTableSink: StreamTableSink[T] =>
        ...
        try {
          val tableName = ObjectIdentifier.from(sinkName)
          val catalogTable = planner.catalogManager.getTable(tableName)
          if( catalogTable.isPresent ){
            val slotGroup = catalogTable.get().getTable.getOptions.get(SlotGroupOptions.SLOT_SLOT.key());
            if( slotGroup != null ){
  ...
  private def translateToTransformation(
    ...
    if (CodeGenUtils.isInternalClass(resultDataType)) {
      ...
      new OneInputTransformation(parTransformation, s"SinkConversionTo${resultDataType.getConversionClass.getSimpleName}", getFlinkRelTypeName, converterOperator, outputTypeInfo, parTransformation.getParallelism)
    }
