...
import org.apache.calcite.rel.core.{CorrelationId, TableFunctionScan, TableScan}
import org.apache.calcite.rel.logical._
import org.apache.calcite.rel.{RelNode, RelShuttle, RelShuttleImpl}
import org.apache.calcite.rex.{RexBuilder, RexCorrelVariable, RexLiteral, RexNode, RexShuttle, RexSubQuery}
...
class CorrelationShuttle(rexBuilder: RexBuilder) extends DefaultRelShuttle {

  private val sameCorrelation = Maps.newHashMap[String, CorrelationId]()

  override def visit(node: RelNode): RelNode = {

    node match {
      case correlate: StreamExecCorrelate =>

        val correlationDigest = FlinkRelOptUtil.getDigest(node.getInput(0));
        def finder = new CorrelFinder
        val originalId = finder.findCorrelId(correlate.getScan)
        val replaceId = sameCorrelation.get(correlationDigest)
        if (replaceId != null && originalId != null) {
          val replacer = new CorrelReplacer(rexBuilder)
          val newScan = replacer.replaceCorrelId(originalId, replaceId, correlate.getScan)
          correlate.replaceScan(newScan.asInstanceOf[FlinkLogicalTableFunctionScan])
        } else {
          sameCorrelation.put(correlationDigest, originalId)
        }
...
class CorrelFinder
  extends RexShuttle {
  private val correlNodes = Sets.newIdentityHashSet[CorrelationId]()

  def findCorrelId(scan: TableFunctionScan): CorrelationId = {
    scan.accept(this)
    if (correlNodes.size() > 1) {
      throw new UnsupportedOperationException("multi correlNodes")
    }
    if (correlNodes.size == 0) {
      null
    } else {
      correlNodes.iterator().next()
    }
  }

  override def visitCorrelVariable(node: RexCorrelVariable): RexNode = {
    correlNodes.add(node.id)
    node
  }
}

class CorrelReplacer(rexBuilder: RexBuilder)
  extends RexShuttle {
  private val maps = Maps.newConcurrentMap[CorrelationId, CorrelationId]()

  def replaceCorrelId(oldId: CorrelationId, newId: CorrelationId, scan: TableFunctionScan): RelNode = {
    maps.put(oldId, newId)
    scan.accept(this)
  }

  override def visitCorrelVariable(node: RexCorrelVariable): RexNode = {
    if (maps.containsKey(node.id)) {
      rexBuilder.makeCorrel(node.getType,maps.get(node.id))
    } else {
      node
    }
  }
