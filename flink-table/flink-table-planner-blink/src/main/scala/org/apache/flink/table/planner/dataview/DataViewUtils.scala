...
import org.apache.flink.table.runtime.typeutils.{InternalTypeInfoV2, RowDataTypeInfo}
...
object DataViewUtils {
   ...
   *         field name and state descriptor
   */
  def useNullSerializerForStateViewFieldsFromAccTypeV2(
    index: Int,
    aggFun: UserDefinedAggregateFunction[_, _],
    externalAccType: DataType,
    isStateBackedDataViews: Boolean)
  : (DataType, Array[DataViewSpecV2]) = {

    val acc = aggFun.createAccumulator()
    val accumulatorSpecs = new mutable.ArrayBuffer[DataViewSpecV2]
        ...
        t.getTypeInformation match {
          case pojoType: PojoTypeInfo[_] if pojoType.getArity > 0 =>
            ...
            for (i <- 0 until arity) {
              ...
              field.setAccessible(true)
              val instance = field.get(acc)
              val (newTypeInfo: TypeInformation[_], spec: Option[DataViewSpecV2]) =
                decorateDataViewTypeInfo(
                  pojoField.getTypeInformation,
                  instance,
                  isStateBackedDataViews,
          ...
          // so we add another check => acc.isInstanceOf[GenericRowData]
          case t: InternalTypeInfoV2[RowData] if acc.isInstanceOf[GenericRowData] =>
            val accInstance = acc.asInstanceOf[GenericRowData]
            ...
            val newType = InternalTypeInfoV2.ofFields(newFieldTypes.toArray, fieldNames)
            (fromLegacyInfoToDataType(newType), accumulatorSpecs.toArray)
