...
import org.apache.flink.table.runtime.dataview.{DataViewSpecV2, ListViewSpecV2, MapViewSpecV2, StateListView, StateMapView}
...
import org.apache.flink.table.planner.codegen.agg.AggsHandlerCodeGeneratorV2._
 ...
 */
class AggsHandlerCodeGeneratorV2(
   ...
   */
  def withConstants(literals: Seq[RexLiteral]): AggsHandlerCodeGeneratorV2 = {
    // create constants
   ...
   */
  def needAccumulate(): AggsHandlerCodeGeneratorV2 = {
   ...
   * @return
   */
  def needRetract(): AggsHandlerCodeGeneratorV2 = {
  ...
  def needMerge(
    ...
    mergedAccExternalTypes: Array[DataType] = null): AggsHandlerCodeGeneratorV2 = {
   ...
   */
  private def initialAggregateInformation(aggInfoList: AggregateInfoListV2): Unit = {
    ...
    val distinctCodeGens = aggInfoList.distinctInfos.zipWithIndex.map {
      case (distinctInfo, index) =>
        ...
        val codegen = new DistinctAggCodeGenV2(
          ctx,
          distinctInfo,
          index,
          innerCodeGens,
  ...
  def generateAggsHandler(
    name: String,
    aggInfoList: AggregateInfoListV2): GeneratedAggsHandleFunction = {
    ...
    val udfMetricDefine = if (ctx.udfMetricOpened) {
      s"""
         |private final ${className[MetricGroup]} metricGroup;""".stripMargin
    } else { s"""\n""" }

    val udfMetricInitCode = if(ctx.udfMetricOpened) {
      s"""
         |
         |this.metricGroup = getRuntimeContext().getMetricGroup();
         |this.metricGroup.addGroup("UDFMetric");
         |
         |if (this.metricGroup == null){
         |  throw new RuntimeException("metric group is null");
         |}
         |""".stripMargin
    } else { s"""\n""" }
  ...
  def generateTableAggsHandler(
    name: String,
    aggInfoList: AggregateInfoListV2): GeneratedTableAggsHandleFunction = {
...
object AggsHandlerCodeGeneratorV2 {
   ...
   */
  def createDataViewTerm(spec: DataViewSpecV2): String = {
   ...
   */
  def createDataViewRawValueTerm(spec: DataViewSpecV2): String = {
   ...
   */
  def createDataViewBackupTerm(spec: DataViewSpecV2): String = {
   ...
   */
  def createDataViewBackupRawValueTerm(spec: DataViewSpecV2): String = {
    s"${createDataViewBackupTerm(spec)}_raw_value"
  }

  def addReusableStateDataViews(
    ctx: CodeGeneratorContext,
    viewSpecs: Array[DataViewSpecV2],
