...
import org.apache.flink.configuration.{CheckpointingOptions, ConfigOption, Configuration}
...
object ExpandUtil {
  ...
  private[flink] val TABLE_OPTIMIZER_KEYED_STATE_TTL_ENABLE: ConfigOption[java.lang.Boolean] =
    key("table.optimizer.keyedState.ttl.enable")
      .booleanType()
      .defaultValue(true)
      .withDescription("table.optimizer.keyedState.ttl.enable")
  ...
  // StateBackend Utils
  // ------------------------------------------------------------------------------------------
  def keyedStateTtlEnable(conf: Configuration): Boolean = {
    val backend = conf.getString(CheckpointingOptions.STATE_BACKEND, "")
    val enable = conf.getBoolean(TABLE_OPTIMIZER_KEYED_STATE_TTL_ENABLE)
    backend.equals("taishan") && enable
  }
}
