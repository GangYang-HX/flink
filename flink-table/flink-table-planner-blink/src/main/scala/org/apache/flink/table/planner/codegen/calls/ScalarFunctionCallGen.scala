...
class ScalarFunctionCallGen(scalarFunction: ScalarFunction) extends CallGenerator {
  ...
  override def generate(
    ...
    if (ctx.reuseUdfEnable) {
      generateReuseUdf(ctx, operands, returnType)
    } else {
      generateNormal(ctx, operands, returnType)
    }
  }
  ...
  def generateNormal(
    ...
    val setResult = {
      if (resultClass.isPrimitive && isInternalClass(resultExternalType)) {
        if (ctx.udfMetricOpened) {
          s"""
             |long start_$functionReference = System.nanoTime();
             |$resultTerm = $evalResult;
             |long end_$functionReference = System.nanoTime();
             |this.udfEvalHist_${scalarFunction.toString}.update(end_$functionReference - start_$functionReference);
             |""".stripMargin
        } else {
          s"""
             |$resultTerm = $evalResult;
  ...
  def generateReuseUdf(
    ...
    val resultExternalType = UserDefinedFunctionUtils.getResultTypeOfScalarFunction(
      scalarFunction, operandTypes)
    val javaResultTypeTerm = typeTerm(externalResultConversionClass(resultExternalType))
    val javaResultTerm = if (scalarFunction.isDeterministic) {
      ctx.addReusableLocalVariable(javaResultTypeTerm, "javaResult", "null")
    } else {
      ctx.addReusableLocalVariable(javaResultTypeTerm, "javaResult")
    }

    val internalResultTerm = if (isInternalClass(resultExternalType)) {
      s"$javaResultTerm"
    } else {
      genToInternalConverter(ctx, resultExternalType)(javaResultTerm)
    }

    val resultUnboxing = if (resultClass.isPrimitive) {
      GenerateUtils.generateNonNullField(returnType, internalResultTerm)
    } else {
      GenerateUtils.generateInputFieldUnboxing(ctx, returnType, internalResultTerm, internalResultTerm)
    }
    val evalResult = s"$functionReference.eval(${parameters.map(_.resultTerm).mkString(", ")})"

    val newCode = if (scalarFunction.isDeterministic) {
      val funcOperandsExpr = ctx.functionOperandsExpr(functionReference, operands)
      val reusableFuncExpr = ctx.reuseScalarFuncExpr(funcOperandsExpr)

      ctx.addReusableResultTerm(resultUnboxing.resultTerm, funcOperandsExpr)
      ctx.addReusableResultTerm(javaResultTerm, funcOperandsExpr)

      val setResult = if (reusableFuncExpr.equals(funcOperandsExpr)) {
        ctx.addReusableScalarFuncExpr(funcOperandsExpr, javaResultTerm)
        s"$javaResultTerm = $evalResult;"
      } else {
        ctx.addActualReusableResultTerm(reusableFuncExpr)
        s"$javaResultTerm = $reusableFuncExpr == null ? $evalResult : $reusableFuncExpr;"
      }
      if (ctx.udfMetricOpened) {
        s"""
           |${parameters.map(_.code).mkString("\n")}
    ...
    resultUnboxing.copy(code = newCode)
  }
