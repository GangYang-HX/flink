...
import org.apache.flink.table.planner.plan.nodes.physical.stream.{StreamExecCalc, StreamExecExpand, StreamPhysicalWindowAggregate, StreamPhysicalWindowTableFunction}
...
/**
 * This rule transposes [[StreamExecExpand]] past [[StreamPhysicalWindowTableFunction]] to make
 ...
 * So we need to transpose [[StreamExecExpand]] past [[StreamPhysicalWindowTableFunction]]
...
class ExpandWindowTableFunctionTransposeRule
  extends RelOptRule(
    operand(classOf[StreamExecExpand],
      operand(classOf[StreamExecCalc],
        operand(classOf[StreamPhysicalWindowTableFunction], any()))),
    "ExpandWindowTableFunctionTransposeRule") {

  override def matches(call: RelOptRuleCall): Boolean = {
    val expand: StreamExecExpand = call.rel(0)
    val calc: StreamExecCalc = call.rel(1)
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val newCalc = new StreamExecCalc(
      cluster,
      calc.getTraitSet,
      newInput,
      newProgram,
    ...
    val topRexProgram = RexProgram.create(
      ...
      expand.getRowType,
      cluster.getRexBuilder)
    val topCalc = new StreamExecCalc(
      cluster,
      expand.getTraitSet,
      newWindowTVF,
      topRexProgram,
  ...
  private def buildNewExpand(
                              expand: StreamExecExpand,
                              newCalc: StreamExecCalc,
                              inputFieldShifting: Array[Int],
                              newTimeField: Int,
                              timeFieldAdded: Boolean): StreamExecExpand = {
