...
import org.apache.flink.table.api.{TableConfig, TableEnvironment, TableException, TableSchema, ValidationException}
...
abstract class PlannerBase(
      ...
      case catalogSink: CatalogSinkModifyOperation =>
        ...
        getTableSink(identifier, dynamicOptions).map {
          ...
          case (table, sink: DynamicTableSink) =>
            // validate TableSink
            validateTableSink(catalogSink, identifier, sink, table.getPartitionKeys)
            //validate write metaddata ability
            validateAndApplyMetadata(table.getSchema, sink)
            // validate query schema and sink schema, and apply cast if possible
            val query = validateSchemaAndApplyImplicitCast(
              input,
              TableSchemaUtils.getNoGeneratedSchema(table.getSchema),
              getTypeFactory,
              Some(catalogSink.getTableIdentifier.asSummaryString()))
            LogicalSink.create(
              query,
  ...
  private def validateAndApplyMetadata(schema: TableSchema, tableSink: DynamicTableSink) : Unit = {
    val metaColumns = TableSchemaUtils.extractMetadataColumn(schema)
    if (!metaColumns.isEmpty) {
      if (!tableSink.isInstanceOf[SupportsWriteMetadata]) {
        throw new ValidationException(String.format("Table '%s' declares persistable metadata columns, " +
          "but the underlying %s " + "doesn't implement the %s interface. If the column should not " + "be persisted," +
          " it can be declared with the VIRTUAL keyword.", tableSink.asSummaryString,
          classOf[DynamicTableSink].getSimpleName, classOf[SupportsWriteMetadata].getSimpleName))
      }
      val metadataSink = tableSink.asInstanceOf[SupportsWriteMetadata]
      ...
      metadataSink.applyWriteMetadata(metaKeys, metaIndices.toArray[Int])
    }
  }
  ...
  private[flink] def translateToExecNodePlan(
    ...
    // reuse subplan
    val reusedPlan = SubplanReuser.reuseDuplicatedSubplan(optimizedRelNodes, config, getRelBuilder.getRexBuilder)
