...
class CatalogSourceTable[T](
  ...
  override def toRel(context: RelOptTable.ToRelContext): RelNode = {
    ...
    // 2. push no-physical column project
    if (containsMetadataColumns(catalogTable) || containsGeneratedColumns(catalogTable)) {
      pushMetadataProjection(relBuilder, typeFactory, catalogTable.getSchema)
      pushComputedProjection(flinkContext, relBuilder)
    }
  ...
  private def pushMetadataProjection(relBuilder: FlinkRelBuilder,
                                     typeFactory: FlinkTypeFactory,
                                     schema: TableSchema): Unit = {
    val rexBuilder = relBuilder.getRexBuilder
    val fieldNames = schema.getTableColumns
      .filter(col => !col.isGenerated)
      .map(col => col.getName)
      .toList
    val fieldNodes = schema.getTableColumns
      .filter(col => !col.isGenerated)
      .map(col => {
        val relDataType = typeFactory.createFieldTypeFromLogicalType(col.getType.getLogicalType)
        if (col.isMetadata) {
          val metadataColumn = col.asInstanceOf[MetadataColumn]
          val metadataKey = metadataColumn.getName
          rexBuilder.makeAbstractCast(relDataType, relBuilder.field(metadataKey))
        } else {
          relBuilder.field(col.getName)
        }
      }).toList
    relBuilder.projectNamed(fieldNodes, fieldNames, true)
  }

  private def pushComputedProjection(flinkContext: FlinkContext,
                                     relBuilder: FlinkRelBuilder): Unit = {
    val converter = flinkContext.getSqlExprToRexConverterFactory.create(
      relBuilder.peek().getRowType
    )
    val fieldNames = util.Arrays.asList(catalogTable.getSchema.getFieldNames: _*)
    val projection = catalogTable.getSchema.getTableColumns
      .map(c => {
        if (c.isGenerated) converter.convertToRexNode(c.getExpr.get())
        else relBuilder.field(c.getName)
      }).toList
    relBuilder.projectNamed(projection, fieldNames, true)
  }
  ...
  private def buildTableScan(
    ...
    validateAndApplyMetadata(catalogTable.getSchema, tableSource)
  ...
  private def containsMetadataColumns(catalogTable: CatalogTable): Boolean = {
    catalogTable.getSchema.getTableColumns.exists(_.isMetadata)
  }
  ...
  private def validateAndApplyMetadata(schema: TableSchema, tableSource: DynamicTableSource) : Unit = {
    val metaColumns = TableSchemaUtils.extractMetadataColumn(schema)
    if (!metaColumns.isEmpty) {
      if (!tableSource.isInstanceOf[SupportsReadMetadata]) {
          throw new ValidationException(String.format("Table '%s' declares metadata columns, " +
            "but the underlying %s doesn't implement " + "the %s interface. " +
            "Therefore, metadata cannot be read from the given source.", tableSource.asSummaryString,
            classOf[DynamicTableSource].getSimpleName, classOf[SupportsReadMetadata].getSimpleName))
      }
      val metadataSource = tableSource.asInstanceOf[SupportsReadMetadata]
      val metaKeys = metaColumns.map(col => col.getMetadataAlias.orElse(col.getName)).toList
      val metaIndices = new ArrayBuffer[Int]()
      val noGeneratedCols = TableSchemaUtils.getNoGeneratedSchema(schema).getTableColumns
      for( i <- noGeneratedCols.indices ){
        if (noGeneratedCols.get(i).isMetadata) metaIndices.append(i) else None
      }
      metadataSource.applyReadMetadata(metaKeys, metaIndices.toArray[Int])
    }
  }
