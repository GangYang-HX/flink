...
class FlinkTypeFactory(typeSystem: RelDataTypeSystem)
   ...
   */
  def buildRelNodeRowTypeV2(rowType: RowType): RelDataType = {
    val fields = rowType.getFields
    buildStructTypeV2(fields.map(_.getName), fields.map(_.getType), StructKind.FULLY_QUALIFIED)
  }
