...
public final class ObjectIdentifier implements Serializable {
    ...
    public static ObjectIdentifier from(String fullName) {
        ...
        if (paths.length != 3) {
            throw new IllegalArgumentException(
                    String.format("Cannot get split '%s' to get catalog, databaseName and objectName", fullName));
        }
        String catalog = paths[0].replace("`","");
        String database = paths[1].replace("`","");
        String table = paths[2].replace("`","");
        return new ObjectIdentifier(catalog,database,table);
    }
