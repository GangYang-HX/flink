...
/**
 * @author zhouxiaogang,zhangyang
 */
public final class SaberStringUtils {

    private SaberStringUtils() {
    ...
    public static char unicodeStringDecodeChar(String input) {
        if (input.length() == 1) {
            ...
            for (int i = 0; i < maxLoop; i++) {
                if (input.charAt(i) == '\\') {
                    boolean isOk = (i < maxLoop - 5) && ((input.charAt(i + 1) == 'u') || (input.charAt(i + 1) == 'U'));
                    if (isOk) {
                        try {
                            retBuf.append((char) Integer.parseInt(input.substring(i + 2, i + 6), 16));
                            i += 5;
     ...
     * @return
     */
    public static String[] replace(String[] sqlSplit, int fromLine, int fromLinePos, int endLine, int endLinePos,
                                   String newStr) {
    ...
    public static List<String> replace(List<String> sqlSplit, int fromLine, int fromLinePos, int endLine,
                                       int endLinePos, String newStr) {
        String[] newSqlSplit = new String[sqlSplit.size()];
        for (int i = 0; i < sqlSplit.size(); i++) {
            newSqlSplit[i] = sqlSplit.get(i);
        }
        return Arrays.asList(replace(newSqlSplit, fromLine, fromLinePos, endLine, endLinePos, newStr));
    }
