...
public class TableSchema {
	...
	public DataType toPersistRowDataType() {
		final Field[] fields = columns.stream()
			.filter(col -> {
				if (col.isMetadata()) {
					TableColumn.MetadataColumn metadataColumn = (TableColumn.MetadataColumn) col;
					return !metadataColumn.isVirtual();
				} else {
					return col.isPhysical();
				}
			})
			.map(column -> FIELD(column.getName(), column.getType()))
			.toArray(Field[]::new);
