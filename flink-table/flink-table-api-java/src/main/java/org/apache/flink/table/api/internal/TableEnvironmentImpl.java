...
public class TableEnvironmentImpl implements TableEnvironmentInternal {
	...
	@Override
	public TableResult executeInternalWithJobName(List<ModifyOperation> operations, String jobName) {
		StatisticsUtils.reportEndTracking(StartupUtils.StartupPhase.INVOKE_USER_PROGRAM);
		StatisticsUtils.reportStartTracking(StartupUtils.StartupPhase.TRANSLATE_TO_TRANSFORMATION);
		List<Transformation<?>> transformations = translate(operations);
		StatisticsUtils.reportEndTracking(StartupUtils.StartupPhase.TRANSLATE_TO_TRANSFORMATION);
		List<String> sinkIdentifierNames = extractSinkIdentifierNames(operations);
		...
		try {
			...
			StatisticsUtils.reportEndTracking(
					StartupUtils.StartupPhase.SUBMIT_JOB_TO_DISPATCHER);
