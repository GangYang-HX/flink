...
public class ExecutionConfigOptions {
    ...
    @Documentation.TableOption(execMode = Documentation.ExecMode.STREAMING)
    public static final ConfigOption<Integer> TABLE_EXEC_ASYNC_LOOKUP_RETRY_TIMES =
            key("table.exec.async-lookup.retry-times")
                    .defaultValue(2)
                    .withDescription("The max number retry times. please make sure OptimizerConfigOptions.TABLE_OPTIMIZER_JOIN_LOOKUP_KEYED_ENABLED is enabled");

    @Documentation.TableOption(execMode = Documentation.ExecMode.STREAMING)
    public static final ConfigOption<Integer> TABLE_EXEC_ASYNC_LOOKUP_RETRY_BACKOFF =
            key("table.exec.async-lookup.retry-backoff")
                    .defaultValue(5000)
                    .withDescription("The retry backoff mills. please make sure OptimizerConfigOptions.TABLE_OPTIMIZER_JOIN_LOOKUP_KEYED_ENABLED is enabled");
	// ------------------------------------------------------------------------
	//  MiniBatch Options
	// ------------------------------------------------------------------------
	@Documentation.TableOption(execMode = Documentation.ExecMode.STREAMING)
