...
public class OptimizerConfigOptions {
    ...
    @Documentation.TableOption(execMode = Documentation.ExecMode.STREAMING)
    public static final ConfigOption<Boolean> TABLE_OPTIMIZER_JOIN_LOOKUP_KEYED_ENABLED =
            key("table.optimizer.join.lookup-keyed-enabled")
                    .defaultValue(false)
                    .withDescription("Enables join keyed. Default is disabled.");
}
