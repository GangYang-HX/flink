...
abstract class TimeIntervalJoin extends KeyedCoProcessFunction<RowData, RowData, RowData, RowData> {
	...
	TimeIntervalJoin(
		...
		if (config.getCleanUpInterval() != -1) {
			this.cleanUpInterval = config.getCleanUpInterval();
		}
		else {
		...
		this.keyedStateTtlSize = Math.max(leftRelativeSize, rightRelativeSize) + cleanUpInterval + allowedLateness + 10;
	...
	public void open(Configuration parameters) throws Exception {
		...
		if (keyedStateTtlEnable) {
			rightMapStateDescriptor.enableTimeToLive(StateTtlConfigUtil.createTtlConfig(keyedStateTtlSize));
		}
		...
		ValueStateDescriptor<Long> leftValueStateDescriptor = new ValueStateDescriptor<>(
				...
				Long.class);
		if (keyedStateTtlEnable) {
			leftValueStateDescriptor.enableTimeToLive(StateTtlConfigUtil.createTtlConfig(keyedStateTtlSize));
		}
		...
		ValueStateDescriptor<Long> rightValueStateDescriptor = new ValueStateDescriptor<>(
				...
				Long.class);
		if (keyedStateTtlEnable) {
			rightValueStateDescriptor.enableTimeToLive(StateTtlConfigUtil.createTtlConfig(keyedStateTtlSize));
		}
	...
	private void registerCleanUpTimer(Context ctx, long rowTime, boolean leftRow) throws IOException {
		if (leftRow) {
			long cleanUpTime = rowTime + leftRelativeSize + cleanUpInterval + allowedLateness + 1;
			registerTimer(ctx, cleanUpTime);
			rightTimerState.update(cleanUpTime);
		} else {
			long cleanUpTime = rowTime + rightRelativeSize + cleanUpInterval + allowedLateness + 1;
			registerTimer(ctx, cleanUpTime);
