...
package org.apache.flink.table.runtime.operators.join.lookup.hash;
 ...
 */
public class KeyedAsyncLookupJoinRunner extends RichAsyncFunction<RowData, RowData> {
	...
	public KeyedAsyncLookupJoinRunner(
	 ...
	 *     see {@link KeyedAsyncLookupJoinWithCalcRunner#createFetcherResultFuture(Configuration)}</li>
	 *     <li>filter the result if a join condition exist,
	 *     see {@link KeyedAsyncLookupJoinRunner#createFetcherResultFuture(Configuration)}</li>
	...
	private static final class JoinedRowResultFuture implements ResultFuture<Object> {
		...
		public void complete(Collection<Object> result) {
			...
			if (rightRows == null || rightRows.isEmpty()) {
				if (isLeftOuterJoin) {
					...
					realOutput.completeOrRetry(Collections.singleton(outRow));
				} else {
					realOutput.completeOrRetry(Collections.emptyList());
				}
			} else {
		...
		private final class DelegateResultFuture implements ResultFuture<RowData> {
			...
			@Override
			public void completeExceptionally(Throwable error) {
				KeyedAsyncLookupJoinRunner.JoinedRowResultFuture.this.completeExceptionally(error);
			}
		}
	}
}
