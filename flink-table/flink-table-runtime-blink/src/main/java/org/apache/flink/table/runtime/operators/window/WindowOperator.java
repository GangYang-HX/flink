...
public abstract class WindowOperator<K, W extends Window>
	...
	private final boolean enableStateTtl;
	...
	WindowOperator(
		...
		this.enableStateTtl = enableStateTtl;
	...
	public void open() throws Exception {
		...
		long ttlTime = windowAssigner.retentionTime();
		if (ttlTime > 0) {
			ttlTime = ttlTime + stateRetentionTime;
		}
		StateTtlConfig ttlConfig = StateTtlConfigUtil.createTtlConfig(ttlTime);
		if (enableStateTtl && ttlConfig.isEnabled()) {
			windowStateDescriptor.enableTimeToLive(ttlConfig);
		}
		...
		if (produceUpdates) {
			...
			if (enableStateTtl && ttlConfig.isEnabled()) {
				previousStateDescriptor.enableTimeToLive(ttlConfig);
			}
