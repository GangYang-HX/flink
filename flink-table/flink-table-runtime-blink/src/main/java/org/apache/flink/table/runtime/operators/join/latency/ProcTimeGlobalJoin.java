 ...
 * zhouxiaogang Exp $$
 */
public class ProcTimeGlobalJoin extends KeyedCoProcessFunction<RowData, RowData, RowData, RowData> {

	public static final String LABEL = "redisJoin";
	...
	protected final long timerDelay = 120_000;
	...
	private final long keyedStateTtlSize;
...
//	private transient EmitAwareCollector joinCollector;
	...
	public ProcTimeGlobalJoin(
		...
		this.keyedStateTtlEnable = config.isKeyedStateTtlEnable();
		this.keyedStateTtlSize = config.getStateTtl();
	...
	public void open(Configuration parameters) throws Exception {
		...
		MapStateDescriptor<String, String> leftMapStateDescriptor = new MapStateDescriptor<>(
			...
			BasicTypeInfo.STRING_TYPE_INFO);
		if (keyedStateTtlEnable) {
			leftMapStateDescriptor.enableTimeToLive(StateTtlConfigUtil.createTtlConfig(keyedStateTtlSize));
		}
		...
		ttlCacheConfig.setMaxCacheSize(maxCacheSize);
...
//			joinedRow.replace(leftTableRow.getRow(),rightRow);
		...
		JoinRow existingRow = rightCache.get(rightKey);
		long s = System.nanoTime();
		//if right flow exists the key,then combine
		if (isMergeJoin && (existingRow != null)) {
			RowData currentRowData = existingRow.getRow();

			long currentCacheSize = 0;
			for (int i = 0; i < currentRowData.getArity(); i++) {
				currentCacheSize += currentRowData.getString(i).toString().length();
			}
			if (currentCacheSize >= maxCacheSize) {
				// if right row size has already larger than maxCacheSize, do not combine new keys, just drop them.
				metric.discardRight.inc();
				return;
			} else {
	...
	private RowData combineRightDuplicateKey(RowData rightRow1, RowData rightRow2) {
		...
		if (size > 1024 * 1024 * 2) {
			String rowString = rightRow1.getString(0).toString();
			LOGGER.warn("After right stream merging，the size : {} kb is greater than 2MB and the key is : {}",
				size / 1024,
				rowString.substring(0, Math.min(rowString.length(), 100)));
		}
		return newJoinedRow;
	}
	...
	public void onTimer(long timestamp, OnTimerContext ctx, Collector<RowData> out) throws Exception {
		...
		if(System.currentTimeMillis() - timestamp > timerDelay){
			timerMetric.timerDelay.inc();
			return;
		}
		metric.out.inc();
	...
	private static class Metric {
		...
		private Counter discardRight;
		private Counter timerIn;
		private Counter timerOut;
		private Counter timerDelay;
		...
		private Metric(MetricGroup metricGroup) {
			...
			discardRight = metricGroup.counter("discardRight");
			timerIn = metricGroup.counter("timerIn");
			timerOut = metricGroup.counter("timerOut");
			timerDelay = metricGroup.counter("timerDelay");
