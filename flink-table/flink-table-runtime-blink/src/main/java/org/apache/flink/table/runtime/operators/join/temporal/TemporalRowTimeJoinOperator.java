...
public class TemporalRowTimeJoinOperator
	...
	public void open() throws Exception {
		...
		ValueStateDescriptor<Long> nextIndexDescriptor = new ValueStateDescriptor<>(NEXT_LEFT_INDEX_STATE_NAME, Types.LONG);
		MapStateDescriptor<Long, RowData> leftStateDescriptor = new MapStateDescriptor<>(LEFT_STATE_NAME, Types.LONG, leftType);
		MapStateDescriptor<Long, RowData> rightStateDescriptor = new MapStateDescriptor<>(RIGHT_STATE_NAME, Types.LONG, rightType);
		ValueStateDescriptor<Long> timerDescriptor = new ValueStateDescriptor<>(REGISTERED_TIMER_STATE_NAME, Types.LONG);
		if (keyedStateTtlEnable) {
			StateTtlConfig ttlConfig = StateTtlConfigUtil.createTtlConfig(minRetentionTime);
			nextIndexDescriptor.enableTimeToLive(ttlConfig);
			leftStateDescriptor.enableTimeToLive(ttlConfig);
			rightStateDescriptor.enableTimeToLive(ttlConfig);
			timerDescriptor.enableTimeToLive(ttlConfig);
		}
		nextLeftIndex = getRuntimeContext().getState(nextIndexDescriptor);
		leftState = getRuntimeContext().getMapState(leftStateDescriptor);
		rightState = getRuntimeContext().getMapState(rightStateDescriptor);
		registeredTimer = getRuntimeContext().getState(timerDescriptor);
