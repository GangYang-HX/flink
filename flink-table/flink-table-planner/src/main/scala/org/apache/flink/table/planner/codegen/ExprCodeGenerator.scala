...
class ExprCodeGenerator(ctx: CodeGeneratorContext, nullableInput: Boolean)
  ...
  override def visitWindowAttrVariable(fieldRef: RexWindowAttrVariable): GeneratedExpression =
    throw new CodeGenException("rex window attr are not supported yet.")
