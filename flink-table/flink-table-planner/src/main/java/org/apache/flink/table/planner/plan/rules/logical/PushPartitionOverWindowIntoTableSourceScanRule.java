 ...
 * LogicalTableScan}.
 */
public class PushPartitionOverWindowIntoTableSourceScanRule extends RelOptRule {
    public static final PushPartitionOverWindowIntoTableSourceScanRule INSTANCE =
            new PushPartitionOverWindowIntoTableSourceScanRule();

    public PushPartitionOverWindowIntoTableSourceScanRule() {
        super(
                operand(
                        Filter.class,
                        operand(
                ...
                "PushPartitionOverWindowIntoTableSourceScanRule");
    }
    ...
    public boolean matches(RelOptRuleCall call) {
        ...
        TableSourceTable tableSourceTable = call.rel(3).getTable().unwrap(TableSourceTable.class);
    ...
    public void onMatch(RelOptRuleCall call) {
        Filter filter = call.rel(0);
        LogicalTableFunctionScan functionScan = call.rel(1);
        ...
        if (nonPartitionPredicate.isAlwaysTrue()) {
            ...
            LogicalProject newProject =
                    project.copy(
                            ...
                            project.getProjects(),
                            project.getRowType());
            LogicalTableFunctionScan newFunctionScan =
                    functionScan.copy(
                            functionScan.getTraitSet(),
                            Collections.singletonList(newProject),
                            functionScan.getCall(),
                            functionScan.getElementType(),
                            functionScan.getRowType(),
                            functionScan.getColumnMappings());
            Filter newFilter =
                    filter.copy(filter.getTraitSet(), newFunctionScan, nonPartitionPredicate);
