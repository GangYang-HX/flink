    ...
    public RowData deserialize(byte[] message) throws IOException {
        ...
        for (int position = 0; position < message.length; position++) {
            ...
            if (message[position] == 10) {
                ...
                //                logTraceError(trace, headers,
                // TraceErrorCode.CONTAINS_NEWLINE_CHARACTER.errorCode, sinkDest);
            }
            ...
            if (currentFieldIndexInRowData >= rowType.getFieldCount()) {
                ...
                hasSomeUnexpected = true;
                //                logTraceError(trace, headers,
                // TraceErrorCode.TOO_MANY_FIELDS.errorCode, sinkDest);
                break;
            }
            ...
            if (current == delimiterKey) {
                ...
                try {
                    ...
                    //                    logTraceError(trace, headers,
                    // TraceErrorCode.PARSE_ERROR.errorCode, sinkDest);
                    if (useLancerDebug) {
        ...
        if (currentFieldIndexInRowData < rowType.getFieldCount()) {
            for (int i = currentFieldIndexInRowData; i < rowType.getFieldCount(); i++) {
                ...
                //                logTraceError(trace, headers, TraceErrorCode.MISS_FIELD.errorCode,
                // sinkDest);
            ...
            if (message.length == 0 || message[message.length - 1] != delimiterKey) {
                hasSomeUnexpected = true;
                //                logTraceError(trace, headers,
                // TraceErrorCode.END_WITHOUT_DELIMITER_KEY.errorCode, sinkDest);
            }
        }
