...
public class HadoopUtils {
	...
	// in 1.11.3, we can not get ExecutionOpitions in HadoopUtils, 
	// cause ExecutionOpitions was removed to flink-core
	private static final String CALLER_CONTEXT_APP_ID_KEY = "execution.caller-context-app-id";
	...
	public static Configuration getHadoopConfiguration(org.apache.flink.configuration.Configuration flinkConfiguration) {
		...
		if (!foundHadoopConfiguration) {
			...
			Supplier<Boolean> approachOne = new Supplier() {
				@Override
				public Boolean get() {
					Boolean foundHadoopConfiguration = false;
			...
			Supplier<Boolean> approachTwo = new Supplier<Boolean>() {
				@Override
				public Boolean get() {
					Boolean foundHadoopConfiguration = false;
					...
					if (hdfsDefaultPath != null) {
						...
						LOG.info("Using hdfs-default configuration-file path from Flink config: {}", hdfsDefaultPath);
					...
					if (hdfsSitePath != null) {
						...
						LOG.info("Using hdfs-site configuration-file path from Flink config: {}", hdfsSitePath);
						foundHadoopConfiguration = true;
					}
					...
					if (hadoopConfigPath != null) {
						LOG.info("Searching Hadoop configuration files in Flink config: {}", hadoopConfigPath);
			...
			Supplier<Boolean> approachThree = new Supplier<Boolean>() {
				@Override
				public Boolean get() {
					Boolean foundHadoopConfiguration = false;
			...
			Supplier<Boolean> approachFour = new Supplier<Boolean>() {
				@Override
				public Boolean get() {
					Boolean foundHadoopConfiguration = false;
					String secondHadoopConfig = flinkConfiguration.getString(ConfigConstants.USER_DEFINE_CONFIG_DIR, USER_DEFINE_CONFIG_DEFAULT_PATH);
					if (secondHadoopConfig != null) {
						LOG.info("Searching Second Hadoop configuration files in : {}", secondHadoopConfig);
						foundHadoopConfiguration = addHadoopConfIfFound(result, secondHadoopConfig) || foundHadoopConfiguration;
					}
					return foundHadoopConfiguration;
				}
			};
			...
			List<Supplier<Boolean>> approaches = Arrays.asList(approachOne, approachTwo, approachThree, approachFour);
	 ...
	 */
	public static void setCallerContextJobID(String jobID,
											 org.apache.flink.configuration.Configuration flinkConfiguration) {
		if (isMinHadoopVersion(2,8)) {
			String callerContext = CALLER_CONTEXT_JOB_ID_KEY_STRING + CALLER_CONTEXT_KEY_VALUE_DELIMITER +
				CALLER_CONTEXT_JOB_ID_PREFIX_STRING +
				flinkConfiguration.getString(CALLER_CONTEXT_APP_ID_KEY, "") +
				CALLER_CONTEXT_CONTENT_DELIMITER + jobID;
			CURRENT_JOB_ID.set(CALLER_CONTEXT_JOB_ID_PREFIX_STRING +
				flinkConfiguration.getString(CALLER_CONTEXT_APP_ID_KEY, "") +
				CALLER_CONTEXT_CONTENT_DELIMITER + jobID);
			try {
