...
public class HadoopRecoverableWriter implements RecoverableWriter {
	...
	public void setMetricWrapper(MetricsWrapper wrapper) {
		this.metricsWrapper = wrapper;
	}
	...
	public RecoverableFsDataOutputStream open(Path filePath) throws IOException {
		...
		return new HadoopRecoverableFsDataOutputStream(fs, targetFile, tempFile, metricsWrapper);
	}
	...
	public RecoverableFsDataOutputStream recover(ResumeRecoverable recoverable) throws IOException {
		if (recoverable instanceof HadoopFsRecoverable) {
			return new HadoopRecoverableFsDataOutputStream(fs, (HadoopFsRecoverable) recoverable, metricsWrapper);
		}
		else {
...
//					parent, name + ".inprogress." + UUID.randomUUID().toString() + ".tmp");
					parent, "." + name + ".inprogress." + UUID.randomUUID().toString() + ".tmp");
