...
class HadoopRecoverableSerializer implements SimpleVersionedSerializer<HadoopFsRecoverable> {
	...
	public byte[] serialize(HadoopFsRecoverable obj) throws IOException {
		...
		final byte[] targetBytes = new byte[20 + targetFileBytes.length + tempFileBytes.length + 8];
	...
	private static HadoopFsRecoverable deserializeV1(byte[] serialized) throws IOException {
		...
		long size = 0;
		// 兼容新版序列化CK恢复，同时保证回滚可从CK恢复
		if (28 == serialized.length - targetFileBytes.length - tempFileBytes.length) {
			size = bb.getLong();
		}
		...
		HadoopFsRecoverable recoverable = new HadoopFsRecoverable(new Path(targetPath), new Path(tempPath), offset);
		recoverable.setSize(size);
		return recoverable;
