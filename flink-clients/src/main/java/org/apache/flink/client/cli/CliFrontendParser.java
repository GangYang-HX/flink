...
public class CliFrontendParser {
	...
	private static Options getRunOptionsWithoutDeprecatedOptions(Options options) {
		...
		return o.addOption(SAVEPOINT_ALLOW_NON_RESTORED_OPTION).addOption(SAVEPOINT_RESTORE_MODE);
	}
	...
	public static SavepointRestoreSettings createSavepointRestoreSettings(CommandLine commandLine) {
		if (commandLine.hasOption(SAVEPOINT_PATH_OPTION.getOpt())) {
			...
			if (commandLine.hasOption(SAVEPOINT_RESTORE_MODE.getOpt())) {
				restoreMode = RestoreMode.valueOf(commandLine.getOptionValue(SAVEPOINT_RESTORE_MODE.getOpt()).toUpperCase());
			} else {
